...
/* All code after this line entirely the creation and (c) Eric Albert, 1999-2001. Used as per conditions. */
 ...
 *
 * <p>Yes, this is platform-specific code, and yes, it may rely on classes on certain platforms that
 * are not part of the standard JDK. What we're trying to do, though, is to take something that's
 * frequently desirable but inherently platform-specific -- opening a default browser -- and allow
 * programmers (you, for example) to do so without worrying about dropping into native code or doing
 * anything else similarly evil.
 *
 ...
 * found, will not cause this to do anything other than returning an error when opening the browser.
 *
 ...
 * have the Internet Scripting AppleScript dictionary installed in the Scripting Additions folder in
 * the Extensions folder (which is installed by default as far as I know under Mac OS 8.0 and 8.1),
 * and for all Mac OS 8.5 and later systems. On Windows, it only runs under Win32 systems (Windows
 * 95, 98, and NT 4.0, as well as later versions of all). On other systems, this drops back from the
 * inherently platform-sensitive concept of a default browser and simply attempts to launch Netscape
 * via a shell command.
 *
 * <p>This code is Copyright 1999-2001 by Eric Albert (ejalbert@cs.stanford.edu) and may be
 * redistributed or modified in any form without restrictions as long as the portion of this comment
 ...
 * href="http://www.javaworld.com/javaworld/javatips/jw-javatip66.html">Java Tip 66</a>) <br>
 * Thanks also to Ron B. Yeh, Eric Shapiro, Ben Engber, Paul Teitlebaum, Andrea Cantatore, Larry
 * Barowski, Trevor Bedzek, Frank Miedrich, and Ron Rabakukk
 *
...
public class BrowserLauncher {
  ...
  /**
   * The Java virtual machine that we are running on. Actually, in most cases we only care about the
   * operating system, but some operating systems require us to switch on the VM.
   */
   ...
   * dynamically loaded at runtime loaded successfully.
   *
   * <p>Note that if this is <code>false</code>, <code>openURL()</code> will always return an
   * IOException.
   */
   ...
   *
   * @return The browser for the system. Note that this may not be what you would consider to be a
   *     standard web browser; instead, it's the application that gets called to open the default
   *     web browser. In some cases, this will be a non-String object that provides the means of
      ...
      case MRJ_2_0:
        try {
          Integer finderCreatorCode =
              (Integer) makeOSType.invoke(null, new Object[] {FINDER_CREATOR});
  ...
  /**
   * Methods required for Mac OS X. The presence of native methods does not cause any problems on
   * other platforms.
   */
  ...
  private static native int ICLaunchURL(
      int instance, byte[] hint, byte[] data, int len, int[] selectionStart, int[] selectionEnd);
}
