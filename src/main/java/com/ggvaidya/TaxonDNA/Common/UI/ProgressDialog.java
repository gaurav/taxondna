...
public class ProgressDialog extends Dialog
  ...
  /**
   * This function causes the window to close and dispose itself. This will cause the new thread we
   * forked to keep the setVisible in to die. This function MUST be called once the delay is "done",
   * because leaving a modal dialog in front of your main frame is no way to run a program.
   */
  public void end() {
    while (!isVisible()) ;
    setVisible(false);
   ...
   *
   * <p>Big change: now, WE will handle the reporting, i.e. unless the new fraction is significantly
   * (0.1%) larger than the last one, we will ignore it. This means EVERYBODY - effective
   * immediately - must use delay.delay() without going through that interval shit.
   */
  ...
  /**
   * actionPerformed, used to set up a "Cancel" button. Once we actually have a cancel button, it'll
   * be really cool. We promise.
   */
