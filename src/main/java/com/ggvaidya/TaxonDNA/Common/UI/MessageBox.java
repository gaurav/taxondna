 ...
 * Create an object of this class, then call the showMessageBox() function to display it. You can
 * also call go() if you are not interested in the response.
 *
 * <p>The static function messageBox() can also be used to do it all in one step.
 *
...
public class MessageBox extends Dialog implements ActionListener {
    ...
    // flags
    /**
     * Specify that the title is unique to the entire program, and can be used to uniquely identify
    ...
    // Frame constructors
    /**
     * Creates a messagebox, with the given parent frame, title and message. The messagebox is an
     * "Error" box, by default.
     */
    ...
    /**
     * Creates a messagebox, with the given parent frame, title, message and flags. The flags can be
     * used to change the type of the message box, and/or the buttons displayed.
     */
    ...
    public MessageBox(Dialog parent, String title, String message, int flags) {
        ...
        if ((flags & MB_YESNOTOALL) != 0) {
            if (((flags & MB_TITLE_IS_UNIQUE) == 0) && ((flags & MB_MESSAGE_IS_UNIQUE) == 0)) {
                throw new IllegalArgumentException(
                        "MessageBox creation attempted with MB_YESNOTOALL set, but without either"
                                + " MB_TITLE_IS_UNIQUE or MB_MESSAGE_IS_UNIQUE set!");
            }
        }
    }

    /**
     * A static 'constructor', if all you've got is a Window. We'll figure out whether it's a Dialog
     * or a Frame, and fire the appropriate constructor.
     */
    public static MessageBox createFromWindow(
            Window parent, String title, String message, int flags) {
        MessageBox mb = null;
        ...
        if (parent.getClass().equals(Frame.class))
            ...
            throw new IllegalArgumentException(
                    "The parent window (" + parent + ") is neither a Frame nor a Dialog!");

        return mb;
    }

    /**
     * A static 'constructor', if all you've got is a Window, without a flags field. We'll figure
     * out whether it's a Dialog or a Frame, and fire the appropriate constructor.
     */
    ...
    /** Our internal draw function, which actually creates the messagebox when it needs to be. */
    private void draw() {
        removeAll();
    ...
    /** Shows the messageBox. will return one of the MB_OK, MB_CANCEL, etc. constants */
    public int showMessageBox() {
        if ((flags & MB_YESNOTOALL) != 0
                && hash_setForAll.get(this.getUniqueIdentifier()) != null) {
    ...
    public String getUniqueIdentifier() {
        if ((flags & MB_MESSAGE_IS_UNIQUE) != 0) return message;
        else return title;
    }

    /** If you don't need to retrieve the clicked button, you can use this instead. */
    public void go() {
        showMessageBox();
    }
