 ...
 * RightLayout.java
 *
 * <p>A rectilinear layout is one with lots of rectangles. In this case, it's a hack resembling a
 * much more flexible GridLayout hacked over the real underlying class (which is the intensively
 * complicated GridBagLayout).
 *
 * <p>The way it works is this: 1. You add objects. Every time you add an object, you tell me
 * whether you want it BESIDE or UNDER the last one. Thus, to make following layout: A B C D you
 * would use: add(A); // Default is BESIDE add(B, RightLayout.BESIDE); add(C, RightLayout.UNDER);
 * add(D, RightLayout.BESIDE); Yes, it's a bit simplistic, but it's all I need. If you need more,
 * you can always ring up a proper GridBagLayout.
 */
...
public class RightLayout implements LayoutManager2 {
  ...
  public void addLayoutComponent(Component comp, Object constraints) {
    ...
    if ((flags & BESIDE) != 0) {
      ...
      if (gbc.gridx > right_most) right_most = gbc.gridx;
    }
  ...
  /**
   * Our custom add-with-flags. You can use addLayoutComponent (via Container.add(component,
   * constraints), but this is really so much easier, as you don't ahve to keep defining an
   * 'Integer' to handle the flags
   */
   ...
   * From LayoutManager
   */
  // We don't do this, so we'll pretend like the String is the constraint (whatever sense that
  // makes)
