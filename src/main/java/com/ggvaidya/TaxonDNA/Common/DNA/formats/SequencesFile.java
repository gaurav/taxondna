/**
 * A very simple but complete format for biological sequences. It works like this:
 *
 * <p>1. The 'header', on the FIRST line, needs to be '#sequences'. Plural! That's our file
 * identification magic word.
 *
 * <p>A 'straight' sequences file can actually avoid the header entirely: in which case it degrades
 * straight into a perfectly ordinary Fasta file!
 *
 * <p>The header is crucial because it contains the capabilities string, which outlines which set of
 * commands this file contains. This will allow parses to quickly identify what they are capable of
 * (TaxonDNA can warn the user that it will disregard protein sequences if the 'protein' command is
 * used, for instance). The capabilities string is a space delimited string on the same line as the
 * header. It is put into brackets to stand out. For instance:
 *
 * <p>#sequences (nucleotide taxonomy)
 *
 * <p>2. '#' is a comment indicator; ANY '#' is removed and everything after it is completely
 * ignored. You have been warned.
 *
 * <p>3. Sequence names are indicated with '&gt;'. After that, anything goes until the end of line.
 *
 * <p>4. Everything without a special starting character is 'data'. Data is associated with the last
 * sequence named, or '0' if there hasn't been one yet. Anything goes, but it's up to the program
 * what it will accept or reject (or complain about)
 *
 * <p>5. '&gt;' is used to change the sequence name (like with Fasta). This means that to convert a
 * Fasta file into a Sequences file (why would you want to?), you just need to add the magic word to
 * the top.
 *
 * <p>6. '@' is used to indicate a global command. The first word on the line will indicate the
 * nature of the command. If you do not understand a command, GO ON. Global commands will NOT affect
 * the data processing, only the data interpretation. If you don't look upon data that way, that's
 * fine.
 *
 * <p>If we really need to use some sort of strictiness, we will eventually introduce a '@strict'
 * command which will force strict processing.
 *
 * <p>'@' commands are read in the order entered RIGHT AT THE VERY END OF THE FILE. Basically, all
 * '@' commands ought to be read up into a buffer, then processed one by one in the order they were
 * present in the file. This is to bring home the message that '@' commands are GLOBAL commands
 * which do NOT affect data processing.
 *
 * <p>7. '^' commands refer to the LAST sequence mentioned. These should be processed in order of
 * appereance JUST BEFORE the next sequence name is set. This will be used to set sequence-specific
 * parameters etc.
 *
 * <p>As before, any commands you don't understand can be ignored.
 *
 * <p>8. You can use the capabilities string (see #1) to check for which commands you _can_
 * understand. Any commands not specified in the capabilities can be ignored, or informed to the
 * user, at your convenience.
 *
 * <p>ADDENDUM 1. Names which are composed entirely of whitespace are allowed, and accepted as such.
 * Really.
 *
 * <p>SECTION 2: TAG GROUPS nucleotide: the sequence information is nucleotide information taxonomy:
 * ^gi GI number ^species Species name ^subspecies Subspecies name ^family Family name
 *
...
public class SequencesFile extends BaseFormatHandler implements Testable {
  ...
  /**
   * Helper function to get a key/value pair out of sequences. There are three better ways of
   * writing this function depending on whether we can use: 1. Multiple return values 2.
   * Continuations 3. References/pointers
   *
   * @param ret An array of two parts, both strings. ret[0] is filled in with the key, ret[1] is
   *     filled in with the value.
   */
      ...
      if (!handled)
        throw new FormatException(
            "Local command '"
                ...
                + "' in sequence "
                + seq.getFullName()
                + " cannot be interpreted by this program!");
    }
  }
  ...
  public void fireGlobalCmds(SequenceList list, Vector globalCmds) throws FormatException {
    ...
    while (i.hasNext()) {
      ...
      if (!handled)
        throw new FormatException(
            "Global command '" + globalCmd + "' cannot be interpreted by this program!");
    }
  }
  ...
  private String getLocalCommands(Sequence seq) {
    ...
    if (buff == null) return null;
      ...
      return buff.toString()
          .trim(); // this is okay, since the local command will be written out with a writeln
    // anyway
  }
  ...
  public void appendFromFile(SequenceList list, File file, DelayCallback delay)
    ...
    while (reader.ready()) {
      ...
      try {
        if (delay != null) delay.delay(line_no, count_lines);
  ...
  public void writeFile(File file, SequenceList set, DelayCallback delay)
    ...
    while (i.hasNext()) {
      try {
        if (delay != null) delay.delay(x, count);
      } catch (DelayAbortedException e) {
        writer.close();
        if (delay != null) delay.end();
        throw e;
      ...
      if (commands != null) writer.println(commands);
  ...
  public void test(TestController testMaster, DelayCallback delay) throws DelayAbortedException {
    ...
    else testMaster.failed(test + " was not recognized as a FASTA file");
    ...
    else testMaster.failed(notfasta + " was incorrectly identified as a FASTA file");
    ...
    if (spaces.canRead() && ff.mightBe(spaces)) {
      try {
        ...
        if (count == 3) testMaster.succeeded();
        else testMaster.failed("I got back " + count + " sequences instead of 3!");
      } catch (IOException e) {
        testMaster.failed("There was an IOException reading " + test + ": " + e);
      } catch (SequenceException e) {
        testMaster.failed("There was a SequenceException reading " + test + ": " + e);
    ...
    try {
      ...
      if (testMaster.isIdentical(success, output2)) testMaster.succeeded();
      else
        testMaster.failed(
            "I read a FASTA file from "
                + input
                + ", then wrote it to '"
                + output
      ...
      testMaster.failed(
          "I read a FASTA file from "
              ...
              + ", then wrote it to '"
              + output
              + "', but I got a SequenceListException: "
              + e);
    } catch (FormatException e) {
      testMaster.failed(
          "I read a FASTA file from "
