/**
 * A file format handler from DNA.formats. Each format handler is tied in to different file formats
 * - there would be one for FASTA files, one for Mega files, etc. You can interact with that code
 * using five methods:
 *
 * <p>String getShortName(): returns the short name of the format String getExtension(): the default
 * extension for this format. Should be three letters (just in case). String getLongName(): returns
 * the long name of the format void readFile(): reads a file, and appends it onto a SequenceList
 * void writeFile(): writes a SequenceList into the specific file boolean mightBe(): checks the file
 * signature and syntax to see if it might possibly belong to this format. This should be relatively
 * fast: we'll be running through the entire list, looking for matches.
 *
 * <p>This is fairly low level, and so you'll find long lists of exceptions so that people can
 * figure out what's going on. If you want a simple way of loading a file, look at SequenceList.
 */
