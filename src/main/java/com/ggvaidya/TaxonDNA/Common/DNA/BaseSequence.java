/**
 * A BaseSequence is a Sequence (like, DNA.Sequence) which can hold any data you'd like. It *is*
 * derived from DNA.Sequence, so hopefully it'll just sneak along with any
 * SequenceLists/UIExtensions/etc., but will completely fail to be compared to a *real* Sequence.
 *
 * <p>Note that if you need to make another type of sequence, you will - in this model - have to
 * extend Sequence, NOT BaseSequence.
 *
 * <p>Note that every time anybody adds anything to Sequence, somebody will have to add
 * no-I-don't-mean-you code to BaseSequence.
 *
 * <p>Note that this is all really very evil and grimy, but it's quick and cheap, and pulling
 * Sequence out from underneath TaxonDNA will be several different kinds of painful.
 *
 * <p>ALSO TO BE NOTED that BaseSequence is really a sub-in for 'symbolic' data; i.e. Any Symbol
 * Will Do. The downside of this is that we, and we alone, need to handle symbols which represent
 * other symbols.
 *
 * <p>The current proposal is to do that the dead-simple way: every incoming string will be checked
 * for completeness in bracketing (no_closing_brackets === no_opening_brackets). BOTH square and
 * round brackets are assumed to be bracketing information; you can use angle brackets if you need
 * to use your own bracketing symbol we'll ignore. When we're asked for the sequence, we return it
 * with 'TaxonDNA' brackets (which are by definition square brackets).
 *
 * <p>The only really tricky thing here is getSubsequence(), which must count indexes using
 * bracketting information. For now, each and every getSubsequence() request will traverse the
 * entire sequence, one character at a time, then traverse the copy region, again, one character at
 * a time.
 *
 * <p>If that doesn't work, we can store the sections in chunks, viz: chunk# string 0: 1394919191 1:
 * [124 2: 56193381 Any chunch not beginning with '[' can be assumed to have the length of
 * str.length(), while any chunk beginning with '[' has a length of 1, unless the 2nd character is
 * [[, in which case it has a lenght of (str.length() - 1). But you know what they say about
