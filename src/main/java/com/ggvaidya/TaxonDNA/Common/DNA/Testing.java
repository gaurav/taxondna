/**
 * Testing tests the DNA segment. You can either call Testing.testAll() (or another 'testing'
 * function) or you can run the class directly (we've got our own main()).
 */
    ...
    public static void testTransScoring() {
        ...
        try {
            ...
            for (int x = 0; x < pyrimidines.length; x++) {
                if (!Sequence.isPyrimidine(pyrimidines[x]))
                    println(
                            "ERROR: pyrimidine "
                                    + pyrimidines[x]
                                    + " is not counted as a pyrimidine!");
            ...
            if (seq_a.getPairwise(seq_b) != 0.2)
                println(
                        "Incorrect: ought to be 0.2, reported as "
                                + seq_a.getPairwise(seq_b)
                                + "; #trans = "
                                + seq_a.countTransversions(seq_b)
                                + "; overlap = "
                                + seq_a.getSharedLength(seq_b));
            else println("Simple test successful!");
