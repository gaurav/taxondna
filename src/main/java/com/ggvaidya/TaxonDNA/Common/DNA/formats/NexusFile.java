...
public class NexusFile extends BaseFormatHandler {
        ...
        try {
            ...
            if (tok.nextToken() != '#')
                throw new FormatException(
                        "This file does not have a Nexus header (it doesn't start with '#')! Are"
                                + " you sure it's a Nexus file?");

            tok.nextToken();

            if (tok.sval == null || !tok.sval.equalsIgnoreCase("nexus")) {
                throw new FormatException(
                        "This file does not have a Nexus header (it doesn't start with '#nexus')!"
                                + " Are you sure it's a Nexus file?");
            }
                    ...
                    // the BEGIN command
                        ...
                        if (tok.nextToken() == NexusTokenizer.TT_WORD) {
                            ...
                            if (beginWhat.equalsIgnoreCase("DATA")
                                    || beginWhat.equalsIgnoreCase("CHARACTERS"))
                            ...
                            // you know. to be anal, and all that.
                                ...
                                inStrangeBlock = true;
                                // warn the user!
                                // delay.addWarning("Block '" + beginWhat + "' cannot be read by
                                // TaxonDNA yet! This
                                // block will NOT be imported, and therefore cannot be
                                // re-exported.");
                            }
                            ...
                            // something is wrong!
                            throw formatException(
                                    tok, "BEGIN is specified without a valid block name.");
                        }
                    } else {
                        if (!inStrangeBlock)
    ...
    public void blockData(
        ...
        while (true) {
            ...
            if (type == NexusTokenizer.TT_EOF) {
                ...
                throw formatException(
                        tok,
                        "I've reached the end of the file, and the DATA/CHARACTERS block has"
                            + " *still* not been closed! Please make sure the block is closed.");
            }
                ...
                if (type == NexusTokenizer.TT_WORD) {
                    ...
                    if (str.equalsIgnoreCase("INTERLEAVE")) {
                        ...
                        if (str == null)
                            throw formatException(
                                    tok,
                                    "'DATATYPE' is misformed (or there's a comment in there"
                                            + " somewhere. I can't abide comments in FORMAT).");
                        ...
                        if (str.equalsIgnoreCase("CONTINUOUS"))
                            throw formatException(
                                    tok,
                                    "I can't understand DATATYPE '"
                                            + str
                                            + "'. I can only understand DATATYPE=DNA.");
                    } else if (str.equalsIgnoreCase("GAP")) {
                        str = getValueOfKey(tok);
                        if (str == null)
                            throw formatException(
                                    tok,
                                    "'GAP' is misformed (or there's a comment in there somewhere. I"
                                            + " can't abide comments in FORMAT).");

                        if (str.length() > 1)
                            throw formatException(
                                    tok,
                                    "I can't use more than one character as the GAP character. The"
                                            + " file specifies: '"
                                            + str
                                            + "'");
                        ...
                        if (str == null)
                            throw formatException(
                                    tok,
                                    "'MISSING' is misformed (or there's a comment in there"
                                            + " somewhere. I can't abide comments in FORMAT).");

                        if (str.length() > 1)
                            throw formatException(
                                    tok,
                                    "I can't use more than one character as the MISSING character."
                                            + " The file specifies: '"
                                            + str
                                            + "'");
                        ...
                        // delay.addWarning("FORMAT SYMBOLS=... in the DATA/CHARACTERS block cannot
                        // be read by
                        // TaxonDNA yet. There is no guarantee that illegal symbols are not being
                        // used in your
                        // sequences.");
                    } else {
                        throw formatException(
                                ...
                                "I found '"
                                        + str
                                        + "' in the FORMAT line of the DATA (or CHARACTERS) block."
                                        + " I don't understand "
                                        + str
                                        + " at the moment - I can only comprehend 'MISSING=x',"
                                        + " 'GAP=x', 'DATATYPE=DNA' and 'INTERLEAVE'. Sorry!");
                    }
                } else {
                ...
                if (type == ';') {
                    if (name != null) {
                        ...
                        throw formatException(
                                ...
                                "The sequence named '"
                                        + name
                                        + "' has no DNA sequence associated with it.");
                    }
                    inMatrix = false;
                    ...
                    while (type != ')') {
                        ...
                        if (type == NexusTokenizer.TT_WORD) {
                            ...
                            throw formatException(
                                    tok, "Unterminated brackets in sequence '" + name + "'");
                        else
                            throw formatException(
                                    tok, "Unexpected '" + (char) type + "' in ambiguous data");
                    }
                    if (name == null) {
                        // wtf name?!
                        ...
                        try {
                            ...
                            throw formatException(
                                    tok, "The sequence for taxon '" + name + "' is invalid: " + e);
                        }
                    }
                ...
                if (type == NexusTokenizer.TT_WORD) {
                    ...
                    if (str.equalsIgnoreCase("MATRIX")) {
                        if (gapChar == missingChar) {
                            ...
                            delay.addWarning(
                                    "This Nexus file defines BOTH the gap character and the missing"
                                            + " character as '"
                                            + gapChar
                                            + "'. I will use this as the gap character only; no"
                                            + " missing data will be recognized for this dataset.");
                        }
                    ...
                    if (str.equalsIgnoreCase("DIMENSIONS")
                        ...
                        // delay.addWarning("Ignoring command '" + str + "' in the CHARACTERS/DATA
                        // block;
                    ...
                    if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
                        ...
                        break;
                        //						} else {
                        //							throw formatException(tok, "I found something strange after the
                        // END! I can't
                    ...
                    throw formatException(
                            ...
                            "I found '"
                                    + (char) type
                                    + "' rather unexpectedly in the DATA/CHARACTERS block! Are you"
                                    + " sure it's supposed to be here?");
                }
            }
    ...
    public void blockSets(
        ...
        while (true) {
            ...
            if (type == NexusTokenizer.TT_EOF) {
                ...
                throw formatException(
                        tok,
                        "I've reached the end of the file, and the SETS block has *still* not been"
                                + " closed! Please make sure the block is closed.");
            }
            ...
            if (inIgnoredCommand) {
                ...
                if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
                    ...
                    break;
                    //					} else {
                    //						throw formatException(tok, "I found something strange after the END! I
                    // can't just
                    // ignore it. I'm sorry.");
                    //					}
                    ...
                    if ((type = tok.nextToken()) != NexusTokenizer.TT_WORD)
                        throw formatException(
                                ...
                                "Unexpected symbol '"
                                        + (char) type
                                        + "' found after 'CHARSET'. This doesn't look like the name"
                                        + " of a CHARSET to me!");
                    ...
                    while (true) {
                        ...
                        if ((char) type == ';') {
                            ...
                            try {
                                ...
                                throw formatException(
                                        ...
                                        "I found a non-number in CHARSET "
                                                ...
                                                + " (the non-number is '"
                                                + str
                                                + "'). I can't currently understand 'ALL' or"
                                                + " 'REMAINDER' commands. Can you please remove"
                                                + " them from the source file if that's at all"
                                                + " possible?");
                            }
                            ...
                            throw formatException(
                                    tok,
                                    "Unexpected '" + (char) type + "' in CHARSET " + name + ".");
                        }
                    }
                    ...
                    inIgnoredCommand = true;
                    // delay.addWarning("Command '" + str + "' in the SETS block cannot be read by
                    // TaxonDNA,
                    // and will be ignored.");
                } else {
                    throw formatException(
                            tok, "Unknown word/command '" + str + "' found in the SETS block.");
                }
            } else {
                throw formatException(
    ...
    /**
     * Extracts a single CodonPosSet range for a particular codon position. I suppose we'll
     * eventually be generating events for this, but for now we need to consume all that
     * information.
     *
     * <p>Note that the first argument is 0 for 'N', and 1, 2, 3 for the three positions
     * respectively.
     */
    private void addCodonPosSet(
        ...
        while (true) {
            ...
            try {
                ...
                throw formatException(
                        tok,
                        "One of the values in CodonPosSet position "
                                + position
                                + " wasn't a number: "
                                + e);
            }
    ...
    public void blockCodons(
        ...
        while (true) {
            ...
            if (type == NexusTokenizer.TT_EOF) {
                ...
                throw formatException(
                        tok,
                        "I've reached the end of the file, and the CODONS block has *still* not"
                                + " been closed! Please make sure the block is closed.");
            }
            ...
            if (inIgnoredCommand) {
                ...
                if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
                    ...
                    // okay, the line looks like 'CODONPOSSET * \w+ = N: {desc}+, 1: {desc}+, 2:
                    // {desc}+, 3:
                    // {desc}+;'. And hopefully that's all we'll have to process.
                    // where:
                    //      desc = ((\d+)-(\d+)(\\3))*
                    // let's just be strict for now, and pretend that anything even slightly off
                    // (except
                    ...
                    if (tok.nextToken() != NexusTokenizer.TT_WORD) {
                        throw formatException(
                                tok,
                                "Expecting the name of the CodonPosSet, but got "
                                        + tok
                                        + " instead");
                    }
                    ...
                    if (tok.nextToken() != '=') {
                        throw formatException(
                                tok, "Expecting a '=', but got something else altogether.");
                    }

                    if (codonposset_already_defined) {
                        delay.addWarning(
                                "More than one CODONPOSSET command was found in this file. Only the"
                                        + " first one will be added.");
                    }
                    ...
                    for (int x = 0; x < str_positions.length; x++) {
                        if (token != NexusTokenizer.TT_WORD
                                || !tok.sval.equalsIgnoreCase(
                                        str_positions[
                                                x])) // throw formatException(tok, "I expect '" +
                        // str_positions[x] + "' to
                        ...
                        addCodonPosSet(
                                ...
                                evt,
                                tok,
                                !codonposset_already_defined); // Only add the first one.
                        token = tok.nextToken();
                    }
                    ...
                    inIgnoredCommand = true;
                    // delay.addWarning("Command '" + str + "' in the CODONS block cannot be read by
                    // TaxonDNA,
                    // and will be ignored.");
                } else {
                    throw formatException(
    ...
    /**
     * Gets the String 'value' of the key=value pair from the NexusTokenizer. I assume you've
     * already got the 'key', so I'll get the '=' token, followed by a TT_WORD, and return the
    ...
    private String getValueOfKey(NexusTokenizer tok) throws FormatException, IOException {
        if (tok.nextToken() == '=') {
            ...
            if (type == NexusTokenizer.TT_WORD) {
                // yay! we got a word
                return tok.sval;
            } else if (!Character.isWhitespace((char) type)
    ...
    public void writeNexusFile(
                ...
                if (no == 10000) {
                    ...
                    throw new IOException(
                            "There are 9999 sequences named '"
                                    ...
                                    + "', which is the most I can handle. Sorry. This is an"
                                    + " arbitary limit: please let us know if you think we set it"
                                    + " too low.");
                }
            }
        ...
        if (!interleaved) {
            ...
            while (i_names.hasNext()) {
                ...
                writer.println(
                        display_name + " " + seq.getSequence() + " [" + seq.getLength() + "]");
                    ...
                    try {
                        ...
                        throw new IOException(
                                "Could not get subsequence ("
                                        ...
                                        + ") from sequence "
                                        + seq
                                        + ". This is most likely a programming error. The reason"
                                        + " given was: "
                                        + e.getMessage());
                    }
        ...
        for (int x = 1; x <= 3; x++) {
            ...
            if (v != null) {
                ...
                while (i_v.hasNext()) {
                    ...
                    // buff_nexus_positions.append("(" + ftp.from + ") - (" + ftp.to + ")" +
                    // str_end);
    ...
    public void writeNexusFile(
        ...
        if (how != EXPORT_AS_BLOCKS
                && how != EXPORT_AS_SINGLE_LINE
                && how != EXPORT_AS_INTERLEAVED) {
                ...
                while (i_cols.hasNext()) {
                    ...
                    if (how == EXPORT_AS_SINGLE_LINE) writer.print(seq.getSequence());
                        ...
                        throw new RuntimeException(
                                "'how' makes no sense in NexusFile.exportAsNexus()! [how = "
                                        + how
                                        + "]");
                }
