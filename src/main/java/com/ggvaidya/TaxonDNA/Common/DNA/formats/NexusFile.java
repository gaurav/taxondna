/**
 * NexusFile allows you to read and write Nexus files. I'm going to try to replicate the format from
 * Maddison, D. et. al. (1997). NEXUS: An extensible file format for systematic information. Syst.
 * Biol. 46(4): 590-621. Writing will be written first, so this program might have a *little* more
 * difficulty reading Nexus files than it does writing them. Also, to simplify things, we always
 * "write fresh" - we forget that we read out of a file, and write back exactly whatever we got out
 * of it.
 *
 * <p>TODO: 1. Nexus has a different way of handling polymorphisms: [AT] instead of one letter
 * shortages. So we need to convert it both ways (right now, we convert NEITHER) 2. 'block
 * CHARACTERS: FORMAT symbols'
 */
...
public class NexusFile extends BaseFormatHandler {
  ...
  /**
   * Returns a valid Mega OTU (Operation Taxonomic Unit), that is, a taxon name. Note that this
   * function isn't actually being used. Freaky.
   */
    ...
    try {
      ...
      if (tok.nextToken() != '#')
        throw new FormatException(
            "This file does not have a Nexus header (it doesn't start with '#')! Are you sure it's a Nexus file?");

      tok.nextToken();

      if (tok.sval == null || !tok.sval.equalsIgnoreCase("nexus")) {
        throw new FormatException(
            "This file does not have a Nexus header (it doesn't start with '#nexus')! Are you sure it's a Nexus file?");
      }
      ...
      while (true) {
        ...
        if (type == NexusTokenizer.TT_EOF) break;

        // is it a comment?
        if (type == '[') commentLevel++;
          ...
          // the BEGIN command
            ...
            if (tok.nextToken() == NexusTokenizer.TT_WORD) {
              ...
              // you know. to be anal, and all that.
                ...
                inStrangeBlock = true;
                // warn the user!
                // delay.addWarning("Block '" + beginWhat + "' cannot be read by TaxonDNA yet! This
                // block will NOT be imported, and therefore cannot be re-exported.");
              }
            ...
            if (!inStrangeBlock) throw formatException(tok, "Unknown command '" + str + "' found.");
          }
        } else {
   ...
   *
   * <p>Things to do: 1. Change the definition of ' such that: 'x y z' == x_y_z 'Test''s testing' ==
   * Test''s_testing == [Test's testing] 2.
   */
  public void blockData(
    ...
    while (true) {
      ...
      if (type == NexusTokenizer.TT_EOF) {
        ...
        throw formatException(
            tok,
            "I've reached the end of the file, and the DATA/CHARACTERS block has *still* not been closed! Please make sure the block is closed.");
      }

      if (type == ';') newCommand = true;
      ...
      if (inIgnoredCommand) {
        ...
        if (type == NexusTokenizer.TT_WORD) {
          ...
          if (str.equalsIgnoreCase("INTERLEAVE")) {
            ...
            if (str == null) {
              ...
              if (str.equalsIgnoreCase("YES")) isDatasetInterleaved = true;
              else isDatasetInterleaved = false;
            }
            ...
            if (str == null)
              throw formatException(
                  tok,
                  "'DATATYPE' is misformed (or there's a comment in there somewhere. I can't abide comments in FORMAT).");
            ...
            if (str.equalsIgnoreCase("CONTINUOUS"))
              throw formatException(
                  tok,
                  "I can't understand DATATYPE '" + str + "'. I can only understand DATATYPE=DNA.");
          } else if (str.equalsIgnoreCase("GAP")) {
            str = getValueOfKey(tok);
            if (str == null)
              throw formatException(
                  tok,
                  "'GAP' is misformed (or there's a comment in there somewhere. I can't abide comments in FORMAT).");

            if (str.length() > 1)
              throw formatException(
                  tok,
                  "I can't use more than one character as the GAP character. The file specifies: '"
                      + str
                      + "'");
            ...
            if (str == null)
              throw formatException(
                  tok,
                  "'MISSING' is misformed (or there's a comment in there somewhere. I can't abide comments in FORMAT).");

            if (str.length() > 1)
              throw formatException(
                  tok,
                  "I can't use more than one character as the MISSING character. The file specifies: '"
                      + str
                      + "'");
            ...
            // delay.addWarning("FORMAT SYMBOLS=... in the DATA/CHARACTERS block cannot be read by
            // TaxonDNA yet. There is no guarantee that illegal symbols are not being used in your
            // sequences.");
          } else {
            throw formatException(
                tok,
                "I found '"
                    + str
                    + "' in the FORMAT line of the DATA (or CHARACTERS) block. I don't understand "
                    + str
                    + " at the moment - I can only comprehend 'MISSING=x', 'GAP=x', 'DATATYPE=DNA' and 'INTERLEAVE'. Sorry!");
          }
        } else {
        ...
        if (type == ';') {
          if (name != null) {
            ...
            throw formatException(
                tok, "The sequence named '" + name + "' has no DNA sequence associated with it.");
          }
          inMatrix = false;
        ...
        if (type == NexusTokenizer.TT_WORD) {
          ...
          if (str.equalsIgnoreCase("MATRIX")) {
            if (gapChar == missingChar) {
              ...
              delay.addWarning(
                  "This Nexus file defines BOTH the gap character and the missing character as '"
                      + gapChar
                      + "'. I will use this as the gap character only; no missing data will be recognized for this dataset.");
            }
          ...
          // Commands we ignore
          if (str.equalsIgnoreCase("DIMENSIONS")
            ...
            // delay.addWarning("Ignoring command '" + str + "' in the CHARACTERS/DATA block;
            // TaxonDNA cannot interpret this at present.");
          }
          ...
          if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
            ...
            break;
            //						} else {
            //							throw formatException(tok, "I found something strange after the END! I can't
          ...
          if (inIgnoredCommand) continue; // who *KNOWS* what ignored commands do these days?!
          ...
          throw formatException(
              ...
              "I found '"
                  + (char) type
                  + "' rather unexpectedly in the DATA/CHARACTERS block! Are you sure it's supposed to be here?");
        }
      }
  ...
  public void blockSets(
    ...
    while (true) {
      ...
      if (type == NexusTokenizer.TT_EOF) {
        ...
        throw formatException(
            tok,
            "I've reached the end of the file, and the SETS block has *still* not been closed! Please make sure the block is closed.");
      }
      ...
      if (inIgnoredCommand) {
        ...
        if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
          ...
          break;
          //					} else {
          //						throw formatException(tok, "I found something strange after the END! I can't just
          // ignore it. I'm sorry.");
          //					}
          ...
          if ((type = tok.nextToken()) != NexusTokenizer.TT_WORD)
            throw formatException(
                ...
                "Unexpected symbol '"
                    + (char) type
                    + "' found after 'CHARSET'. This doesn't look like the name of a CHARSET to me!");
          ...
          if ((char) type != '=') {
            ...
            if (type != NexusTokenizer.TT_WORD)
              throw formatException(
                  tok, "Unexpected '" + (char) type + "' after the '=' in CHARSET " + name + ".");
            ...
            if (str.equalsIgnoreCase("STANDARD")) {
              ...
              throw formatException(
                  tok,
                  "Sorry, we don't support VECTOR CHARSETs at the moment! ("
                      + name
                      + " is defined as being a vector charset)");
            } else {
              throw formatException(
                  tok, "Unexpected '" + str + "' after the '=' in CHARSET " + name + ".");
            }
          }
          ...
          while (true) {
            ...
            if ((char) type == ';') {
              ...
              try {
                ...
                throw formatException(
                    tok,
                    "I found a non-number in CHARSET "
                        + name
                        + " (the non-number is '"
                        + str
                        + "'). I can't currently understand 'ALL' or 'REMAINDER' commands. Can you please remove them from the source file if that's at all possible?");
              }
              ...
              if (expectingATo) {
                ...
                if (from != -1) fireEvent(evt.makeCharacterSetFoundEvent(name, from, from));
              ...
              throw formatException(
                  tok, "Unexpected '" + (char) type + "' in CHARSET " + name + ".");
            }
          }
          // commands we ignore in SETS
        } else if (str.equalsIgnoreCase("STATESET")
          ...
          inIgnoredCommand = true;
          // delay.addWarning("Command '" + str + "' in the SETS block cannot be read by TaxonDNA,
          // and will be ignored.");
        } else {
        ...
        throw formatException(
            tok, "Unexpected symbol '" + (char) type + "' found in the SETS block.");
      }
  ...
  /**
   * Extracts a single CodonPosSet range for a particular codon position. I suppose we'll eventually
   * be generating events for this, but for now we need to consume all that information.
   *
   * <p>Note that the first argument is 0 for 'N', and 1, 2, 3 for the three positions respectively.
   */
  private void addCodonPosSet(
    ...
    // System.err.println("CodonPosSet detected at " + pos + (actuallyAdd ? " and will be added." :
    // " which will not be added."));
    ...
    if (token != ':')
      throw formatException(
          ...
          "Position "
              + position
              + " not followed by ':' as mandated, but '"
              + (char) token
              + "/"
              + tok.sval
              + "' instead");
    ...
    while (true) {
      ...
      try {
        ...
        if (token != '-') {
          if (actuallyAdd) fireEvent(evt.makeCharacterSetFoundEvent(":" + pos, from, from));
          tok.pushBack();
          continue;
        }
        ...
        if (tok.nextToken() != '\\') {
          if (actuallyAdd) fireEvent(evt.makeCharacterSetFoundEvent(":" + pos, from, to));
        ...
        if (tok.nextToken() != NexusTokenizer.TT_WORD || !tok.sval.equalsIgnoreCase("3")) {
          throw formatException(
              tok,
              "I'm sorry, I can deal with CodonPosSets ending with /"
                  + tok.sval
                  + " - I only support 3!");
        }
        ...
        throw formatException(
            tok,
            "One of the values in CodonPosSet position " + position + " wasn't a number: " + e);
      }
    }
  }
  ...
  public void blockCodons(
    ...
    while (true) {
      ...
      if (type == NexusTokenizer.TT_EOF) {
        ...
        throw formatException(
            tok,
            "I've reached the end of the file, and the CODONS block has *still* not been closed! Please make sure the block is closed.");
      }
      ...
      if (inIgnoredCommand) {
        ...
        if (str.equalsIgnoreCase("END") || str.equalsIgnoreCase("ENDBLOCK")) {
          ...
          // okay, the line looks like 'CODONPOSSET * \w+ = N: {desc}+, 1: {desc}+, 2: {desc}+, 3:
          // {desc}+;'. And hopefully that's all we'll have to process.
          // where:
          //      desc = ((\d+)-(\d+)(\\3))*
          ...
          if (tok.nextToken() != NexusTokenizer.TT_WORD) {
            throw formatException(
                tok, "Expecting the name of the CodonPosSet, but got " + tok + " instead");
          }
          ...
          if (codonposset_already_defined) {
            delay.addWarning(
                "More than one CODONPOSSET command was found in this file. Only the first one will be added.");
          }
          ...
          for (int x = 0; x < str_positions.length; x++) {
            if (token != NexusTokenizer.TT_WORD
                || !tok.sval.equalsIgnoreCase(
                    str_positions[
                        x])) // throw formatException(tok, "I expect '" + str_positions[x] + "' to
            // be in the correct order in CODONPOSSET. The standard says so!");
          ...
          if (token != ';') {
            throw formatException(
                tok,
                "Wait, the CODONPOSSET didn't end with ';' (it ended with "
                    + (char) token
                    + "/"
                    + tok.sval
        ...
        } else if (str.equalsIgnoreCase("GENETICCODE") || str.equalsIgnoreCase("CODESET")) {
          inIgnoredCommand = true;
          // delay.addWarning("Command '" + str + "' in the CODONS block cannot be read by TaxonDNA,
          // and will be ignored.");
        } else {
          throw formatException(
              tok, "Unknown word/command '" + str + "' found in the CODONS block.");
        }
      } else {
        throw formatException(
            tok, "Unexpected symbol '" + (char) type + "' found in the CODONS block.");
      }
  ...
  /**
   * Gets the String 'value' of the key=value pair from the NexusTokenizer. I assume you've already
   * got the 'key', so I'll get the '=' token, followed by a TT_WORD, and return the
   * Integer.parseInt of that word.
   *
   * <p>I'll return null if something went wrong.
   *
  ...
  private String getValueOfKey(NexusTokenizer tok) throws FormatException, IOException {
    if (tok.nextToken() == '=') {
      ...
      if (type == NexusTokenizer.TT_WORD) {
        // yay! we got a word
        return tok.sval;
      } else if (!Character.isWhitespace((char) type) && !Character.isISOControl((char) type)) {
        char data[] = {(char) type};
        ...
        throw formatException(
            ...
            "Unexpected value '"
                + (char) type
                + "' found when looking for the value of a KEY=VALUE pair.");
      }
    } else {
  ...
  /**
   * A species NexusFile-only method to have a bit more control over how the Nexus file gets
   * written.
   *
   * @param interleaveAt Specifies where you want to interleave. Note that NexusFile.INTERLEAVE_AT
  ...
  public void writeNexusFile(
    ...
    if (interleaveAt > 0) interleaved = true;
    ...
    Hashtable names =
        new Hashtable(); // Hashtable(Strings) -> Sequence	of all the names currently in use
    Vector vec_names = new Vector(); // Vector(String)
    Iterator i = set.iterator();
    int count_names = 0;
    while (i.hasNext()) {
      if (delay != null) delay.delay(count_names, set.count());
      count_names++;
        ...
        if (no == 10000) {
          ...
          throw new IOException(
              "There are 9999 sequences named '"
                  + seq.getFullName(MAX_TAXON_LENGTH)
          ...
          try {
            ...
            throw new IOException(
                "Could not get subsequence ("
                    ...
                    + ") from sequence "
                    + seq
                    + ". This is most likely a programming error. The reason given was: "
                    + e.getMessage());
          }
    ...
    for (int x = 1; x <= 3; x++) {
      ...
      if (v != null) {
        ...
        while (i_v.hasNext()) {
          ...
          if (x == 0) str_end = " ";
          else if (x == 1 || x == 2 || x == 3) str_end = "\\3 ";

          if (ftp.from == ftp.to) {
            array_strbuff_positions[x].append((ftp.from) + " ");
          } else {
            array_strbuff_positions[x].append((ftp.from) + "-" + (ftp.to) + str_end);
          }
        }
      }
    }

    StringBuffer buff_nexus_positions =
        new StringBuffer("BEGIN CODONS;\n\tCODONPOSSET * CodonPositions = \n");
    ...
    for (int x = 0; x <= 3; x++) {
      ...
      if (array_strbuff_positions[x] != null) {
        buff_nexus_positions.append(
            "\t\t" + position_names[x] + ": " + array_strbuff_positions[x] + str_end + "\n");
    ...
    if (flag_display_nexus_positions) writer.println(buff_nexus_positions);
  ...
  /**
   * Export an entire SequenceGrid in one shot. We can do this, because we are the coolest. One
   * problem: how do we let user decide what constants to use?
   */
  public void writeFile(File f, SequenceGrid grid, DelayCallback delay)
      throws IOException, DelayAbortedException {
   ...
   * SequenceMatrix.
   *
   * <p>HUH? Does anybody use this code any more?
   */
  public void writeNexusFile(
      File f, SequenceGrid grid, int exportAs, int interleaveAt, DelayCallback delay)
      throws IOException, DelayAbortedException {
    int countThisLoop = 0;
    ...
    */
    ...
    while (i.hasNext()) {
      countThisLoop++;
      if (delay != null) delay.delay(countThisLoop, grid.getColumns().size());
        ...
        for (int x = 1; x <= 3; x++) {
          ...
          if (v != null) {
            ...
            while (i_v.hasNext()) {
              ...
              buff_nexus_positions.append(
                  (horzOffset + ftp.from) + "-" + (horzOffset + ftp.to) + " ");
            }
    ...
    if (how == EXPORT_AS_BLOCKS || how == EXPORT_AS_SINGLE_LINE) {
      ...
      writer.println(
          "\tDIMENSIONS NTAX="
              + grid.getSequencesCount()
              + " NCHAR="
              + grid.getCompleteSequenceLength()
              + ";");

      writer.print("\tFORMAT DATATYPE=DNA GAP=- MISSING=? ");
      if (how == EXPORT_AS_BLOCKS) writer.print("INTERLEAVE");
      writer.println(";");
    ...
    if (how == EXPORT_AS_BLOCKS) {
      ...
      while (i_cols.hasNext()) {
        if (delay != null) delay.delay(countThisLoop, total_count);
        countThisLoop++;
        ...
        while (i_seqs.hasNext()) {
          ...
          if (seq == null) seq = Sequence.makeEmptySequence(seqName, colLength);

          writer.println(
              getNexusName(seqName, MAX_TAXON_LENGTH)
                  + " "
                  + seq.getSequence()
                  + " ["
                  + colLength
      ...
      while (i_rows.hasNext()) {
        if (delay != null) delay.delay(countThisLoop, grid.getSequences().size());
        countThisLoop++;
        ...
        else if (how == EXPORT_AS_INTERLEAVED) seq_interleaved = new Sequence();
        ...
        while (i_cols.hasNext()) {
          ...
          if (how == EXPORT_AS_SINGLE_LINE) writer.print(seq.getSequence());
          else if (how == EXPORT_AS_INTERLEAVED)
            seq_interleaved = seq_interleaved.concatSequence(seq);
          else
            throw new RuntimeException(
                "'how' makes no sense in NexusFile.exportAsNexus()! [how = " + how + "]");
        }

        if (how == EXPORT_AS_INTERLEAVED)
          seq_interleaved.changeName(getNexusName(seqName, MAX_TAXON_LENGTH));

        if (how == EXPORT_AS_SINGLE_LINE) writer.println(" [" + length + " bp]");
        else if (how == EXPORT_AS_INTERLEAVED) list.add(seq_interleaved);
      }
    }
