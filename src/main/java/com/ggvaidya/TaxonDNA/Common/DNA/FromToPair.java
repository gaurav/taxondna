...
/**
 * A special datatype storing coordinates within a sequence. At the moment, this is a pretty simple
 * thing; you'll need outside information to figure out what it means or where it's actually point
 * to.
 *
 * <p>Most important note: FromToPairs must ALWAYS be one-based. Zero-based FTPs are pure evil. This
...
public class FromToPair implements Comparable {
    ...
    public FromToPair(int from, int to) {
        if (from <= 0 || to <= 0)
            throw new RuntimeException(
                    "FromTo of " + from + " to " + to + " invalid: less than or equal to zero!");

        if (to < from)
            throw new RuntimeException(
                    "FromTo of " + from + " to " + to + " invalid: incorrect order!");
    ...
    public String toString() {
        if (from == to) return from + ""; // String convert on the fly.
    ...
    /**
     * * Checks for overlap, but assumes both FTPs move in threes. (i.e. they were marked with \3s).
     * Note that there is at present no way for this code to confirm that both FTPs move in threes:
     * please make sure they do before calling this method.
     *
     * @param ftp The FTP to compare against.
    ...
    public boolean overlapsMovesInThrees(FromToPair ftp) {
        ...
        // So:
        if (!this.overlaps(ftp)) return false;
        ...
        if ((this.from % 3) != (ftp.from % 3)) {
            System.err.println(
                    "No comparison necessary between /3s loci "
                            + this.from
                            + " and "
                            + ftp.from
                            + ": separate tracks.");
        ...
        if (rounded_us.to % 3 != track)
            throw new RuntimeException(
                    "Algorithm fails: our 'to' rounded from "
                            + this.to
                            + " to "
                            + rounded_us.to
                            + " is not on track "
        ...
        if (rounded_ftp.to % 3 != track)
            throw new RuntimeException(
                    "Algorithm fails: their 'to' rounded from "
                            + ftp.to
                            + " to "
                            + rounded_ftp.to
                            + " is not on track "
