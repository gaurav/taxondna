...
public class SequenceList implements List, Testable {
   ...
   *
   * @throws SequenceListException if any exception was thrown creating this SequenceList. It wraps
   *     the exception and provides a useful message at the same time.
  ...
  /**
   * Creates a SequenceList from a Collection of Sequences. Any collection will do (we just feed it
   * into the addAll anyway). Note that this will leave the file as 'null', so you'll have to use
   * changeFile(), etc. to make this SequenceList feel like it *belongs* somewhere.
   */
   ...
   * constants of this class. There is no way to determine how this class is sorted, because I can't
   * think of why anyone would want to do this. You just call 'resort' to whatever you want; we're
   * smart enough not to resort if we're already sorted that way. Of course, this means that: 1.
   * Once sorted, we must maintain the sort in all additions/deletions. (oh dear, what a pain! - but
   * what rewards! Eh. No time. Really.) 2. Once UNsorted, we need to make sure that sortMethod HAS
   * been reset.
   *
  ...
  //
  /**
   * Loads up a SequenceList from the specified file using the specified formatHandler and providing
   * feedback via the specified DelayCallback.
   */
  ...
  /**
   * Loads up a SequenceList from the specified file. We will "intelligently" (ha!) try to determine
   * the type of file, by running through the various FormatHandler.mightBe() functions. There are
   * many holes to this, but on the whole it works, and even if it doesn't, we'll have prominent
   * Export/Import systems.
   *
   * <p>Oh, and it makes a nice list of the format handlers it *does* support, so you have a better
   * idea about what's going on, and so on.
   */
  ...
  public static SequenceList readFile(File file, DelayCallback delay, FormatListener listener)
    ...
    throw new SequenceListException(
        "I could not understand the input file '"
            ...
            + " file, which are the only types of file I can read. Please check the format of the file, or convert it into one of the formats I can read.");
  }
  ...
  //
  /**
   * Has this SequenceList been modified? TODO: This mechanism needs writing up, if it's any use, or
   * to be got rid of, if it isn't.
   */
   ...
   * fail-fast iterator. I don't think it's guaranteed to be in order, but I think it is guaranteed
   * not to repeat elements. I think. Don't look at me, I'm just using Vector.iterator() for now.
   */
   ...
   *
   * @return An iterator to all species named 'speciesName' in this list, or 'null' if there are no
   *     such species.
   */
   ...
   * Note that this Sequence must actually be IN this sequence list. To search by species name, use
   * the slighly slower conspecificIterator(String speciesName).
   *
   ...
   * lots of angst. public Iterator conspecificIterator(Sequence seq) { lock(); resort(SORT_BYNAME);
   * // so we're going by name
   *
  ...
  /**
   * Somebody modified either this dataset itself (add, remove, etc.), or one of the sequences which
   * comprise us.
   */
  ...
  /**
   * Adds a Sequence to this SequenceSet. This is only a stub to handle the conversion: we really do
   * prefer add(Sequence).
   */
  ...
  /**
   * Retains only the elements in this collection; i.e. intersection of this and Collection c. Right
   * now, this is unimplemented, until someone can figure out what it's good for.
   */
  ...
  public void test(TestController testMaster, DelayCallback delay) throws DelayAbortedException {
    ...
    if (sl.count() == 4) testMaster.succeeded();
      ...
      testMaster.failed(
          "After removing, there should be 4 sequences; instead, there are " + sl.count() + "!");
...
class ConspecificIterator implements Iterator {
  ...
  public void remove() {
    ...
    // the trick is: we decrement 'x' (to move 'back' one) and remove whatever is at 'x'. x is now
