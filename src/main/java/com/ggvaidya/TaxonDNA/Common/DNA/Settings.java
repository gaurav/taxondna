/**
 * DNA.Settings controls settings for all DNA classes. They check back here for constants, controls,
 * and optimizations.
 *
 * <p>Just to clarify: Sequence has a certain, fixed format for dealing with Sequences (using '-'
 * for gaps, for instance). That is how it works. Deal with it.
 *
 * <p>Settings controls the (very little) stuff in DNA which can be played around with. It should be
 * noted that values are *read from* here, so all values which might be affected ought to be
 * recalculated in between changing settings.
 *
 * <p>Somewhere, somehow, people who actually know Java are laughing at me ...
 */
...
public class Settings {
  /**
   * What proportion of the total memory the pairwise distance cache is allowed to use until. (Ha!
   * Okay, seriously: the fraction of used memory to total memory at which the cache flushes itself)
   */
  ...
  /** Sets the value we should be accurate to. e.g. 0.0001 */
  public static void setAccurateTo(double d) {
    accurateTo = (int) (1 / d);
  }

  /** Sets the number of digits we should be accurate to. * e.g. 4 for 0.0001 */
  public static void setAccurateToDigits(int digits) {
    accurateTo = (int) Math.pow(10, digits);
  }

  /**
   * Converts the specified double to an int at the necessary level of accuracy. Think of it as a
   * log-transform. You can use makeDoubleFromLong() to convert it back.
   */
  ...
  /**
   * Converts the specified int (from makeLongFromDouble) to an int at the necessary level of
   * accuracy.
   *
  ...
  public static double percentage(double x, double y) {
    if (y == 0) return 0; // x% of nothing is zero percent
  ...
  /**
   * Returns true if ambiguous bases are allowed. If not, we'll convert ambiguous bases to 'N'
   * during comparisons.
   */
  ...
  /**
   * Set whether ambiguous bases are allowed. If not, we'll convert ambiguous bases to 'N' during
   * comparisons.
   */
