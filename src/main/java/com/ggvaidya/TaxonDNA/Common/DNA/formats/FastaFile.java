...
public class FastaFile extends BaseFormatHandler implements Testable {
  ...
  /**
   * Returns true if it seems likely (heck, possible) the file is a FASTA file. To be strict, we'll
   * parse the entire file, counting as we go. This also saves us a count later on, and it seems
   * unlikely that we'll ever run readFile() without calling this first.
   *
  ...
  private Sequence makeSequence(DelayCallback delay, String name, String seq)
    ...
    if (seq.indexOf('U') != -1 || seq.indexOf('u') != -1) {
      ...
      if (delay != null)
        delay.addWarning(
            "Sequence '"
                + name
                + "' contains one or more uracil bases. This program can only handle DNA sequences at the moment. I'm converting all uracil (U) bases into thymine (T) bases.");
    ...
    return BaseSequence.createSequence(
        name,
        seq.replaceAll(
            "\\s+", "")); // delete whitespace characters from the sequence before adding.
  }
