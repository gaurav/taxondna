/**
 * NexusTokenizer is like StreamTokenizer, except that it is specifically keyed in to Nexus's
 * oddball attitudes. The idea is, you should be able to (mostly) use this as a normal
 * StreamTokenizer, except that: - 's can be used to demarcate 'words'. - _s are automatically
 * converted into spaces. - [s and ]s can be used to open and close comments.
 *
 * <p>We still need to come up with a smart solution for Nexus sequence blocks with wholes in them,
 * but whatever.
 *
...
public class NexusTokenizer {
  ...
  private class Results {
    ...
    public boolean canDelete() {
      return ((status == TT_NULL) || (status == TT_WORD && sval == null));
    }
    ...
    public String toString() {
      ...
      if (sval != null) {
        ...
        else str.append("status = TT_WORD, word = '" + sval + "'");
        hasString = true;
      }
      ...
      if (status == TT_WORD) {
        ...
        if (hasString) str.append(" | also hiding: ");

        if (status > 0) str.append("status = '" + (char) status + "'");
        else str.append("status = " + status);
      }
  ...
  public boolean isValidCharacter(char ch) {
    ...
    if (ch
        == '.') // a LOT of Nexus files seem to think '.' is alphanumeric. What to do. Go with flow,
      // etc.
      return true;
    ...
    if (ch == gapChar) return true;

    if (ch == missingChar) return true;
   ...
   * @throws IOException if something went wrong while reading the file (note that EOFException will
   *     NEVER be thrown)
   */
  public Results eatNextToken() throws IOException, FormatException {
    ...
    while (true) {
      ...
      // System.err.println("character = '" + ch + "', isWhitespace: " + Character.isWhitespace(ch)
      // + ", lastChar.isWhitespace: " + Character.isWhitespace(lastChar) + ", isLetterOrDigit: " +
      // Character.isLetterOrDigit(ch) + ", heck; token = '" + token + "'");
      ...
      if (Character.isWhitespace(ch)) {
        if (Character.isWhitespace(lastChar)) {
          if (inSingleQuotes) token.append(ch);
          // skip it!
          continue;
        } else {
          if (inSingleQuotes) {
      ...
      if (!inSingleQuotes
          ...
          && !isValidCharacter(ch)) { // TODO are hyphens always okay?
  ...
  public int nextToken() throws IOException, FormatException {
    ...
    if (retVal == TT_WORD) {
      ...
      } else previousResults.push(new Results(this, sval, retVal));
    } else previousResults.push(new Results(this, null, retVal));
  ...
  */
  ...
  /**
   * Returns the nextToken() (with a little preprocessing from your friends) public int nextToken()
   * throws IOException { int tok = super.nextToken();
   *
   * <p>return tok; }
   */
}
