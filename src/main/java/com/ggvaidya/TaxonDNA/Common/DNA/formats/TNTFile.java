...
public class TNTFile extends BaseFormatHandler {
  ...
  public void appendFromFile(SequenceList appendTo, File fileFrom, DelayCallback delay)
    ...
    try {
      ...
      tok.ordinaryChar(
          '/'); // this is the default StringTokenizer comment char, so we need to set it back to
      // nothingness
      ...
      tok.ordinaryChar(
          '\''); // We need this to be an 'ordinary' so that we can use it to discriminate comments
      tok.ordinaryChar(
          ';'); // We need this to be an 'ordinary' so that we can use it to distinguish commands
            ...
            if ((token == StreamTokenizer.TT_WORD) && (tok.sval.equalsIgnoreCase("dna"))) {
              ...
              if (tok.sval.equalsIgnoreCase("cont"))
                throw formatException(
                    tok,
                    "This program can currently only load files which contain discrete sequences. This file does not (it contains continuous data, as indicated by 'nstates "
                        + tok.sval
                        + "').");
            }
          }
   ...
   * considered: we'll ignore amperstands, and we'll barf quickly and early if we think we're going
   * in over our head. Pretty much, we are just targeting trying to be able to open files we spit
   * out. Can't be _that_ hard, can it?
   *
  ...
  public void xreadBlock(
    ...
    if (tok.ttype == '\'') {
      ...
      while (true) {
        ...
        if (type == StreamTokenizer.TT_EOF)
          throw formatException(
              tok,
              "The title doesn't seem to have been closed properly. Are you sure the final quote character is present?");
      }
    } else {
      // err, no title?
      tok.pushBack();
    ...
    if (tok.ttype != StreamTokenizer.TT_WORD)
      throw formatException(
          tok,
          "Couldn't find the number of characters. I found '" + (char) tok.ttype + "' instead!");
    try {
    ...
    if (tok.ttype != StreamTokenizer.TT_WORD)
      throw formatException(
          tok, "Couldn't find the number of taxa. I found '" + (char) tok.ttype + "' instead!");
    try {
      ...
      */
      if (type == StreamTokenizer.TT_WORD) {
        ...
        if (word.equalsIgnoreCase("[cont]")) {
          throw formatException(
              tok,
              "This program can currently only load files which contain discrete sequences. This file does not (it contains continuous data, as indicated by '[cont]').");
        }
        ...
        try {
          ...
          throw formatException(
              ...
              "Sequence '"
                  + name
                  + "' contains invalid characters. The exact error encountered was: "
                  + e);
        }
        ...
        throw formatException(
            ...
            "I found '"
                + (char) type
                + "' rather unexpectedly in the xread block! Are you sure it's supposed to be here?");
      }
    }
   ...
   * \------------------- the name of this group (if any) \-------------------------- the number of
   * this group (not important to us, except that /=\d+/ indicates a new taxongroup starting In this
   * function, I'll use ?group or _group to indicate, err, well, /[ax]group/.
   */
      ...
      if (type == '=') {
        ...
        if (sequence_begin == -1 || sequence_end == -1) {
          ...
          if (which_group == GROUP_CHARSET) {
            fireEvent(
                evt.makeCharacterSetFoundEvent(currentName, sequence_begin + 1, sequence_end + 1));
            // System.err.println("New multi-character sequence [2]: " + currentName + " from " +
        ...
        if (tok.nextToken() != StreamTokenizer.TT_WORD) {
          ...
          // throw formatException(tok, "Expecting the group id, but found '" + (char)tok.ttype + "'
          // instead!");
          ...
          if (tok.ttype == StreamTokenizer.TT_EOF)
            throw formatException(
                ...
                "The title which began in "
                    ...
                    + " on line "
                    + title_began
                    + " is not terminated! (I can't find the ')' which would end it).");
        ...
        if (word.indexOf('.') == -1) {
          ...
          if (sequence_end == -1
              || sequence_end == locus - 1) { // if sequence_end is pointing at the LAST locus,
            ...
            if (which_group == GROUP_CHARSET) {
              if (currentName.charAt(0) == ':') {
                for (int d = sequence_begin + 1; d <= (sequence_end + 1); d++) {
                  ...
                  // System.err.println("New multicharacter sequence [6]: " + currentName + " from "
                  // + d + " to " + d);
                }
              } else {
                fireEvent(
                ...
                // System.err.println("New multicharacter sequence [3]: " + currentName + " from " +
                // sequence_begin + " to " + sequence_end);
          ...
          if (which_group == GROUP_CHARSET) {
            fireEvent(
                evt.makeCharacterSetFoundEvent(currentName, sequence_begin + 1, sequence_end + 1));
            // System.err.println("New multicharacter sequence [1]: " + currentName + " from " +
            // sequence_begin + " to " + sequence_end);
          }
          ...
          if (which_group == GROUP_CHARSET) {
            ...
            // System.err.println("New multi-character block [4]: " + currentName + " from " + from
            // + " to " + to);
          }
    ...
    if (sequence_begin != -1 && sequence_end != -1) {
      ...
      if (which_group == GROUP_CHARSET) {
        if (currentName.charAt(0) == ':') {
          for (int d = sequence_begin + 1; d <= sequence_end + 1; d++) {
            fireEvent(evt.makeCharacterSetFoundEvent(currentName, d, d));
            // System.err.println("New multicharacter sequence [7]: " + currentName + " from " +
            // sequence_begin + " to " + sequence_end);
          }
        } else {
          fireEvent(
    ...
    for (int x = 1; x <= 3; x++) {
      if (array_strbuff_positions[x] != null) {
        buff_tnt_positions.append(
            "=" + colid + " (pos" + position_names[x] + ") " + array_strbuff_positions[x] + "\n");
        colid++;
      }
    }
