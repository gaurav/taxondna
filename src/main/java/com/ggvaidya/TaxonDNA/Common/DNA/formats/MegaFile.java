...
public class MegaFile extends BaseFormatHandler implements Testable {
    ...
    public String getMegaOTU(String name, int len) {
        ...
        // Rule #3: spaces we'll turn into '_' (although really they ought to have been fixed in
        // rule
        // #2)
        name = name.replace(' ', '_');
    ...
    public void writeFile(File file, SequenceList list, DelayCallback delay)
        ...
        while (i.hasNext()) {
            ...
            while (names.contains(name)) {
                ...
                name =
                        getMegaOTU(seq.getFullName(MAX_NAME_SIZE), MAX_NAME_SIZE - 1 - 4)
                                + "_"
                                + next_index;
                next_index++;
                ...
                if (next_index > 9999)
                    throw new IOException(
                            "Sorry, I can only handle upto 9,999 duplicate names while exporting a"
                                + " Mega file! Please let us (the developers) know if this limit is"
        ...
        try {
            ...
            while ((line = read.readLine()) != null) {
                ...
                if (waitingForSig) {
                    ...
                    if (line != "") {
                        if (line.toLowerCase().equals("#mega")) waitingForSig = false;
                            ...
                            throw new FormatException(
                                    "This file does not contain a valid MEGA signature");
                        }
                    }
                    ...
                    line =
                            line.substring(1)
                                    .trim(); // so we don't have to worry abt spaces after the '#'
                    ...
                    if (seq_name.toString().trim() != "" && seq_seq.toString().trim() != "") {
                        ...
                        throw new FormatException(
                                "Error in file on line "
                                        + lineno
                                        + ": something wrong with this line! Is there a illegible"
            ...
            if (waitingForTitle) {
                throw new FormatException(
                        "This file does not contain a 'TITLE', a requirement in all valid MEGA"
                                + " files");
            }
            ...
            throw new FormatException(
                    "There was an error reading the Mega file '" + file + "': " + e, e);
        } finally {
            if (delay != null) delay.end();
