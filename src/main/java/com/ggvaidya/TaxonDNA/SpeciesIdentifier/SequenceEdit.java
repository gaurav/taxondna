/**
 * A UIExtension which allows for sequence viewing and editing. This is tricky, because this will
 * actually need to talk to SpeciesIdentifier to figure out a lot of the stuff (since
 * SpeciesIdentifier controls the All Sequences list)
 */
/*
    TaxonDNA
...
public class SequenceEdit extends Panel
        implements UIExtension, ActionListener, ItemListener, FocusListener {
    ...
    public SequenceEdit(SpeciesIdentifier view) {
        ...
        c.gridwidth =
                2; // all of these spread for '2' units, so that we can squeeze in extra buttons
    ...
    /**
     * Returns the Sequence() indicated. Note that you are entirely responsible for ensuring that
     * the seqId list stays in sync. Maybe a method in seqId?
     */
    ...
    /**
     * Updates name related information. (i.e. based on the full name, it sets family, species, gi
     * and the like.
     */
    private void updateInfo(Sequence seq) {
        if (seq != null) {
            ...
            if (seq.getGI() != null) text_gi.setText(seq.getGI().trim());
            else text_gi.setText("");
            text_len.setText(
                    seq.getActualLength()
                            + " bp, entire sequence (including gaps): "
                            + seq.getLength()
                            + " bp");
        } else {
            text_family.setText("");
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (cmd.equals("Copy to Clipboard") || cmd.equals("Oops, try again?")) {
            ...
            if (gi_number != null
                    && gi_number.length() > 0
                    && ((gi = Integer.parseInt(gi_number)) != 0)) {
                String url =
                        new String(
                ...
                try {
                    ...
                    MessageBox mb =
                            new MessageBox(
                                    seqId.getFrame(),
                                    "Could not open NCBI website!",
                                    "This program couldn't open your web browser to access NCBI."
                                            + " You can access this entry at "
                                            + url);
                    mb.go();
                }
            }
    ...
    // Key listener
    /**
     * We save the text in the 'name' and 'sequence' field when we gain focus. Then, when we loose
     * it, we can check to see if anything changed.
     */
    ...
    public void focusGained(FocusEvent e) {
        if (e.getSource().equals(text_name)) focusGained_name = text_name.getText().trim();
    ...
    public void focusLost(FocusEvent e) {
        ...
        if (e.getSource().equals(text_name)) {
            ...
            // sequence changed
            String newSequence =
                    text_sequence.getText().replaceAll("\\d", "").replaceAll("\\s", "");
            // if you're wondering '\d' is being eliminated so that you can copy straight out of
            // NCBI
            // 1	ACTG
            // 5	ATTG
            // 9	ACCF ... format
            try {
                ...
                MessageBox mb =
                        new MessageBox(
                                seqId.getFrame(),
                                "Error in sequence!",
                                "There is an error in this sequence: " + ex);
                mb.go();
    ...
    public String getDescription() {
        return "Edit individual sequences";
    }
