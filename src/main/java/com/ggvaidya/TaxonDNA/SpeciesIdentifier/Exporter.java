 ...
 * Exporter.java
 *
 * <p>Allows you to export sequences from SpeciesIdentifier, with a set of options to export them to
...
public class Exporter extends Panel implements Runnable, UIExtension, ActionListener, ItemListener {
    ...
    private Checkbox check_spacesToUnderscore =
            new Checkbox("Change spaces to underscore in the taxon name");
    private Checkbox check_summaryOnTop =
            new Checkbox("Write a summary (number of sequences) at the top");
    ...
    private Checkbox check_onePerSpecies =
            new Checkbox("Export only one sequence for each sequence.");
    ...
    /**
     * Add commands to the SpeciesIdentifier menu. We only add one option: called rather oddly
     * "Custom export"
     */
    ...
    private String getHeader(int count_sequences, int max_length) {
        ...
        if (format.equals("MEGA"))
            return "#mega\nTITLE SpeciesIdentifier Exporter: "
                    + count_sequences
                    + " exported, of maximum length "
                    + max_length
                    + " bp\n";
        ...
        if (format.equals("TNT")) return "xread\n" + max_length + " " + count_sequences + "\n";
    ...
    private String getFooter() {
        ...
        if (format.equals("NEXUS")) return "\n;\nEND;";

        if (format.equals("TNT")) return "\n;\nccode -.;\nproc/;";
    ...
    public Exporter(SpeciesIdentifier view) {
        ...
        rl.add(
                new Label(
                        "Please select the options you would like to use, then press the 'Go!'"
        ...
        rl.add(
                choice_whatToOutput,
                RightLayout.BESIDE | RightLayout.STRETCH_X | RightLayout.FILL_2);

        rl.add(
                new Label("How would you like me to arrange names and sequences?"),
                RightLayout.NEXTLINE);
        choice_howToOutput.add(
                "Taxon name, followed by the sequence name on a new line (like Fasta)");
        choice_howToOutput.add(
                "Taxon name, followed by the sequence name on the same line (like Nexus)");
        ...
        rl.add(
                new Label("Which character should I use to indicate taxon names?"),
                RightLayout.NEXTLINE);
    ...
    public void run() {
            ...
            if (multipleMode != 0) {
                MessageBox mb =
                        new MessageBox(
                                seqId.getFrame(),
                                "Mea culpa!",
                                "Sorry, but at present you can't export into multiple files *and*"
                                    + " use the 'add species matrix' options. Please unselect one"
                                    + " of them.\n\n"
                                    + "This is entirely my fault: it's doable, but very"
                                    + " complicated, and I haven't gotten around to doing it yet."
                                    + " Sorry!");
                mb.go();
                return;
            }
        ...
        ProgressDialog pd =
                ProgressDialog.create(
                        ...
                        "Please wait, I am writing out the sequences as per your instructions. This"
                            + " shouldn't take too long, but what ARE reusable components for if"
                            + " you can't pop them at the user any time something might take too"
                            + " long to run?");
        ...
        if (check_onePerSpecies.getState()) i = createOnePerSpeciesIterator(sl);
        else i = sl.iterator();
        ...
        try {
                ...
                if (multipleMode == 1) {
                    if (!s.getFamilyName().equals(lastFamily)) {
                        ...
                        if (current_file.length() > 0) {
                            ...
                            if (check_insertHeaders.getState()) buffer.append(getFooter());
                        }
            ...
            if (multipleMode == 1) {
                ...
                if (check_insertHeaders.getState()) buffer.append(getHeader(sl.count(), maxLength));
    ...
    /**
     * This function processes the name of a sequence (based on the settings provided) and returns
     * it.
     */
    private String getSequenceName(Sequence seq) {
        ...
        if (check_spacesToUnderscore.getState()) name = name.replace(' ', '_');
    ...
    public void actionPerformed(ActionEvent evt) {
        if (evt.getActionCommand().equals("Custom export")) {
        ...
        } else if (evt.getSource().equals(btn_Go)) new Thread(this).start();
            ...
            if (check_exportIntoMultiple.getState()) {
                MessageBox mb =
                        new MessageBox(
                                ...
                                "You have selected multiple mode. Please select a single file in"
                                    + " which to save summary information. Other files will be"
                                    + " created in the same directory. ANY FILE WITH THE SAME NAME"
                                    + " IN THIS DIRECTORY WILL BE OVERWRITTEN.");
            ...
            else if (fd.getDirectory() == null) f = new File(fd.getFile());
            ...
            try {
                ...
                if (check_exportIntoMultiple.getState()) {
                    ...
                    Pattern pattern_new_file =
                            Pattern.compile("^\\[taxondna_begin_new_file:(.*)\\]$");
    ...
    public void itemStateChanged(ItemEvent evt) {
        if (evt.getSource().equals(choice_sizeRestrictions)) {
            ...
            if (index != -1) {
                ...
                tf_sizeRestriction.setText(
                        String.valueOf(Integer.parseInt(str.substring(index, index_until))));
            }
        }
    ...
    public String getDescription() {
        return "Allows you to export this dataset in a custom format";
    }
