...
public class Exporter extends Panel implements Runnable, UIExtension, ActionListener, ItemListener {
  ...
  /**
   * Add commands to the SpeciesIdentifier menu. We only add one option: called rather oddly "Custom
   * export"
   */
  ...
  public Exporter(SpeciesIdentifier view) {
    ...
    rl.add(
        new Label(
            "Please select the options you would like to use, then press the 'Go!' button to see the output."),
        RightLayout.FILL_3);
    ...
    rl.add(
        new Label("How would you like me to arrange names and sequences?"), RightLayout.NEXTLINE);
    ...
    rl.add(
        new Label("Which character should I use to indicate taxon names?"), RightLayout.NEXTLINE);
  ...
  /**
   * This function processes the name of a sequence (based on the settings provided) and returns it.
   */
  ...
  public void actionPerformed(ActionEvent evt) {
    if (evt.getActionCommand().equals("Custom export")) {
      ...
      FileDialog fd =
          new FileDialog(
              seqId.getFrame(), "Where would you like to export these sequences?", FileDialog.SAVE);
