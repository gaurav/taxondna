/**
 * A UIExtension which allows you to figure out the 'extreme' pairwise distances - the largest intra
 * and the smallest inter.
 */
/*
    TaxonDNA
...
public class ExtremePairwise extends Panel implements UIExtension, ActionListener, Runnable {
  ...
  public void run() {
    ...
    results.append(
        "Sequence name\tLargest conspecific match\tDistance\tOverlap\tClosest congeneric, interspecific match\tDistance\tOverlap\n");
    try {
      ProgressDialog pd =
      ...
      while (i.hasNext()) {
        ...
        if (seq_largestIntra == null) {
          ...
          results.append(
              seq_largestIntra.getDisplayName()
                  + "\t"
                  + percentage(seq_largestIntra.getPairwise(seq), 1)
                  + "\t"
                  + seq_largestIntra.getOverlap(seq)
                  + "\t");
          //					System.err.println(seq + "\tINTRA\t" + seq_largestIntra.getDisplayName() + "\t" +
          // percentage(seq_largestIntra.getPairwise(seq), 1) + "\t" +
          // seq_largestIntra.getOverlap(seq) + "\n");
        }
        ...
        if (seq_smallestInter == null) {
          ...
          results.append(
              seq_smallestInter.getDisplayName()
                  + "\t"
                  + percentage(seq_smallestInter.getPairwise(seq), 1)
                  + "\t"
                  + seq_smallestInter.getOverlap(seq)
                  + "\t");
          //					System.err.println(seq + "\tINTER\t" + seq_smallestInter.getDisplayName() + "\t" +
          // seq_smallestInter.getPairwise(seq) + "\t" + seq_smallestInter.getOverlap(seq) + "\n");
        }
  ...
  public String getShortName() {
    return "Extreme Pairwise";
  }

  public String getDescription() {
    return "Determines the extreme pairwise distance - the largest intraspecific and smallest congeneric, interspecific distance - for each sequence.";
  }
