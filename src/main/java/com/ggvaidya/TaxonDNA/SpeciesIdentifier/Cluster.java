...
public class Cluster extends Panel implements UIExtension, ActionListener, ItemListener, Runnable {
  ...
  public void writeupItemStrings(DelayCallback delay) throws DelayAbortedException {
    ...
    if (set != null) delaySteps += set.count() * 2; // try to get as accurate a guess as possible
    ...
    str.append(
        "Cluster\tNo of sequences\tNo of species\tMax pairwise distance\tPercentage of valid comparisons over "
            + max_pairwise
            + "\n");
    int delay_count = 0;
    for (int x = 0; x < clusters.size(); x++) {
      ...
      while (i1.hasNext()) {
        ...
        if (hash_species_counts_in_this_cluster.get(spName) == null) {
          ...
          if (hash_species.get(spName) == null) {
            ...
            hash_species.put(
                spName,
                new Integer(((Integer) hash_species.get(seq.getSpeciesName())).intValue() + 1));
          }
      ...
      String consensusName =
          cluster_type.replace(' ', '~') + " " + species_name + " (cluster #" + (x + 1) + ")";
      try {
        list_of_consensus.add(new Sequence(consensusName, consensus.getSequence()));
      } catch (SequenceException e) {
        item_strings[0] =
            "Clustering incomplete; please recluster: could not process split consensus sequence for "
                + consensusName;
        return;
      }
    ...
    while (enu.hasMoreElements()) {
      ...
      while (i.hasNext()) {
        ...
        if (seq.getSpeciesName() != null && seq.getSpeciesName().equals(name)) count_sequences++;
      }
    ...
    str_final.append(
        "Number of clusters with threshold violations:\t"
            ...
            + "%)\n");
    str_final.append(
        "Largest pairwise distance:\t" + percentage(largest_pairwise_distance_observed, 1) + "%\n");
    ...
    str_final.append(
        "Profiles corresponding to traditional taxonomy:\t"
            ...
            + "\n");
    str_final.append(
        "Largest number of species in a cluster:\t" + largest_no_of_species_in_a_cluster + "\n");
    ...
    if (set != null) {
      ...
      while (i.hasNext()) {
        ...
        while (i2.hasNext()) {
          ...
          if (seqName == null || seqName.length() == 0) seqName = "{No name}";

          countChars += seqName.length();

          if (countChars
              > CHAR_LIMIT_ON_CLUSTER_NAMES) { // if the count of the names is more than 40 chars,
