...
public class SpeciesSummary extends Panel implements UIExtension, Runnable, ActionListener, ItemListener {
	...
	public void run() {
		...
		str.append("Number of sequences with atleast one valid conspecific sequence: " + species.getSequencesWithValidConspecificsCount() + " (" + com.ggvaidya.TaxonDNA.Common.DNA.Settings.percentage(species.getSequencesWithValidConspecificsCount(), list.count()) + "%)\n");
		str.append("Number of species with valid conspecifics: " + species.getValidSpeciesCount() + " (" + com.ggvaidya.TaxonDNA.Common.DNA.Settings.percentage(species.getValidSpeciesCount(), species.count()) + "% of all species)\n");
		// set up list_species
		//
		list_species.removeAll();
