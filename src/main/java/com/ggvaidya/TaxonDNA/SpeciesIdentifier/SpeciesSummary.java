...
public class SpeciesSummary extends Panel
  ...
  public void actionPerformed(ActionEvent e) {
      ...
      try {
        ...
        if (sl_results.count() == 0) {
          new MessageBox(
                  seqId.getFrame(),
                  "No sequences found!",
                  "This dataset does not contain any sequences with valid conspecific matches!")
              .go();
          return;
        }
        ...
        if (fd.getFile() != null) {
          ...
          // assume that 'f' has already been tested for existance, and overwriting verified by
          // FileDialog.
          ...
          try {
            ...
            new MessageBox(
                    seqId.getFrame(),
                    "Error!",
                    "There was a problem while exporting species with multiple sequences. The technical description of the error is: "
                        + ex.getMessage()
          ...
          new MessageBox(
                  ...
                  "Success!",
                  sl_results.count()
                      + " sequences containing atleast one valid conspecific sequence exported to "
                      + f
                      + " in the FASTA format.")
              .go();
        }
    ...
    if (e.getSource().equals(btn_export_multiple)) {
      ...
      FileDialog fd =
          new FileDialog(seqId.getFrame(), "Export sequences to Fasta file ...", FileDialog.SAVE);
      fd.setVisible(true);
      ...
      species =
          list.getSpeciesDetails(
              ProgressDialog.create(
                  ...
                  "Species summary information is being calculated. Sorry for the wait.",
                  0));
  ...
  public void exportRandomly() {
    ...
    DirectoryInputPanel dinp =
        new DirectoryInputPanel(
            "Directory to export random subsets to:", DirectoryInputPanel.MODE_DIR_SELECT, w);
      ...
      try {
        ...
        if (ch_selected == ch_species) {
          ...
          new MessageBox(
                  seqId.getFrame(),
                  "All done!",
                  rands + " randomizations of " + species + " species each were exported to " + dir)
              .go();
