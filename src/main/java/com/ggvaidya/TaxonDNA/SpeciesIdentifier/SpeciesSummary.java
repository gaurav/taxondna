...
public class SpeciesSummary extends Panel
        implements UIExtension, Runnable, ActionListener, ItemListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource().equals(btn_export_with_cons)) {
            ...
            try {
                SpeciesDetails sd =
                        sl.getSpeciesDetails(
                                ProgressDialog.create(
                                        seqId.getFrame(),
                                        "Please wait, determining sequences with conspecifics ...",
                                        "Please wait, I'm looking through the sequences to"
                                            + " determine which ones have conspecific sequences."
                                            + " Give me a second!"));
                ...
                if (sl_results.count() == 0) {
                    new MessageBox(
                                    seqId.getFrame(),
                                    "No sequences found!",
                                    "This dataset does not contain any sequences with valid"
                                            + " conspecific matches!")
                            .go();
                    return;
                }
                ...
                FileDialog fd =
                        new FileDialog(
                                seqId.getFrame(),
                                "Export sequences with valid conspecifics as Fasta files to ...",
                                FileDialog.SAVE);
                ...
                File f = null;
                if (fd.getFile() != null) {
                    if (fd.getDirectory() != null) f = new File(fd.getDirectory(), fd.getFile());
                    else f = new File(fd.getFile());

                    // assume that 'f' has already been tested for existance, and overwriting
                    // verified by
                    // FileDialog.
                    ...
                    try {
                        ...
                        new MessageBox(
                                        seqId.getFrame(),
                                        "Error!",
                                        "There was a problem while exporting species with multiple"
                                            + " sequences. The technical description of the error"
                                            + " is: "
                                                + ex.getMessage()
                                                + "\n\n"
                                                + "Are you sure you have adequate permissions to"
                                                + " write to that file, and the disk isn't empty?"
                                                + " If not, it's probably a programming problem."
                    ...
                    new MessageBox(
                                    ...
                                    "Success!",
                                    sl_results.count()
                                            + " sequences containing atleast one valid conspecific"
                                            + " sequence exported to "
                                            + f
                                            + " in the FASTA format.")
                            .go();
                }
        ...
        if (e.getSource().equals(btn_export_multiple)) {
            ...
            File file = null;
            if (fd.getFile() != null) {
                if (fd.getDirectory() != null) file = new File(fd.getDirectory() + fd.getFile());
                else file = new File(fd.getFile());
                ...
                try {
                    ...
                    new MessageBox(
                                    ...
                                    "Error!",
                                    "There was a problem while exporting species with multiple"
                                        + " sequences. The technical description of the error is: "
                                            + ex.getMessage()
                                            + "\n\n"
                                            + "Are you sure you have adequate permissions to write"
                                            + " to that file, and the disk isn't empty? If not,"
                                            + " it's probably a programming problem. Please let the"
                                            + " developers know!",
                                    MessageBox.MB_ERROR)
                            .go();
                    return;
            ...
            new MessageBox(
                            ...
                            "Done!",
                            result.count()
                                    + " sequences were successfully exported to "
                                    + file
                                    + " in the Fasta format.")
                    .go();
        }
            ...
            if (det == null) {
                MessageBox mb =
                        new MessageBox(
                                ...
                                "I cannot find any sequences with the species '"
                                        + sp_name
                                        + "'! Are you sure you have't already removed or renamed"
                                        + " sequences?\n\n"
                                        + "Try recalculating the Species Summary. If this species"
                                        + " still appears, there might be a programming error in"
                                        + " this program.");
                mb.go();
            ...
            MessageBox mb =
                    new MessageBox(
                            ...
                            "Are you sure you want to delete all "
                                    + count
                                    + " sequences belonging to the species '"
                                    + sp_name
        ...
        if (e.getSource().equals(list_species)) {
            ...
            if (seq != null) seqId.getSequencePanel().selectSequence(seq);
            seqId.unlockSequenceList();
        }
    }
    ...
    public void run() {
        ...
        try {
            species =
                    list.getSpeciesDetails(
                            ProgressDialog.create(
                                    seqId.getFrame(),
                                    "Please wait, calculating species information ...",
                                    "Species summary information is being calculated. Sorry for the"
                                            + " wait.",
                                    0));
        ...
        str.append(
                "Number of sequences without a species name: "
                        + species.getSequencesWithoutASpeciesNameCount()
                        + "\n\n");
        // check
        str.append(
                "Number of sequences shorter than "
                        + Sequence.getMinOverlap()
                        + " base pairs: "
                        + species.getSequencesInvalidCount()
                        + "\n"); // check

        str.append(
                "Number of sequences with atleast one valid conspecific sequence: "
                        + species.getSequencesWithValidConspecificsCount()
                        + " ("
                        + com.ggvaidya.TaxonDNA.Common.DNA.Settings.percentage(
                                species.getSequencesWithValidConspecificsCount(), list.count())
                        + "%)\n");
        str.append(
                "Number of species with valid conspecifics: "
                        + species.getValidSpeciesCount()
                        + " ("
                        + com.ggvaidya.TaxonDNA.Common.DNA.Settings.percentage(
                                species.getValidSpeciesCount(), species.count())
                        + "% of all species)\n");
        // set up list_species
        //
        list_species.removeAll();
    ...
    public String getDescription() {
        return "Information on the species present in this dataset";
    }
    ...
    /**
     * exportRandomly() lets you export species (or, hopefully, sequences) at random. It pops up a
     * little dialog box, which lets you pick the number of sequences (or species) to export, the
     * format to export the remaining sequences in, and the directory into which to spit them out.
     */
    public void exportRandomly() {
        ...
        DirectoryInputPanel dinp =
                new DirectoryInputPanel(
                        "Directory to export random subsets to:",
        ...
        for (int x = 0; x < vec_Species.size(); x++) {
            ch_no_of_species.add(
                    (x + 1) + " species (" + percentage(x + 1, vec_Species.size()) + "%)");
        }
                ...
                if (ch_selected == ch_species) {
                    ...
                    new MessageBox(
                                    ...
                                    rands
                                            + " randomizations of "
                                            + species
                                            + " species each were exported to "
                                            + dir)
                            .go();

                } else {
                    exportSpecimensRandomly(
                            dir,
                            specimens,
                            rands,
                            ProgressDialog.create(
                    ...
                    new MessageBox(
                                    ...
                                    rands
                                            + " randomizations of "
                                            + specimens
                                            + " specimens each were exported to "
                                            + dir)
                            .go();
                }
    ...
    public void exportSpeciesRandomly(File dir, int species, int rands, ProgressDialog pd)
    ...
    public void exportSpecimensRandomly(File dir, int specimens, int rands, ProgressDialog pd)
            throws IOException, DelayAbortedException {
        if (pd != null) pd.begin();
