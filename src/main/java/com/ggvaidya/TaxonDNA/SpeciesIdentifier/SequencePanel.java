...
public class SequencePanel extends Panel
  ...
  /**
   * Flips the lists. The way it works is: - We have 'list', which is where all the event capturing,
   * etc. go on. 'list' is always in the front. - We have 'backbuffer', which is where all edit
   * operations are carried out. When completed, we call flipLists() to flip them around.
   *
   ...
   * AGAINST MY WILL i'm going to be quick about this, which unfortunately means O(n) time. Bite me.
   *
  ...
  public void selectSequence(Sequence seq) {
    for (int x = 0; x < list.count(); x++) {
      if (list.get(x).equals(seq)) {
        ...
        itemStateChanged(
            new ItemEvent(this, ItemEvent.ITEM_STATE_CHANGED, new Integer(x), ItemEvent.SELECTED));
