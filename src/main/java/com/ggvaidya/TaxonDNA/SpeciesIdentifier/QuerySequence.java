/**
 * A UIExtension which compares a given sequence against the whole set of sequences. We have a
 * public function - query(Sequence) - which we can use to query a sequence.
 *
 * <p>NOTES: SortedSequenceList We maintain our very own SortedSequenceList in our own class. This
 * will pretty obviously (and pretty quickly) fall out of sync with SpeciesIdentifier's list. THIS
 * MEANS THAT IF YOU MODIFY A FILE IN TAXONDNA, THE RESULTS OF THE SEARCH WILL USE THE LAST SET.
 * Click on "Query", and everything will make sense again.
 *
 * <p>I am not locking our local SSL. PLEASE NOBODY TOUCH THIS EVER PLEASE OH PLEASE.
 */
/*
    TaxonDNA
...
public class QuerySequence extends Panel implements UIExtension, ActionListener, ItemListener {
  ...
  private void query(String string_sequence) {
    ...
    try {
      ...
      for (int c = 0; c < x.length(); c++) {
        if (!Character.isWhitespace(x.charAt(c))) buff.append(x.charAt(c));
      }
      ...
      try {
        sset.sortAgainst(
            ...
            ProgressDialog.create(
                ...
                "All the pairwise distances are being calculated. Please wait.",
                0));
        seqId.unlockSequenceList();
  ...
  private void displayDetails(Sequence seq) {
    ...
    if (seq != null && sset != null) {
      ...
      str.append(
          "This sequence has a pairwise distance of "
              + str_pairwise
      ...
      if (seqId.getExtension("Pairwise Summary") != null) {
        ...
        if (pd_intra == null || pd_inter == null) {
          ...
          double prob_intra =
              100 - percentage(pd_intra.getBetween(0, pairwise), pd_intra.countValidComparisons());
          double prob_inter =
              percentage(pd_inter.getBetween(0, pairwise), pd_inter.countValidComparisons());

          if (pd_intra.countValidComparisons() > 0)
            str.append(
                prob_intra
                    + "% of intraspecific distances are larger than "
                    + str_pairwise
                    + "%.\n");
          ...
          if (pd_inter.countValidComparisons() > 0)
            str.append(
                prob_inter
                    + "% of intrageneric but interspecific distances are smaller than "
                    + str_pairwise
                    + "%.\n");
        }
      } else {
        str.append(
            "The pairwise distribution module (PairwiseSummary) has not been loaded into this program. I need that program to calculate pairwise distances.\n\nPlease recompile this program.");
      }

      str.append(
          "\nNon-matching nucleotides:\t"
              + (seq.getActualLength() - seq.countIdentical(query))
              + "\tbase pairs");
  ...
  public String getShortName() {
    return "Query against sequences";
  }

  public String getDescription() {
    return "Query a sequence against the other sequences in this dataset";
  }
