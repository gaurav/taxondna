/**
 * SpeciesIdentifier is a program for interacting with DNA sequences in a taxonomy-intelligent way.
 * Once finished (sometime before the heat death of the universe, we hope), this program will deal
 * with DNA sequences, use them to help construct taxonomies, identify species, and do a whole load
 * of other cool things we haven't even thought about yet.
 *
 * <p>How cool is that? Very cool.
 */
...
public class SpeciesIdentifier
    ...
    public static void main(String[] args) {
        if (args.length == 0) new SpeciesIdentifier();
        else CommandLine.processCommandLine(args);
    }
    ...
    //
    /**
     * Create a new SpeciesIdentifier object. Each SpeciesIdentifier is essentially a window to deal
     * with a particular SequenceList. We will start off by having an empty, unset SequenceList.
     */
    ...
    /**
     * Create a new SpeciesIdentifier object. Each SpeciesIdentifier is essentially a class to deal
     * with a particular SequenceList. We will figure out the name of the class, etc. from the
     * SequenceList. The SequenceList can be 'null'.
     *
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (cmd.equals("Open")) {
            ...
            FileDialog fd =
                    new FileDialog(
                            mainFrame, "Which file would you like to open?", FileDialog.LOAD);
            fd.setVisible(true);
        ...
        if (cmd.equals("Save As")) {
            ...
            FileDialog fd =
                    new FileDialog(
                            mainFrame, "Where would you like to save this file?", FileDialog.SAVE);
            fd.setVisible(true);

            if (fd.getFile() != null) {
                filename = fd.getFile();
                if (fd.getDirectory() != null) filename = fd.getDirectory() + filename;
        ...
        //
        if (cmd.equals("Exit")) exitTaxonDNA();
            ...
            if (fd.getFile() != null) {
                ...
                MessageBox mb =
                        new MessageBox(
                                ...
                                "All current sequences were successfully exported to "
                                        + handler.getShortName()
                                        + ", and stored in '"
                                        + filename
                                        + "'.");
            ...
            String copyrightNotice =
                    new String(
                            "You should cite SpeciesIdentifier as follows:\n\t"
                                    + getCitation()
                                    + "\n---\n"
                                    + "TaxonDNA/SpeciesIdentifier "
                                    + Versions.getTaxonDNA()
                                    + ", Copyright (C) 2005-06 Gaurav Vaidya.\n\n"
                                    + "TaxonDNA comes with ABSOLUTELY NO WARRANTY. This is free"
            ...
            MessageBox mb =
                    new MessageBox(
                            ...
                            copyrightNotice
                                    ...
                                    + "Thanks to Guanyang and Prof Rudolf Meier for extensively"
                                    + " testing.\n\n"
                                    + "Thanks to Kathy for single-handedly figuring out the reason"
                                    + " for the 'Cluster' bug, now fixed!\n\n"
                                    + "This program was written with Vim (http://vim.org), with"
                                    + " some refactoring assistance from Eclipse"
                                    + " (http://eclipse.org/). Compilation was handled by Ant"
                                    + " (http://ant.apache.org/). I use BrowserLauncher"
                                    + " (http://browserlauncher.sf.net/) to handle opening a"
                                    + " website in a cross-platform way.");
            mb.go();
        }
    }

    /**
     * We monitor the list of possible actions, (from the list_actions, in the upper left hand
     * corner?) and handle clicks on that list (against the UIExtensions)
     */
                    ...
                    SequenceList sequences =
                            SequenceList.readFile(
                                    ...
                                    ProgressDialog.create(
                                            ...
                                            "Loading '"
                                                    + file
                                                    + "' into a new window. Sorry for the delay!",
                                            ProgressDialog.FLAG_NOCANCEL
                                            // for future reference: this flag is here because we
                                            // *can't* cancel
    ...
    public boolean saveFile() {
        ...
        try {
            sequences.writeToFile(
                    ProgressDialog.create(
                            mainFrame,
                            "Please wait, saving file ...",
                            "Saving sequences into '"
                                    + sequences.getFile().getAbsolutePath()
                                    + "' in "
                                    + sequences.getFormatHandler().getShortName()
                                    + " format",
                            0));
    ...
    public boolean loadFile(File file, FormatHandler handler) {
        ...
        try {
            ...
            MessageBox mb =
                    new MessageBox(getFrame(), "Could not read file " + file + "!", e.getMessage());
            mb.go();
    ...
    /**
     * Closes the current file. Program is reset to a waiting state, with SequenceList in particular
     * being set back to null.
     */
        ...
        if (sequences != null && sequences.isModified()) {
            MessageBox mb =
                    new MessageBox(
                            mainFrame,
                            "Save file now?",
                            "The file currently loaded has been changed since the last time you"
                                    + " saved it. Would you like to save it now?",
    ...
    //
    /**
     * Locks and returns a copy of the SequenceList. This function will block until all previous
     * locks on the SequenceList are cleared.
     */
    public SequenceList lockSequenceList() {
        if (sequences != null) sequences.lock();
        return sequences;
    }

    /**
     * Unlock sequence list. Data MIGHT have been changed internally, so to be on the safe side, we
     * resetFrameTitle() -- this would be unnecessary if the number of sequences weren't in the
     * frame title.
     */
    public void unlockSequenceList() {
        if (sequences != null) sequences.unlock();
        resetFrameTitle();
    }

    /**
     * Unlock sequence list, changing the ENTIRE underlying list as we unlock it. Remember: if you
     * want to change entries, please use methods on SequenceList. This function is STRICTLY for
     * when you want to (and the user is expecting) having the ENTIRE SequenceList changed from
     * under his feet. Technically, saveFile() should be the only who does this.
     *
     * <p>To reiterate: SpeciesIdentifier will respond to this, badly. Everything will flip around.
     * Things will change. It could be MINUTES, even HOURS before this function returns. Don't do
    ...
    /** This function rewrites the SequencePanel and notifies everybody that data has changed. */
    ...
    public void goToExtension(String extName) {
        ...
        MessageBox.messageBox(
                mainFrame,
                "Extension not found!",
                "This part of this program needs to use component '"
                        + extName
                        + "', which does not exist.\n\n"
                        + "This is most likely a programming error. Please contact the"
                        + " programmer.");
    }
    ...
    /**
     * Returns our current sequence panel. Anybody who needs to know, or play around with this (I'm
     * looking at you, SequenceEdit) can just pick it up and talk directly to SequencePanel.
     */
     ...
     * if the extension doesn't exist, this can also be used to check if extensions have been
     * loaded. It's all a bit nuts though, because the code will still need the class to be checked!
    ...
    private void registerUIExtensions() {
        ...
        uiExtensions.add((UIExtension) new DistanceAnalysis(this));
        ...
        uiExtensions.add((UIExtension) new ExportBySpeciesName(this));
        uiExtensions.add((UIExtension) new OverlapAnalysis(this));
    }
    ...
    private void resetFrameTitle() {
        StringBuffer title =
                new StringBuffer("TaxonDNA/SpeciesIdentifier " + Versions.getTaxonDNA());

        if (sequences != null) {
            File file = sequences.getFile();

            if (file != null) title.append(": " + file.getAbsolutePath());

            if (sequences.count() > 0) title.append(" (" + sequences.count() + " sequences)");

            if (sequences.isModified()) title.append(" (modified)");
        }
