...
public class SpeciesIdentifier
  ...
  //
  /**
   * SpeciesIdentifier's main entrypoint. We check for command line arguments (in this case, all of
   * which must be files) and we start them using a call to 'new SpeciesIdentifier(file)'.
   */
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    if (cmd.equals("Open")) {
      ...
      FileDialog fd =
          new FileDialog(mainFrame, "Which file would you like to open?", FileDialog.LOAD);
      fd.setVisible(true);
    ...
    if (cmd.equals("Save As")) {
      ...
      FileDialog fd =
          new FileDialog(mainFrame, "Where would you like to save this file?", FileDialog.SAVE);
      fd.setVisible(true);
    ...
    if ((cmd.length() > 7) && (cmd.substring(0, 7).equals("Export_"))) {
      ...
      if (fd.getFile() != null) {
        ...
        try {
          ...
          MessageBox mb =
              new MessageBox(
                  getFrame(), "Could not write file!", getMessage(Messages.IOEXCEPTION_WRITING, e));
          mb.go();
  ...
  public boolean saveFile() {
    ...
    if (file == null) {
      FileDialog fd =
          new FileDialog(
              mainFrame, "What name would you like to save this file under?", FileDialog.SAVE);
      fd.setVisible(true);
  ...
  //
  /**
   * Locks and returns a copy of the SequenceList. This function will block until all previous locks
   * on the SequenceList are cleared.
   */
   ...
   * resetFrameTitle() -- this would be unnecessary if the number of sequences weren't in the frame
   * title.
   */
   ...
   * want to change entries, please use methods on SequenceList. This function is STRICTLY for when
   * you want to (and the user is expecting) having the ENTIRE SequenceList changed from under his
   * feet. Technically, saveFile() should be the only who does this.
   *
   * <p>To reiterate: SpeciesIdentifier will respond to this, badly. Everything will flip around.
   * Things will change. It could be MINUTES, even HOURS before this function returns. Don't do this
  ...
  /**
   * Retrieve the extension by name from the vector of loaded extensions. Since it returns a null if
   * the extension doesn't exist, this can also be used to check if extensions have been loaded.
  ...
  private MenuBar createMenuBar() {
    ...
    // new MenuShortcut(KeyEvent.VK_A)) --> deleting the shortcut since it gets in the way of Ctrl-A
    // on Windows
    file.addSeparator();
    file.add("Exit");
