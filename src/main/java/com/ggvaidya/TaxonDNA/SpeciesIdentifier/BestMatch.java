            ...
            if (text_threshold.getText().trim().isEmpty()) { // no threshold specified
                MessageBox mb =
                        new MessageBox(
                                ...
                                "You did not specify a threshold for the \"best close match\""
                                    + " algorithm!\n\n"
    ...
    public void dataChanged() {
        ...
        if (set == null) {
            ...
            if (processingDone) {
                ...
                text_main.setText(
                        "Please press the 'Calculate' button to begin a best match analysis.");
            }
        }
        ...
        */
        ...
        while (i.hasNext()) {
            ...
            for (int y = 2; y < count_sequences; y++) {
                ...
                if (match == null) {
                    ...
                    throw new RuntimeException(
                            "I ran out of Sequences when looking up "
                                    + query
            ...
            for (int y = 1; y < count_sequences; y++) {
                ...
                if (match.getSpeciesName() == null) {
                    // if the match has no species name, no worries - we'll catch it in the query
                    // check above
                ...
                if (match.getPairwise(query) >= 0) {
                    if (first_con == null
                            && match.getSpeciesName().equals(query.getSpeciesName())) {
                        // conspecific
                        first_con = match;
                        count_seqs_with_valid_conspecific_matches++;
                    } else if (first_allo == null
                            && !match.getSpeciesName().equals(query.getSpeciesName())) {
                        // allospecific
                        first_allo = match;
                    }
            ...
            if (bestMatch.getSpeciesName() != null
                    && bestMatch.getSpeciesName().equals(query.getSpeciesName()))
                conspecific = true;
            ...
            if (!clean_block && !mixed_block) {
                ...
                if (conspecific) {
                    str_listings.append(
                            "\tSuccessful match at " + percentage(bestMatchDistance, 1) + "%");
                    best_match_correct++;
                    ...
                    str_listings.append(
                            "\tIncorrect match at " + percentage(bestMatchDistance, 1) + "%");
                ...
                throw new RuntimeException(
                        "Programming error: the program is now somewhere where it really shouldn't"
                                + " be. Please contact the programmer!");
            }
        }
        ...
        text_main.setText(
                "Sequences:\t"
                        ...
                        + "\n"
                        + "Sequences without recognizable species names (ignored in all subsequent"
                        + " counts):\t"
                        + count_sequences_without_species_names
                        + "\nSequences with atleast one matching sequence in the data set:\t"
                        + count_sequences_with_valid_matches
                        + "\n"
                        + "Sequences with atleast one matching conspecific sequence in the data"
                        + " set:\t"
                        + count_seqs_with_valid_conspecific_matches
                        ...
                        + "%)"
                        +
                        //				"\nSequences without any match with adequate overlap:\t" +
                        // best_match_noallo + "
