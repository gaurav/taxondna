...
public class BarcodeGenerator extends Panel
    ...
    public BarcodeGenerator(SpeciesIdentifier seqId) {
        ...
        settings.add(
                new Label(
                        "We will use the inclusivity algorithm to generate consensus sequences."
                                + " Thus, A+T will become W, and W+C will become H."),
                cons);
    ...
    public void run() {
        ...
        //		System.err.println("Number of sequences which should match: " +
        // num_sequences_should_match);

        ProgressDialog delay =
                ProgressDialog.create(
                        seqId.getFrame(),
                        "Please wait, generating sequence barcodes ...",
                        "Please wait, sequence barcodes are being generated. This may take a"
                                + " while.",
                        0);
        ...
        try {
            while (i.hasNext()) {
                ...
                if (group.size() == 1) {
                    ...
                    //				System.err.println("Consensus of " + name + " (" + count_sequences + "): "
                    // + str);
                    results.add(
                            new Sequence(
                                    name + " (barcode of " + count_sequences + " sequences)", str));
                }
            }
            ...
            FileDialog fd =
                    new FileDialog(
                            seqId.getFrame(),
                            "Save species barcodes as FASTA file ...",
                            FileDialog.SAVE);
            fd.setVisible(true);
            ...
            MessageBox.messageBox(
                    ...
                    "An incorrect sequence was generated. This is an error in the program. Please"
