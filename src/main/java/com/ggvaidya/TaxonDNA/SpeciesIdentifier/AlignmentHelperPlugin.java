/**
 * AlignmentHelperPlugin.java Since the AlignmentHelper stuff is likely to be a horrible hack, I
 * think it makes sense to make it it's OWN horrible hack, and leave it at that. If you don't want
 * it, just remove the corresponding gridBag.addUIExtension() line from SequenceIdentifier.
 *
...
public class AlignmentHelperPlugin extends Panel implements UIExtension, ActionListener {
    ...
    public AlignmentHelperPlugin(SpeciesIdentifier view) {
        ...
        finp_Input =
                new FileInputPanel(
                        "Clustal input: ", FileInputPanel.MODE_FILE_WRITE, view.getFrame());
        ...
        rl.add(
                new Label("2. Import results back from Clustal"),
                RightLayout.NEXTLINE | RightLayout.FILL_2);

        finp_Output =
                new FileInputPanel(
                        "Clustal output: ", FileInputPanel.MODE_FILE_READ, view.getFrame());
    ...
    */
    ...
    public SequenceList importSequenceSet(SequenceList set_initial, File inputFile) {
        ...
        if (set_map == null) set_map = new SequenceList();
        ...
        if (set_map.count() > 0) {
            ...
            if (list.getFile() != null) path = list.getFile().getParent() + File.separator;
            ...
            if (id == null || id.isEmpty()) {
                ...
                if (!warned) {
                    mb =
                            new MessageBox(
                                    seqId.getFrame(),
                                    "Sequences without GI numbers detected!",
                                    "Some of the sequences in this dataset do not have GI numbers."
                                        + " To create Clustal input files, I'm going to have to"
                                        + " allocate unique identifiers to these sequences. Doing"
                                        + " so will not cause problems with this dataset, but it"
                                        + " WILL rewrite the original file. THE CURRENT FILE WILL"
                                        + " BE OVERWRITTEN, AND ANY CHANGES YOU'VE MADE WILL BE"
                                        + " SAVED PERMANENTLY TO DISK. Are you sure you want to do"
                                        + " this?",
                                    MessageBox.MB_YESNO);
                    if (mb.showMessageBox() == MessageBox.MB_YES) {
                        ...
                        mb =
                                new MessageBox(
                                        seqId.getFrame(),
                                        "Clustal export failed.",
                                        "I can't export files for Clustal without assigning unique"
                                            + " identifiers to each sequence. Please re-run if you"
                                            + " would like to export files for Clustal.");
                        mb.go();
                ...
                mb =
                        new MessageBox(
                                seqId.getFrame(),
                                "You have duplicate sequences!",
                                "Two sequences in this dataset have identical GI numbers. I can't"
                                    + " use this for a mapfile. Please delete one of the duplicate"
                                    + " pair of sequences. The duplicate pair which caused this"
                                    + " error was:\n"
                                    + "\t"
                                        + seq.getFullName()
                                        + "\nand\nGI:\t"
    ...
    public void exportSequenceSet(SequenceList set, File outputFile) {
        ...
        try {
            ...
            if (e.getMessage() != null && e.getMessage().equals("Duplicate:notified")) return;
                ...
                if (uniques.get(id) != null) {
                    ...
                    MessageBox mb =
                            new MessageBox(
                                    seqId.getFrame(),
                                    "You have duplicate sequences!",
                                    "Two sequences in this dataset have identical GI numbers. I"
                                        + " can't use this for a mapfile. Please delete one of the"
                                        + " duplicate pair of sequences. The duplicate pair which"
                                        + " caused this error was:\n"
                                        + "\t"
                                            + seq.getFullName()
                                            + "\nand\nGI:\t"
                                            + id);
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (btn.equals(btn_Export)) {
            ...
            if (set != null) exportSequenceSet(set, file_input);
            seqId.unlockSequenceList();
        ...
        if (btn.equals(btn_Import_Here)) {
            ...
            if (set_fixed == null) set_fixed = new SequenceList();

            String path = "";
            if (set.getFile() != null) path = set.getFile().getParent() + File.separator;
    ...
    public String getShortName() {
        return "Clustal Mapping";
    }

    public String getDescription() {
        return "Exports and re-imports special datasets with shortened names";
    }
