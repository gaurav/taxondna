/**
 * A UIExtension which allows you to set up the basic settings we will use for everything.
 *
...
public class Configuration extends Panel implements UIExtension, ActionListener, FocusListener {
  ...
  Label warningLabel =
      new Label(
          "Please wait, loading SpeciesIdentifier ..."); // warning: please set up BEFORE loading
  // files
  ...
  public Configuration(SpeciesIdentifier seqId) {
    ...
    */
    ...
    rl.add(
        new Label("Calculate pairwise distances as"), RightLayout.NEXTLINE | RightLayout.STRETCH_X);
    ...
    //
    rl.add(
        new Label("Do not compare two sequences unless they have atleast "),
    ...
    choice_ambiguity.add(
        "Ambiguous codons are used ('H' is treated as a combination of 'W' and 'C')");
    ...
    rl.add(
        new Label(
            "Note that differences in leading and trailing gaps are ignored, while differences in internal gaps will be counted."),
        RightLayout.NEXTLINE | RightLayout.FILL_3);
    ...
    if (lockDown_in_effect) lockDown(lockDown_in_effect);
  }
  ...
  public String getDescription() {
    return "Change the settings used by all parts of this program";
  }
  ...
  /**
   * From UIExtension. We'd like to know whether a new file is loaded (and we should be LOCKED) or
   * if a 'null' was loaded, in which case we should be UNLOCKED.
   */
  public void dataChanged() {
    ...
    if (list == null) {
      ...
      warningLabel.setText(
          "There is a file loaded into this program. You cannot change these settings unless you reload it.");
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource().equals(lock_button)) {
      if (locked) unlock();
        ...
        lock();
        warningLabel.setText(
            "You have locked these settings. Please click on the \"Unlock\" button to unlock it.");
        lock_button.setLabel("Unlock Settings");
      }
    }
  }
  ...
  /**
   * Loads all configuration options from the Preferences store. This will update Configuration
   * items as well, so please only call after initialisation.
   */
  public void loadFromPrefs() {
    Preferences prefs =
        Preferences.userNodeForPackage(com.ggvaidya.TaxonDNA.SpeciesIdentifier.Configuration.class);
    ...
    Sequence.setPairwiseDistanceMethod(
        prefs.getInt("pairwiseDistanceMethod", Sequence.PDM_UNCORRECTED));
  ...
  private void lockDown(boolean state) {
    if (state) { // lock down!
      ...
      warningLabel.setText(
          "You cannot modify the Configuration if more than one SpeciesIdentifier windows are open. Please close all but one window if you need to change these settings.");
      lock_button.setEnabled(false);
    } else { // unlock down!
      unlock();
  ...
  /**
   * Set up a 'lockdown'. This is a simple binary, so please don't try locking and unlocking
   * multiple times. It's also a static method: it will unlock or lockup EVERY SINGLE Configuration
   * in existance.
   *
   * <p>Pretty cool, eh?
   */
