/**
 * Determines the largest "complete block" in your dataset. A "complete block" is one where every
 * single sequence in the dataset has no gaps whatsoever.
 *
...
public class CompleteOverlap extends Panel implements UIExtension, Runnable, ActionListener {
    ...
    private Checkbox check_wellDefinedOnly =
            new Checkbox("Drop all sequences which have more than ");
    ...
    private String getOverlapDisplay(SequenceList sl, int from, int to, DelayCallback delay) {
        ...
        int ambiguous_allowed =
                (int) (ambiguous_percent * (double) (step_width)); // how MANY are allowed?
        ...
        while (i.hasNext()) {
            ...
            try {
                ...
                MessageBox mb =
                        new MessageBox(
                                seqId.getFrame(), "Unable to extract subsequence", e.toString());
                mb.go();
            ...
            if (subseq.getSequenceWithExternalGaps().indexOf('_') != -1) continue;

            // if check_wellDefinedOnly is on, don't let ambiguous sequences through
            if (check_wellDefinedOnly.getState()) {
                if (subseq.getAmbiguous() > ambiguous_allowed) continue;
            }
    ...
    private String pad_string(String x, int size) {
        ...
        if (x.length() < size) {
            ...
            return (x + buff);
        } else if (x.length() == size) return x;
        else return (x.substring(0, size - 3) + "...");
    }
        ...
        if (evt.getSource().equals(btn_Export_Fasta)) {
            ...
            ProgressDialog pd =
                    ProgressDialog.create(
                            ...
                            "Writing out all overlaping regions between "
                                    + from
                                    + " and "
                                    + to
            ...
            FileDialog fd =
                    new FileDialog(
                            seqId.getFrame(),
                            "Where would you like me to save the results for "
                                    + from
                                    + " to "
                                    + to
                                    + " to?",
    ...
    /**
     * @param ambiguous_allowed the NUMBER of ambiguous base pairs allowed in any sequence for it be
     *     'defined'
     */
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                for (int x = 0; x < to - from; x++) {
                    ...
                    if (Sequence.isAmbiguous(ch)) no_ambiguous_bases++;
                }

                if (sequence_failed) continue;
                ...
                if (no_ambiguous_bases <= ambiguous_allowed) no_sequences_defined++;
    ...
    public void run() {
        ...
        ProgressDialog pd =
                ProgressDialog.create(
                        ...
                        "I am attempting to determine the largest complete block right now. Sorry"
    ...
    public String getDescription() {
        return "Find continuous sections (\"blocks\") without missing bases or external gaps";
    }
