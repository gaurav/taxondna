...
public class PairwiseExplorer extends Panel
    ...
    public PairwiseExplorer(SpeciesIdentifier view) {
        ...
        buttons.setLayout(
                new FlowLayout(FlowLayout.LEFT)); // doesn't work; we're in BorderLayout.NORTH
    ...
    public void run() {
        ...
        try {
            ...
            if (Thread.currentThread().getName().equals("IntraPairwiseExplorer")) {
                if (intra == null) {
                    intra =
                            new PairwiseDistances(
                                    list,
                                    PairwiseDistances.PD_INTRA,
                                    ProgressDialog.create(
                                            identifier.getFrame(),
                                            "Calculating pairwise distances",
                                            "All intraspecific pairwise distances are being"
                    ...
                    inter =
                            new PairwiseDistances(
                                    ...
                                    ProgressDialog.create(
                                            identifier.getFrame(),
                                            "Calculating pairwise distances",
                                            "All interspecific, congeneric pairwise distances are"
                                                    + " being calculated. Sorry for the delay!",
                                            0));
                }
                current = inter;
            }
        ...
        for (int x = -5; x < 1000; x += 5) {
            ...
            if (distances.size() > 0) {
                ...
                if (x + 5 == 0) key = "Before 0% (" + distances.size() + " matches)";
     ...
     * same.
     */
    private void printAllDistances(PrintWriter pw, PairwiseDistances pd, DelayCallback delay)
            ...
            if (hash_distances != null && e.getStateChange() == ItemEvent.SELECTED) {
                ...
                if (key.equals("Averages")) {
                    ...
                    buff =
                            new StringBuffer(
                                    "The following are the average "
                                            + type
                                            + " distances for the following sequences:\n");
                    ...
                    while (i.hasNext()) {
                        ...
                        if (Double.isNaN(d)) buff.append(spName + "\t\tNo valid comparisons\n");
                        else {
                            d *= 100;
                            d *= 10000;
                    ...
                    while (i.hasNext()) {
                        ...
                        buff.append(
                                "\t"
                                        + pd.getSequenceA().getDisplayName()
                                        + "\t"
                                        + pd.getSequenceB().getDisplayName()
                                        + "\t"
                                        + distance
                                        + "%\n");
                    }
    ...
    public String getShortName() {
        return "Pairwise Explorer";
    }

    public String getDescription() {
        return "Allows you to explore the pairwise distances present in this dataset";
    }
