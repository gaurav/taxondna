...
public class PairwiseSummary extends Panel implements UIExtension, ActionListener, Runnable {
  ...
  public void run() {
      ...
      intra =
          new PairwiseDistribution(
              ...
              ProgressDialog.create(
                  ...
                  "All intraspecific pairwise distances are being calculated. Sorry for the delay!",
                  0));
      inter =
          new PairwiseDistribution(
              set,
              ...
              ProgressDialog.create(
                  seqId.getFrame(),
                  "Calculating pairwise distances",
                  "All interspecific, congeneric pairwise distances are being calculated. Sorry for the delay!",
                  0));
    ...
    // all.countValidComparisons() + "\tcomparisons (minimum overlap is " + Sequence.getMinOverlap()
    // + " bp)");
    ...
    println(
        ...
        "Note that minimum overlap is set to "
            ...
            + Sequence.getMinOverlap()
            + " bp overlap will not be counted as a pairwise distance for this analysis.");
    ...
    println(
        str,
        "WARNING: These calculations are new to Species Identifier as of June 14, 2012, and have not been comprehensively tested yet. Please use with caution!\n");

    if (intra.countValidComparisons() == 0) {
      // No intra distances.
      println(
          str,
          "There are no valid intraspecific pairwise distances (i.e. no two sequences have less than "
              + Sequence.getMinOverlap()
              + " bp overlap).");
      ...
      println(
          ...
          "Intraspecific pairwise distances range between "
              ...
              + "% (contains "
              + percentage(
                  intra.getBetweenIncl(intra.getMinimumDistance(), intra.getMaximumDistance()),
      ...
      println(
          ...
          "  Removing the largest 5% of intraspecific distances reduces that range to "
              ...
              + "% (contains "
              + percentage(
                  intra.getBetweenIncl(intra.getMinimumDistance(), max_intra_distance),
                  intra.countValidComparisons())
              + "% of all valid intraspecific pairwise distances)");
    }
    ...
    if (inter.countValidComparisons() == 0) {
      ...
      println(
          str,
          "There are no valid interspecific pairwise distances (i.e. no two sequences have less than "
              + Sequence.getMinOverlap()
              + " bp overlap).");
      ...
      println(
          ...
          "Interspecific, intrageneric pairwise distances range between "
              ...
              + "% (contains "
              + percentage(
                  inter.getBetweenIncl(inter.getMinimumDistance(), inter.getMaximumDistance()),
                  inter.countValidComparisons())
              + "% of all valid interspecific, intrageneric pairwise distances)");

      println(
          str,
          "  Removing the largest 5% of interspecific, intrageneric distances reduces that range to "
              + percentage(inter.getMinimumDistance(), 1)
              + "% and "
              + +percentage(min_inter_distance, 1)
              + "% (contains "
              + percentage(
                  inter.getBetweenIncl(min_inter_distance, inter.getMaximumDistance()),
                  inter.countValidComparisons())
              + "% of all valid interspecific, intrageneric pairwise distances)");
    }
    ...
    if (inter.countValidComparisons() == 0) {
      ...
      if (distances_intra.size() == 0) { // no distances for intra
        ...
        println(
            str, "Overlap with 5% error margs on both ends:\t No intraspecific distances present.");
      } else if (distances_inter.size() == 0) { // no distance for inter
        println(str, "Total overlap:\t No interspecific distances present.");
        println(
            str, "Overlap with 5% error margs on both ends:\t No interspecific distances present.");
      } else {
        int x = 0;
    ...
    println(
        str, intra.getDistributionAsString(0.0, 0.20, 0.005, PairwiseDistribution.CUMUL_BACKWARD));

    println(
        str, "\nDISTRIBUTION FOR ALL INTRASPECIFIC DISTANCES (FROM 0.0 TO 0.30) AT 1% intervals");
    println(
        str, intra.getDistributionAsString(0.0, 0.30, 0.01, PairwiseDistribution.CUMUL_BACKWARD));

    println(str, "\nDISTRIBUTION FOR ALL INTRASPECIFIC DISTANCES");
    println(
        str, intra.getDistributionAsString(0.0, 1.0, 0.05, PairwiseDistribution.CUMUL_BACKWARD));

    println(
        str, "\nDISTRIBUTION FOR ALL INTERSPECIFIC DISTANCES WITHIN A GENUS (FROM 0.0 to 0.20)");
    println(
        str, inter.getDistributionAsString(0.0, 0.20, 0.005, PairwiseDistribution.CUMUL_FORWARD));

    println(
        str,
        "\nDISTRIBUTION FOR ALL INTERSPECIFIC DISTANCES WITHIN A GENUS (FROM 0.0 TO 0.30) AT 1% intervals");
    println(
        str, inter.getDistributionAsString(0.0, 0.30, 0.01, PairwiseDistribution.CUMUL_FORWARD));
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    if (btn.equals(btn_Copy)) {
      ...
      if (dialog.getDirectory() != null) file = new File(dialog.getDirectory() + dialog.getFile());
