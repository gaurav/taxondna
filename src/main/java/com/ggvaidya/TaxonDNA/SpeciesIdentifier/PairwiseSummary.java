/**
 * A UIExtension which allows for summary of all pairwise distances. This ought to be the
 * architypical "basic" pairwise distance thingie.
 */
/*
    TaxonDNA
...
public class PairwiseSummary extends Panel implements UIExtension, ActionListener, Runnable {
    ...
    public void run() {
        ...
        if (set.count() < 2) {
            text_main.setText(
                    "Pairwise distributions cannot be determined for less than two sequences");
            seqId.unlockSequenceList();
            return;
        }
        ...
        try {
            intra =
                    new PairwiseDistribution(
                            set,
                            PairwiseDistribution.PD_INTRA,
                            ProgressDialog.create(
                                    seqId.getFrame(),
                                    "Calculating pairwise distances",
                                    "All intraspecific pairwise distances are being calculated."
                                            + " Sorry for the delay!",
                                    0));
            inter =
                    new PairwiseDistribution(
                            ...
                            ProgressDialog.create(
                                    seqId.getFrame(),
                                    "Calculating pairwise distances",
                                    "All interspecific, congeneric pairwise distances are being"
                                            + " calculated. Sorry for the delay!",
                                    0));
        ...
        while (i.hasNext()) {
            ...
            if (seq.getSpeciesName() != null) species.put(seq.getSpeciesName(), new Integer(1));
        }
        ...
        // println(str, "No of valid pairwise comparisons with adequate overlap:",
        // all.countValidComparisons() + "\tcomparisons (minimum overlap is " +
        // Sequence.getMinOverlap()
        // + " bp)");

        println(
                str,
                "Note that minimum overlap is set to "
                        + Sequence.getMinOverlap()
                        + " bp: pairs of sequences with less that "
                        + Sequence.getMinOverlap()
                        + " bp overlap will not be counted as a pairwise distance for this"
                        + " analysis.");
        ...
        println(
                str,
                "WARNING: These calculations are new to Species Identifier as of June 14, 2012, and"
                        + " have not been comprehensively tested yet. Please use with caution!\n");

        if (intra.countValidComparisons() == 0) {
            // No intra distances.
            println(
                    str,
                    "There are no valid intraspecific pairwise distances (i.e. no two sequences"
                            + " have less than "
                            + Sequence.getMinOverlap()
                            + " bp overlap).");
            ...
            float max_intra_distance =
                    ((Float) distances_intra.get(distances_intra.size() - 1)).floatValue();

            while (x < distances_intra.size()) {
                Float dist = (Float) distances_intra.get(distances_intra.size() - x);
                if ((float) x / distances_intra.size() > 0.05) break;
            ...
            println(
                    str,
                    "Intraspecific pairwise distances range between "
                            + percentage(intra.getMinimumDistance(), 1)
                            + "% and "
                            + +percentage(intra.getMaximumDistance(), 1)
                            + "% (contains "
                            + percentage(
                                    intra.getBetweenIncl(
                                            intra.getMinimumDistance(), intra.getMaximumDistance()),
                                    intra.countValidComparisons())
                            + "% of all valid intraspecific pairwise distances)");

            println(
                    str,
                    "  Removing the largest 5% of intraspecific distances reduces that range to "
                            + percentage(intra.getMinimumDistance(), 1)
                            + "% and "
                            + +percentage(max_intra_distance, 1)
                            + "% (contains "
                            + percentage(
                                    intra.getBetweenIncl(
                                            intra.getMinimumDistance(), max_intra_distance),
                                    intra.countValidComparisons())
                            + "% of all valid intraspecific pairwise distances)");
        }
        ...
        if (inter.countValidComparisons() == 0) {
            ...
            println(
                    str,
                    "There are no valid interspecific pairwise distances (i.e. no two sequences"
                            + " have less than "
                            + Sequence.getMinOverlap()
                            + " bp overlap).");
            ...
            while (i.hasNext()) {
                ...
                if ((float) x / distances_inter.size() > 0.05) break;
            ...
            println(
                    str,
                    "Interspecific, intrageneric pairwise distances range between "
                            + percentage(inter.getMinimumDistance(), 1)
                            + "% and "
                            + +percentage(inter.getMaximumDistance(), 1)
                            + "% (contains "
                            + percentage(
                                    inter.getBetweenIncl(
                                            inter.getMinimumDistance(), inter.getMaximumDistance()),
                                    inter.countValidComparisons())
                            + "% of all valid interspecific, intrageneric pairwise distances)");

            println(
                    str,
                    "  Removing the largest 5% of interspecific, intrageneric distances reduces"
                            + " that range to "
                            + percentage(inter.getMinimumDistance(), 1)
                            + "% and "
                            + +percentage(min_inter_distance, 1)
                            + "% (contains "
                            + percentage(
                                    inter.getBetweenIncl(
                                            min_inter_distance, inter.getMaximumDistance()),
                                    inter.countValidComparisons())
                            + "% of all valid interspecific, intrageneric pairwise distances)");
        }

        // At this point, we need a one-sentence summary of either:
        //  "There is a gap containing no pairwise distances between inter and intraspecific
        // distances.
        ...
        if (inter.countValidComparisons() == 0) {
            ...
            println(
                    ...
                    "Total overlap:\tNo interspecific, intrageneric comparisons.\n"
                            + "              \tIntraspecific comparisons range from "
                            + percentage(intra.getMinimumDistance(), 1)
                            + "% to "
                            + percentage(intra.getMaximumDistance(), 1)
                            + "% (total width: "
                            + percentage(intra.getMaximumDistance() - intra.getMinimumDistance(), 1)
                            + "%)");
            ...
            if (distances_intra.size() == 0) { // no distances for intra
                ...
                println(
                        str,
                        "Overlap with 5% error margs on both ends:\t No interspecific distances"
                                + " present.");
            } else {
                int x = 0;
                ...
                println(
                        str,
                        "Total overlap:\t"
                                + percentage(overlap, 1)
                                + "% (from "
                                + percentage(min_inter_distance, 1)
                                + "% to "
                                + percentage(max_intra_distance, 1)
                                + "%, covering "
                                + percentage(within, count_comparisons)
                                + "% of all intra and interspecific but intrageneric sequences)");
                ...
                println(
                        str,
                        "Overlap with 5% error margins on both ends:\t "
                                + percentage(overlap, 1)
                                + "% (from "
                                + percentage(min_inter_distance, 1)
                                + "% to "
                                ...
                                + "%, covering "
                                + percentage(within, count_comparisons)
                                + "% of intra and interspecific but intrageneric sequences)");
        ...
        println(
                str,
                intra.getDistributionAsString(
                        0.0, 0.20, 0.005, PairwiseDistribution.CUMUL_BACKWARD));

        println(
                str,
                "\n"
                        + "DISTRIBUTION FOR ALL INTRASPECIFIC DISTANCES (FROM 0.0 TO 0.30) AT 1%"
                        + " intervals");
        println(
                str,
                intra.getDistributionAsString(
                        0.0, 0.30, 0.01, PairwiseDistribution.CUMUL_BACKWARD));

        println(str, "\nDISTRIBUTION FOR ALL INTRASPECIFIC DISTANCES");
        println(
                str,
                intra.getDistributionAsString(0.0, 1.0, 0.05, PairwiseDistribution.CUMUL_BACKWARD));
        ...
        println(
                str,
                inter.getDistributionAsString(
                        0.0, 0.20, 0.005, PairwiseDistribution.CUMUL_FORWARD));

        println(
                str,
                "\n"
                        + "DISTRIBUTION FOR ALL INTERSPECIFIC DISTANCES WITHIN A GENUS (FROM 0.0 TO"
                        + " 0.30) AT 1% intervals");
        println(
                str,
                inter.getDistributionAsString(0.0, 0.30, 0.01, PairwiseDistribution.CUMUL_FORWARD));

        println(str, "\nDISTRIBUTION FOR ALL INTERSPECIFIC DISTANCES WITHIN A GENUS");
        println(
                str,
                inter.getDistributionAsString(0.0, 1.0, 0.05, PairwiseDistribution.CUMUL_FORWARD));
    ...
    private void println(StringBuffer main, String x) {
        ...
        if (x.length() < 30) {
            for (int c = 0; c < (30 - x.length()); c++) buff.append(' ');
    ...
    /**
     * Combines all the distances in 'pd' into a single, long, '\n' delimited string and returns the
     * same.
     */
    private void printAllDistances(PrintWriter pw, PairwiseDistribution pd, DelayCallback delay)
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (btn.equals(btn_Copy)) {
            ...
            else file = new File(dialog.getFile());
            ...
            try {
                ...
                printAllDistances(
                        ...
                        ProgressDialog.create(
                                ...
                                "I'm preparing the list of "
                                        + these
                                        + " pairwise distances. Sorry for the delay!",
                                0));
                ...
                MessageBox mb =
                        new MessageBox(
                                ...
                                SpeciesIdentifier.getMessage(
                                        Messages.IOEXCEPTION_WRITING, file, e));
                mb.go();
                return;
            } catch (DelayAbortedException e) {
                return;
    ...
    public String getDescription() {
        return "Summarises the pairwise distances present in this dataset";
    }
