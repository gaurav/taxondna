/**
 * Block analysis is used to study the properties of the entire SequenceSet: how well it could be
 * resolved if somebody needed to use DNA taxonomy.
 *
...
public class BlockAnalysis extends Panel
                ...
                MessageBox mb =
                        new MessageBox(
                                ...
                                "You did not specify a threshold for the all species barcodes!\n\n"
                                    + "Would you like to continue anyway, using a default threshold"
                                    + " of 3%?",
                                MessageBox.MB_YESNO);
    ...
    public void dataChanged() {
        ...
        if (set == null) {
            ...
            text_main.setText(
                    "Please press the 'Calculate' button to conduct an all species barcodes"
                            + " analysis.");
        }
    ...
    public void run() {
        ...
        StringBuffer str_noSpeciesName =
                new StringBuffer("Sequences without an identifiable species name\n");
        ...
        ProgressDialog pd =
                ProgressDialog.create(
                        seqId.getFrame(),
                        "Please wait, doing all species barcodes analysis ...",
                        "The all species barcodes analysis is being performed. Sorry for the wait!",
                        0);
        ...
        while (i.hasNext()) {
            ...
            if (sset.count() > 0) name_first_match = sset.get(1).getSpeciesName();

            if (sset.count() == 0) {
                block_noseq++;
            } else if (sset.get(1).getPairwise(query) > threshold) {
                ...
                for (int y = 2; y < sset.count(); y++) {
                    ...
                    // dataset.
                    if (match.getSpeciesName() == null) continue;

                    if (!match.getSpeciesName().equals(name_first_match)) break;
                ...
                if (block_size <= 1) {
                    ...
                    if (block_size
                            == sd.getSpeciesDetailsByName(name_first_match)
                                    .getSequencesWithValidConspecificsCount()) {
        ...
        text_main.setText(
                "Sequences:\t"
                        + count_sequences
                        + "\nSequences without valid species names:\t"
                        + count_seq_without_species_name
                        + "\nSequences with atleast one sequence with an overlap of "
                        + Sequence.getMinOverlap()
                        + " base pairs:\t"
                        + valid_sequences
                        + "\n\nCorrect identifications according to \"All Species Barcodes\":\t"
                        + block_correct
                        + " ("
                        + percentage(block_correct, valid_sequences)
                        + "%)"
                        + "\nAmbiguous according to \"All Species Barcodes\":\t"
                        + block_ambiguous
                        + " ("
                        + percentage(block_ambiguous, valid_sequences)
                        + "%)"
                        + "\nIncorrect identifications according to \"All Species Barcodes\":\t"
                        + block_incorrect
                        + " ("
                        + percentage(block_incorrect, valid_sequences)
                        + "%)"
                        + "\nSequences with no match closer than "
                        + percentage(threshold, 1)
                        + "%:\t"
                        + block_nomatch
                        + " ("
                        + percentage(block_nomatch, valid_sequences)
                        + "%)"
                        + "\n\n"
    ...
    public String getShortName() {
        return "All Species Barcodes";
    }

    public String getDescription() {
        return "Determines which sequences can be successfully used as barcodes";
    }
