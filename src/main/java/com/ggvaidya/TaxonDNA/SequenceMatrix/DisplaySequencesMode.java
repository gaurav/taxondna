...
public class DisplaySequencesMode extends DisplayMode implements ItemListener {
   ...
   *
   * <p>if(name1.equalsIgnoreCase(outgroupName)) return -1; if(name2.equalsIgnoreCase(outgroupName))
   * return +1;
   *
   * <p>return 0; }
   */
  ...
  /**
   * A sort Comparator which sorts a collection of Strings in natural order - except that outgroups
  ...
  /**
   * A sort Comparator which sorts a collection of Strings (really taxon names) by the total actual
   * count of bases.
   */
  ...
  //
  /**
   * We need to know the SequenceMatrix we're serving, so that we can talk to the user. All else is
