...
public class SequenceMatrix
  ...
  //
  /**
   * Creates a new SequenceMatrix object. We create a new toplevel. Be warned that we do NOT spawn a
   * thread: if TaxonDNA wants to create a SequenceMatrix for whatever reason, it will have to do
   * this IN a thread - otherwise when TaxonDNA exits, we will automatically exit as well.
   *
  ...
  /**
   * Returns the DataModel. Now, in Reality, this is just the dataStore object, suitable casted. But
   * since it's an abstraction, I figured, hey, why not.
   */
  ...
  /**
   * Resets the title of the main frame. This is from the old SpeciesIdentifier code, but it doesn't
   * actually do anything much any more. Maybe we'll eventually tie this into FileManager or
   * DataStore and put up the number of charsets or something, but for now, we really don't need
   * that. At all.
   */
  ...
  private MenuBar createMenuBar() {
    ...
    export.add(
        new MenuItem(
            "Export dataset in interleaved Nexus format (1000 bp)", new MenuShortcut(KeyEvent.VK_N)));
