/**
 * Finds particular distances. Ideal for looking for all zero percent distances, but we can really
 * find any arbitrary distance you might want to look for.
 */
...
public class FindDistances implements WindowListener, ActionListener {
  ...
  public FindDistances(SequenceMatrix matrix) {
    ...
    // set up the 'buttons' bar ... which is 'Copy to clipboard' and 'Export as text', our old
    // friends
  ...
  public void displayResults() throws DelayAbortedException {
    ...
    try {
      ...
      new MessageBox(
              matrix.getFrame(),
              "What's that number?",
              "I couldn't understand the lower pairwise distance limit, which was '"
                  + tf_percentLow.getText()
                  + "'. I'm going to use 0% instead.")
          .go();
    ...
    try {
      ...
      new MessageBox(
              matrix.getFrame(),
              "What's that number?",
              "I couldn't understand the upper pairwise distance limit, which was '"
                  + tf_percentHigh.getText()
                  + "'. I'm going to use 0% instead.")
          .go();
      ...
      while (i_outer_seq.hasNext()) {
        ...
        while (i_columns.hasNext()) {
          ...
          while (i_inner_seq.hasNext()) {
            ...
            if (seq_inner.equals(seq_outer)) continue;
    ...
    while (i.hasNext()) {
      ...
      buff.append(
          pairdist.getSequenceA().getFullName()
              + "\t"
              + pairdist.getSequenceB().getFullName()
              + "\t"
              + percentage(pairdist.getDistance(), 1)
              + "%\n");
    }
    ...
    text_main.setText(
        count_results
            + " sequence pairs found with distances between "
            + percentage(from, 1)
            + "% and "
            + percentage(to, 1)
            + "%.\nNote that distances were calculated using "
            + pdm_method
            + " and sequence overlaps of less than "
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (text_main.getText().length() > 0) {
      ...
      if (src.equals(btn_Export)) {
        ...
        FileDialog dialog =
            new FileDialog(
                fr_findDistances, "Where would like to save these results?", FileDialog.SAVE);
