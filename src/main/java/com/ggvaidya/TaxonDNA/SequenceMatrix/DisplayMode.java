/**
 * A DisplayMode handles the actual display of content. It does this by being a TableModel, so the
 * JTable can just use this as its backing database. The trick is to make sure that the JTable
 * doesn't realise that the model is coming from three completely different objects.
 *
 * <p>The way we're going to try to play this is like this: every DisplayMode inherits from this
 * class, so they all have a repertoire of functions they don't need to reimplement, etc. They will
 * also have a set of functions that TableManager can use as an interface to talk to them - for
 * instance, something like activateDisplay(JTable) will allow the class to set itself up as the
 * TableModel (and active display), then have a deactivateDisplay() to turn it off or something. Or
 * something.
 */
...
public abstract class DisplayMode implements TableModel, MouseListener {
  ...
  /**
   * Please do put your JTable value here, or put in 'null' if you're doing something funky. There's
   * several functions who expect to find the table here.
   */
  ...
  public int getColumnCount() {
    return sortedColumns.size() // the number of sorted columns
        + additionalColumns; // the number of additional columns
  }

  /**
   * Note that this function will return the *absolute* index (it must, it IS in the interface). So
   * do remember that when you use this function, alright children?
   */
  public String getColumnName(int columnIndex) {
    if (columnIndex < additionalColumns) return (String) getAdditionalColumns().get(columnIndex);
  ...
  public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
    if (columnIndex == 0) tableManager.renameSequence(getRowName(rowIndex), (String) aValue);
    else setValueAt(getColumnName(columnIndex), getRowName(rowIndex), aValue);
  }
  ...
  /**
   * Saves the column widths into a Hashtable. You can use restoreWidths() to restore your widths to
   * where they were a while ago.
   */
  ...
  /** Restores the column widths saved into the Hashtable as the column widths on the JTable. */
  protected void restoreWidths(Hashtable widths) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (oldWidth != null) tc.setPreferredWidth(oldWidth.intValue());
    }
  }
  ...
  //
  /**
   * Adds a new TableModelListener. Since much funkiness could (and probably will!) happen with
   * TableModels being set repeatedly, it makes sense to check that we're not double adding a single
   * listener.
   */
  public void addTableModelListener(TableModelListener l) {
    if (!tableModelListeners.contains(l)) tableModelListeners.add(l);
  }
  ...
  /**
   * Update the display (generally by firing an event at all the tableModelListeners). Feel free to
   * overload this if you think you've got a better idea - but PLEASE remember to save and reload
   * the table headers before you do!
   */
  ...
  /**
   * You can set a menu to be displayed in the main menubar when this display mode is selected.
   * Don't worry about the details: the tableManager will query this either just after or just
   * before activating the display mode. You are responsible for adding any actionListeners which
   * need adding. If you return 'null', no menu will be created or anything.
   *
   * <p>Temporary note: Note that we expect a MENU, with a NAME. This behavior might change in the
   * future. Apologies.
   */
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (e.getSource().equals(table.getTableHeader()) || e.getSource().equals(table)) {
      ...
      if (e.getSource().equals(table.getTableHeader())) popup = true;

      // check for right clicks
      if (popup) rightClick(e, colIndex, rowIndex);

      // check for double clicks (but not both!)
      else if (e.getClickCount() == 2) doubleClick(e, colIndex, rowIndex);
    }
  }
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    //
    if (e.isPopupTrigger()) mouseClicked(e);
  }
}
