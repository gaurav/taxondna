/**
 * Preferences is a common store for preferences. 'Preferences' preferences are stored into the
 * java.util.prefs store, and are thus persistant across sessions (heh heh - ain't I the software
 * engineer!). Everybody can check with us through a generic preferences system
 * (getPreference/setPreference), as well as to check some of the variables we ourselves maintain.
 *
 * <p>Some behaviour (and the underlying behaviour) we control ourselves; this includes: 1. The
 * *first* time a file is loaded, we pop up a dialog to check whether the user wants to use species
 * names or sequence names. We then save this setting for the entire session, but do NOT save it to
 * the prefs store; so it needs to be set once a session.
 *
 * <p>I suppose ideally, we ought to reset it when the all columns are cleared, but that's kind of
 * complicated. But we'll think about it in the future!
 */
...
public class Preferences implements ActionListener {
  ...
  /**
   * Returns the preference specified (as int). You *have* to specify a min and max so we don't have
   * a problem with returning an out-of-range value when the program's defaults change.
   *
   * @param def default value for this key
  ...
  public int getUseWhichName() {
    if (prefName == PREF_NOT_SET_YET) {
      Dialog dg = new Dialog(matrix.getFrame(), "Species names or sequence names?", true); // modal!
      dg.setLayout(new BorderLayout());

      TextArea ta = new TextArea("", 4, 40, TextArea.SCROLLBARS_VERTICAL_ONLY);
      ta.setEditable(false);
      ta.setText(
          "Would you like to use the full sequence name? I could also try to determine the species name from the sequence name, and use that instead.");
      dg.add(ta);
