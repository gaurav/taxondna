/**
 * The TableManager manages the JTable and all related user operations. This includes left clicks,
 * right clicks, selects, jumps, whatever you could possible think of. Our purpose in life is to
 * abstract away all mid-level UI operations. Basically, we want SequenceMatrix to be able to say
 * "tableManager.tableClicked(1, 24)", and dataStore to be able to say "tableManager.repaint()", and
 * just leave it at that.
 *
 * <p>The most complicated thing we have right now is the fact that the Display systems have broken
 * off from the DataStore - a good thing, but, unfortunately, there's TWO of them. The TableManager
 * is one of the things we're explicitly creating to try and work out the issues.
 */
...
public class TableManager implements ActionListener {
    ...
    private JTextField tf_statusBar =
            new JTextField(); // the statusbar of the window we're managing
    ...
    // We need to track the DisplayMode: which one is currently working, which modes do we have,
    // etc.
    ...
    //
    /**
     * Return the current reference sequence. If the user has not explicitly set a reference
     * sequence, we will return null.
     */
    ...
    //
    /**
     * Resizes ALL columns to fit their widest entry. By default, resizeColumns() will ONLY widen
     * columns, not shrink them. This method can therefore be called with minimum user disturbance
     * (I think).
     */
    ...
    /**
     * Resizes column 'x' to fit the widest entry, allowing shrinking to occur. Java, meet default
     * arguments. Default arguments, meet Java.
     */
    ...
    /**
     * Resizes column 'x' to fit the widest entry, WITHOUT allowing shrinking to happen. Java, meet
     * default arguments. Default arguments, Java.
     */
    ...
    public void resizeColumnToFit(String x, boolean shrinkAllowed) {
        ...
        if (tcm == null) return; // it could happen

        int col = sortedColumns.indexOf(x);
        if (col == -1) return; // it could happen

        TableColumn tc = tcm.getColumn(col);
        if (tc == null) return; // it could happen
        ...
        Component comp =
                r.getTableCellRendererComponent(table, tc.getHeaderValue(), true, true, -1, col);
        if (comp != null) maxLength = (int) comp.getPreferredSize().getWidth();
        ...
        for (int row = 0; row < sortedSequences.size(); row++) {
            ...
            Component c =
                    renderer.getTableCellRendererComponent(
                            table, table.getValueAt(row, col), true, true, row, col);

            // get the length
            int length = -1;
            if (c != null) length = (int) c.getPreferredSize().getWidth();
        ...
        if (!shrinkAllowed) {
            if (maxLength < tc.getPreferredWidth()) return;
        }

        // if we have a maximum length, set it
        if (maxLength > 0) tc.setPreferredWidth(maxLength + 10); // The '10' is for the margins
    }
    ...
    // 	see what it says.
    //
    /** Clears all entries from the underlying datastore, and cleans up the JTable. */
    public void clear() {
    ...
    /**
     * Note that unlike getCancelledSequence(), getSequence() will return 'null' when (colName,
     * seqName) is cancelled. Just a reminder, like.
     */
    ...
    public void deleteColumn(String colName) {
        if (warned(
                "Delete this column?",
                "Are you sure you wish to delete column '" + colName + "'?")) {
    ...
    public void deleteRow(String taxonName) {
        if (warned(
                "Delete this taxon?",
                "Are you sure you wish to delete the taxon '" + taxonName + "'?")) {
    ...
    public java.util.List getSequenceNamesByColumn(String colName) {
        ...
        while (i.hasNext()) {
            ...
            if (dataStore.getCancelledSequence(colName, seqName) != null) v.add(colName);
        }
            ...
            MessageBox mb =
                    new MessageBox(
                            ...
                            "Renaming '"
                                    ...
                                    + "' to '"
                                    + newName
                                    + "' will cause the following problems. Are you sure you want"
                                    + " to go ahead with this rename?\n"
                                    + complaints,
                            MessageBox.MB_YESNO);

            if (mb.showMessageBox() == MessageBox.MB_NO) return;
        }
    ...
    //
    /** changeDisplayMode() on a diet: one argument only! Obviously, the mode to switch to. */
    public void changeDisplayMode(int mode) {
        changeDisplayMode(mode, null);
    }

    /**
     * Easily the ugliest function EVER, changeDisplayMode() will change the display mode to the
     * mode specified by mode (which is one of the DISPLAY_* constants), with an optional argument
     * (which is passed straight on to the display mode object.
     */
    ...
    /** I'm sorry, I couldn't resist. Never 'new' when you can reload, right? */
    ...
    public boolean warned(String title, String msg) {
        ...
        if (mb.showMessageBox() == MessageBox.MB_YES) return true;
    ...
    public void updateStatusBar() {
        ...
        if (charsets == 0) {
            ...
            if (cancelled > 0) buff.append(", " + cancelled + " cancelled sequences");
            // TODO: Specify currently selected set:taxa here.
            buff.append('.');
        }

        buff.append(' ');
        if (currentDisplayMode != null) currentDisplayMode.setStatusBar(buff);
    ...
    public void defaultDoubleClick(MouseEvent e, int col, int row) {
        if (row >= 0 && col != -1 && col >= currentDisplayMode.additionalColumns) {
            ...
            toggleCancelled(
                    currentDisplayMode.getColumnName(col), currentDisplayMode.getRowName(row));
        }
    }
    ...
    /** Selects the specified sequence. Warning: the legitimacy of this function is disputed. */
    public void selectSequence(String colName, String seqName) {
        ...
        if (toolbarManager != null) toolbarManager.setToolbarStatus(colName, seqName);
    }

    /**
     * Returns the current datastore. DO NOT MODIFY THE DATASTORE DIRECTLY. If you want to edit the
     * DataStore, you really really really ought to work through TableManager, and not on the DS
     * directly. This is just here for hack's sake.
     */
 ...
 * Manages the Toolbar.
 *
 * <p>TODO: Wire this into the Table's selection interfaces rather than into the MouseListener.
 */
class ToolbarManager implements ActionListener {
        ...
        table.addMouseListener(
                new MouseAdapter() {
                    public void mouseClicked(MouseEvent e) {
                        ...
                        if (colIndex == -1 || rowIndex == -1) return; // can't deal with this!
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (cmd.equals("Delete column")) {
            if (currentCharsetName != null) tm.deleteColumn(currentCharsetName);
        }
        ...
        if (cmd.equals("Delete taxon")) {
            if (currentSeqName != null) tm.deleteRow(currentSeqName);

            // TODO: Use table.changeSelection() to move to the next row
        }

        if (cmd.equals("Excise entire taxon")) {
            if (currentSeqName != null) tm.cancelRow(currentSeqName);
        }

        if (cmd.equals("Make outgroup")) {
            if (currentSeqName != null) tm.setReferenceSequence(currentSeqName);
        }
