/**
 * This module manages the files for SequenceMatrix. Since each column is really a SequenceList
 * file, we keep track of source, handler, and so on for everybody. We also handle exports.
 *
 * <p>It's most important call-to-fame is the asynchronous file loader, handled by spawning a thread
 * which loading the file up. We ensure that: 1. Two files are never loaded simultaneously. 2. The
 * interface is locked up while files are being loaded.
 *
 * <p>Note that after commit e13ef06a09be this file was cleaned up for pretty much the first time
 * ever. This is directly to make it possible to make charactersets and codonpositions to work
 * together in sensible ways; but this might well broken things in horrible, horrible ways. Let's
 * see.
 */
...
public class FileManager implements FormatListener {
    ...
    private final HashMap<String, ArrayList<FromToPair>> hashmap_codonsets =
            new HashMap<String, ArrayList<FromToPair>>();
    //
    ...
    //
    /**
     * Creates a FileManager. We need to know where the SequenceMatrix is, so we can talk to the
     * user.
     */
    ...
    /**
     * Report an IOException to the user. Since IOExceptions are so common in our code, this is a
     * real time saver.
     */
        ...
        MessageBox mb =
                new MessageBox(
                        ...
                        "There was an error while "
                                ...
                                + " '"
                                + f
                                + "'. Please ensure that you have adequate permissions and that"
                                + " your hard disk is not full.\n\n"
                                + "The technical description of this error is: "
                                + e);
    ...
    private void reportDelayAbortedException(DelayAbortedException e, String task) {
        MessageBox mb =
                new MessageBox(
                        ...
                        task + " cancelled",
                        task
                                + " was cancelled midway. The task might be incomplete, and any"
                                + " files being generated might have been partially generated.");

        mb.go();
    }

    /**
     * A dialog box to check whether the user would like to use sequence names or species names
     * during file import.
     *
    ...
    public int checkNameToUse(String str_sequence_name, String str_species_name) {
        ...
        Dialog dg =
                new Dialog(
                        matrix.getFrame(), "Species names or sequence names?", true // modal!
                        );
        ...
        ta.setText(
                "Would you like to use the full sequence name? Sequence names from this file look"
                        + " like this:\n"
                        + "\t"
                        + str_sequence_name
    ...
    /**
     * Checks whether the user would like to convert all external gaps to question marks, and - if
     * so - does it.
     */
    ...
    /**
     * Sets up names to use by filling in INITIAL_SEQNAME_PROPERTY in the Sequence preferences,
     * in-place. You ought to call this on a SequenceList before you process it. Note that this
     * function works IN PLACE, but won't change names or anything - only replace the
     * INITIAL_SEQNAME_PROPERTY with what we think it we think is the right initial seq name NOW.
     *
     * <p>Important note: if the INITIAL_SEQNAME_PROPERTY for a sequence has already been set, we
     * skip processing that sequence altogether. The idea is that if the input file format already
     * knows what name the user likes, we can go ahead and use that.
     */
        ...
        while (i.hasNext()) {
            ...
            if (sequence_name.equals(species_name)) {
                ...
                while (pref_useWhichName == PREF_NOT_SET_YET) {
                    checkNameToUse(sequence_name, species_name);
                }
    ...
    /**
     * Load the specified file with the provided FormatHandler (or null) and returns the loaded
     * SequenceList (or null, if there was an error). This is really part of the addFile() system,
     * so it will provide its own errors to the user if things go wrong.
     *
    ...
    private SequenceList loadFile(File file, FormatHandler handler) {
        ...
        try {
                        ...
                        new SequenceList(
                                ...
                                ProgressDialog.create(
                                        ...
                                        "Loading '"
                                                ...
                                                + ") into memory. Sorry for the delay!",
                                        ProgressDialog.FLAG_NOCANCEL));

            } else {
                list =
                        SequenceList.readFile(
                                file,
                                ProgressDialog.create(
                                        matrix.getFrame(),
                                        "Loading file ...",
            ...
            MessageBox mb =
                    new MessageBox(
                            matrix.getFrame(),
                            "Could not read file!",
                            "I could not understand a sequence in this file. Please fix any errors"
                                    + " in the file.\n\n"
                                    + "The technical description of the error is: "
                                    + e);
        ...
        if (handler == null) handler = list.getFormatHandler();
    ...
    /**
     * A helper method to remap global-coordinates (relative to the entire sequence) to
     * sequence-only coordinates (relative to this sequence). This requires much mathematical
     * thinking which is helpfully provided by this method.
     *
     * <p>One caveat: we assume ALL from-tos where 'goesInThirds' is true goes in thirds, which
     * means we need to do some additional calculations to make sense. This is a little presumptive,
     * but we'll do it anyway.
     */
    private Vector<FromToPair> positionalInformationFor(
            ArrayList<FromToPair> positionData,
            int index_in_sequence,
            int from,
            int to,
            boolean goesInThirds) {
        ...
        for (FromToPair to_check : positionData) {
            ...
            if (to_check.to >= from && to_check.from <= to) {
                ...
                // System.err.println("Match while comparing [" + from + " to " + to + "] against
                // to_check
                // [" + to_check.from + " to " + to_check.to + "]");

                // System.err.println("\tOffset is wrong; expected 1, obtained " + offset + " by
                // subtracting
                // " + from + " from " + to_check.from + "\n");

                // System.err.println("\tWe obtain a " + (goesInThirds ? "goes in thirds" : "goes
                // sequentially" ) + " sequence from " + start_at + " to " + end_at + " from " +
                // from + " to
                // " + to + " comparing against " + to_check.from + " to " + to_check.to + ".");
    ...
    /**
     * Reads the sets stored in hashmap_codonsets and splits the datasets into individual
     * sequencelists for each non-overlapping character sets.
     *
     * <p>(2009-Nov-28) I want to see codonposset information to be read into the file, and I'd love
     * to see it recognize overlapping datasets. Will I live to see my dreams realized? Only time
     * will tell.
     *
     * <p>(2009-Nov-28) These comments are in the code. I'm not sure if they're still relevant, but
     * being dire warnings of the downfall of all we hold dear, I suppose we should atleast keep it
     * lying around.
     *
     * <p>///////////////////////////////////////////////////////////////////////////////// // TODO:
     * This code is busted. Since we're not sure what ORDER the messages come // in (or that they
     * get entered into the vectors), we need to make sure we SORT // the vectors before we start
     * anything funky. Of course, we can't sort them as // is, which means another layer of hacks.
     * // TODO. /////////////////////////////////////////////////////////////////////////////////
     *
     * <p>// do stuff // // this is how it works: // 1. we will NEVER let anybody else see
     * 'sequences' (our main SequenceList) // 2. we split sequences into subsequencelists. These, we
     * will let people see. // 3. any characters remaining 'unmatched' are thrown into 'no sets' //
     * 4. we split everything :(. this is the best solution, but, err, that // means we have to
     * figure out a delete column. Sigh. //
     *
     * @return -1 if there was a fatal error (which has already been shown to the user), otherwise
     *     the number of character sets which have been successfully extracted.
     */
    private int incorporateSets(File file, SequenceList sequences) {
        ...
        synchronized (hashmap_codonsets) {
            ...
            while (i_sets.hasNext()) {
                ...
                // If the dire comment above is correct, then this next bit is likely quite
                // important.
                ...
                while (i_seq.hasNext()) {
                    ...
                    try {
                        if (pd != null) pd.delay(count, total);
                    } catch (DelayAbortedException e) {
                    ...
                    while (i_coords.hasNext()) {
                        ...
                        try {
                            // System.err.println("Cutting [" + name + "] " + seq.getFullName() + "
                            // from " + from
                            // + " to " + to + ": " + seq.getSubsequence(from, to) + ";");
                            ...
                            seq_positions_N.addAll(
                                    positionalInformationFor(
                                            positions_N,
                                            index_assembled_sequence,
                                            from,
                                            to,
                                            false));
                            seq_positions_1.addAll(
                                    positionalInformationFor(
                                            positions_1, index_assembled_sequence, from, to, true));
                            seq_positions_2.addAll(
                                    positionalInformationFor(
                                            positions_2, index_assembled_sequence, from, to, true));
                            seq_positions_3.addAll(
                                    positionalInformationFor(
                                            positions_3, index_assembled_sequence, from, to, true));
                            ...
                            MessageBox mb_2 =
                                    new MessageBox(
                                            ...
                                            "According to this file, character set "
                                                    + charset_name
                                                    + " extends from "
                                                    + from
                                                    + " to "
                            ...
                            new MessageBox(
                                            ...
                                            "A fatal internal error occured while processing "
                                                    + charset_name
                                                    + ", extending from "
                                                    + from
                                                    + " to "
    ...
    private void addSequenceListToTable(String name, SequenceList sl) {
        ...
        if (buff_complaints.length() > 0) {
            new MessageBox(
                            ...
                            "Some sequences in the taxonset "
                                    + name
                                    + " weren't added. These are:\n"
    ...
    /**
     * Adds a new file to the table. All file loading for SequenceMatrix goes through here. This
     * method should mostly coordinate the other methods around to get its work done.
     *
     * <p>There used to be dragons here, but they've moved out into other methods now.
     *
     * <p>This method throws no exceptions; any errors are displayed to the user directly.
     */
            ...
            if (no_of_sets > 0) {
                MessageBox mb =
                        new MessageBox(
                                ...
                                "The file "
                                        + file
                                        + " contains character sets. Do you want me to split the"
                                        + " file into character sets?",
                                MessageBox.MB_YESNOTOALL | MessageBox.MB_TITLE_IS_UNIQUE);
    ...
    /**
     * Tries to load a particular file into the present SequenceList. If successful, it will
     * sequences the present file to be whatever was specified. If not successful, it will leave the
     * external situation (file, sequences) unchanged.
     */
    ...
    public void addFile() {
        FileDialog fd =
                new FileDialog(
                        matrix.getFrame(), "Which file would you like to open?", FileDialog.LOAD);
        fd.setVisible(true);
     ...
     *
     * <p>Right now we're outsourcing this to the SequenceGrid; we might bring it in here in the
     * future.
     */
    public void exportAsNexus() {
        ...
        Dialog frame =
                (Dialog)
                        CloseableWindow.wrap(
                                new Dialog(matrix.getFrame(), "Export as Nexus ...", true));
        RightLayout rl = new RightLayout(frame);
        frame.setLayout(rl);

        rl.add(new Label("Save as:"), RightLayout.NONE);
        FileInputPanel finp = new FileInputPanel(null, FileInputPanel.MODE_FILE_WRITE, frame);
        ...
        if (btn.wasClicked() && ((file = finp.getFile()) != null)) {
            matrix.getPrefs()
                    .setPreference("exportAsNexus_exportAs", choice_exportAs.getSelectedIndex());
            ...
            try {
                ...
                if (new MessageBox(
                                        ...
                                        "I can't figure out which number you mean by '"
                                                + tf_interleaveAt.getText()
                                                + "'. I'm going to use '1000' as a default length."
            ...
            try {
                matrix.getExporter()
                        .exportAsNexus(
                                ...
                                ProgressDialog.create(
                                        ...
                                        "All your sequences are being exported as a single Nexus"
                                                + " file into '"
                                                + file
                                                + "'. Sorry for the wait!"));
            } catch (IOException e) {
                MessageBox mb =
                        new MessageBox(
                                ...
                                "The following error occured while writing sequences to file: "
                                        + e);
                mb.go();
            ...
            new MessageBox(
                            matrix.getFrame(),
                            "All done!",
                            "All your sequences were exported into '" + file + "' as a Nexus file.")
                    .go();
        }
    }
    ...
    public void quickExportAsNexus() {
        if (!checkCancelledBeforeExport()) return;
            ...
            matrix.getExporter()
                    .exportAsNexus(
                            ...
                            ProgressDialog.create(
                                    matrix.getFrame(),
                                    "Please wait, exporting sequences ...",
                                    "All your sequences are being exported as a single Nexus file"
                                            + " into '"
                                            + file
                                            + "'. Sorry for the wait!"));
        } catch (IOException e) {
    ...
    public void quickExportAsPhylip() {
            ...
            matrix.getExporter()
                    .exportAsPhylip(
                            ...
                            ProgressDialog.create(
                                    matrix.getFrame(),
                                    "Please wait, exporting sequences ...",
                                    "All your sequences are being exported for RAxML analyses as '"
                                            + file
                                            + "'. Sorry for the wait!"));
        } catch (IOException e) {
            MessageBox mb =
        ...
        new MessageBox(
                        matrix.getFrame(),
                        "All done!",
                        "All your sequences were exported into '" + file + "' as a Phylip file.")
                .go();
    }
    ...
    public void quickExportAsNexusNonInterleaved() {
            ...
            matrix.getExporter()
                    .exportAsNexus(
                            ...
                            ProgressDialog.create(
                                    matrix.getFrame(),
                                    "Please wait, exporting sequences ...",
                                    "All your sequences are being exported as a single,"
                                            + " non-interleaved Nexus file into '"
                                            + file
                                            + "'. Sorry for the wait!"));
        } catch (IOException e) {
            MessageBox mb =
        ...
        new MessageBox(
                        matrix.getFrame(),
                        "All done!",
                        "All your sequences were exported into '"
                                + file
                                + "' as a non-interleaved Nexus file.")
                .go();
    }
    ...
    public void quickExportAsNakedNexus() {
        ...
        try {
            matrix.getExporter()
                    .exportAsNexus(
                            file,
                            Preferences.PREF_NEXUS_SINGLE_LINE
                                    | Preferences.PREF_NEXUS_NAKED_FORMAT,
                            100,
                            ProgressDialog.create(
                                    matrix.getFrame(),
                                    "Please wait, exporting sequences ...",
                                    "All your sequences are being exported as a naked Nexus file"
                                            + " into '"
                                            + file
                                            + "'. Sorry for the wait!"));
        } catch (IOException e) {
        ...
        new MessageBox(
                        ...
                        "All your sequences were exported into '"
                                + file
                                + "' as a naked Nexus file.")
                .go();
    }

    /**
     * Export the current set as a TNT file. Be warned that File 'f' will be completely overwritten.
     *
     * <p>Right now we're outsourcing this to the SequenceGrid; we might bring it in here in the
     * future.
     */
        ...
        try {
            ...
            MessageBox mb =
                    new MessageBox(
                            matrix.getFrame(),
                            "Success!",
                            "This set was successfully exported to '"
                                    + f
                                    + "' in the Sequences format.");
            mb.go();
    ...
    public void exportSequencesByColumnsInGroups() {
        ...
        Dialog dg =
                (Dialog)
                        CloseableWindow.wrap(
                                new Dialog(
                                        matrix.getFrame(),
                                        "Please choose your output options ...",
                                        true));
        RightLayout rl = new RightLayout(dg);
        dg.setLayout(rl);

        rl.add(new Label("Write all the files into:"), RightLayout.NONE);
        DirectoryInputPanel dinp =
                new DirectoryInputPanel(null, DirectoryInputPanel.MODE_DIR_SELECT, dg);
        ...
        Checkbox check_writeNASequences =
                new Checkbox("Write 'N/A' sequences into the files as well");
        ...
        int taxaCount =
                matrix.getTableManager()
                        .getSequenceNames()
                        .size(); // this sort of statement should not be allowed
        ...
        choice_formats.select(
                matrix.getPrefs()
                        .getPreference(
                                "exportSequencesByColumnsInGroups_choice",
                                2,
                                0,
                                choice_formats.getItemCount()
                                        - 1)); // 2 == NexusFile, at some point of time
        dinp.setFile(
                new File(
                        matrix.getPrefs()
                                .getPreference("exportSequencesByColumnsInGroups_fileName", "")));
        if (matrix.getPrefs()
                        .getPreference("exportSequencesByColumnsInGroups_writeNASequences", 0, 0, 1)
                == 0) {
        ...
        tf_rands.setText(
                matrix.getPrefs()
                        .getPreference("exportSequencesByColumnsInGroups_noOfRands", "10"));
        choice_per_group.select(
                ""
                        + (matrix.getPrefs()
                                .getPreference(
                                        "exportSequencesByColumnsInGroups_choicePerGroup",
                                        0,
                                        0,
                                        choice_per_group.getItemCount() - 1)));
        choice_random_taxa.select(
                ""
                        + (matrix.getPrefs()
                                .getPreference(
                                        "exportSequencesByColumnsInGroups_choiceRandomTaxa",
                                        0,
                                        0,
                                        choice_random_taxa.getItemCount() - 1)));
        int index_sel =
                list_seqNames.indexOf(
                        matrix.getPrefs()
                                .getPreference(
                                        "exportSequencesByColumnsInGroups_choiceRefTaxon", "None"));
        ...
        if (dinp.getFile() == null) return;

        if (btn.wasClicked()) {
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_choice",
                            choice_formats.getSelectedIndex());
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_fileName",
                            dinp.getFile().getParent());
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_writeNASequences",
                            check_writeNASequences.getState() ? 1 : 0);
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_choicePerGroup",
            ...
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_choiceRefTaxon",
                            choice_ref_taxon.getSelectedItem());
            ...
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumnsInGroups_noOfRands", String.valueOf(rands));
                ...
                matrix.getExporter()
                        .exportColumnsInGroups(
                                ...
                                ProgressDialog.create(
                                        matrix.getFrame(),
                                        "Please wait, exporting sequences ...",
                                        "All your sequences are being exported as individual files"
                                                + " into '"
                                                + dinp.getFile()
                                                + "'. Please wait!"));
            } catch (IOException e) {
            ...
            new MessageBox(
                            matrix.getFrame(),
                            "All done!",
                            "All your sequences were exported into individual files in '"
                                    + dinp.getFile()
                                    + "'.")
                    .go();
    ...
    public void exportSequencesByColumn() {
        ...
        choice_formats.select(
                matrix.getPrefs()
                        .getPreference(
                                "exportSequencesByColumn_choice",
                                0,
                                0,
                                choice_formats.getItemCount() - 1));
        dinp.setFile(
                new File(matrix.getPrefs().getPreference("exportSequencesByColumn_fileName", "")));
        if (matrix.getPrefs().getPreference("exportSequencesByColumn_writeNASequences", 0, 0, 1)
                == 0) {
            check_writeNASequences.setState(false);
        ...
        if (btn.wasClicked()) {
            ...
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumn_choice", choice_formats.getSelectedIndex());
            matrix.getPrefs()
                    .setPreference("exportSequencesByColumn_fileName", dinp.getFile().toString());
            matrix.getPrefs()
                    .setPreference(
                            "exportSequencesByColumn_writeNASequences",
                            check_writeNASequences.getState() ? 1 : 0);

            // phew ... go!
            try {
                matrix.getExporter()
                        .exportSequencesByColumn(
                                dinp.getFile(),
                                (FormatHandler) fhs.get(choice_formats.getSelectedIndex()),
                                check_writeNASequences.getState(),
                                ProgressDialog.create(
    ...
    /**
     * A format Listener for listening in on character sets. Every time the FormatHandler sees a
     * characterset it calls this method, which then stores the characterset into hashmap_codonsets.
     * Once the file has finished loading, incorporateSets() will use this data to subdivide the
     * input file.
     */
                ...
                synchronized (hashmap_codonsets) {
                    ...
                    if (name.startsWith(":") && !name.equals(":0")) {
                        ...
                        for (String compare_to : hashmap_codonsets.keySet()) {
                            ...
                            if (!compare_to.startsWith(":") || compare_to.equals(":0")) continue;

                            ArrayList<FromToPair> compare_list = hashmap_codonsets.get(compare_to);
                            for (FromToPair compare_ftp : compare_list) {
                                // System.err.println("Comparing position " + name + ":" + ftp + "
                                // with position " +
                                // compare_to + ":" + compare_ftp + " -> " +
                                // ftp.overlapsMovesInThrees(compare_ftp));
                                if (compare_ftp.overlapsMovesInThrees(ftp)) {
                                    String pos = name.substring(1); // Eliminate the leading ':'.
                                    String compare_pos =
                                            compare_to.substring(1); // Eliminate the leading ':'.

                                    throw new FormatException(
                                            "Sequence Matrix cannot handle overlapping character"
                                                + " sets, including in codon positional data. In"
                                                + " this file, position #"
                                                    + pos
                                                    + " ("
                                                    + ftp
                                                    ...
                                                    + compare_ftp
                                                    + ").\n\n"
                                                    + "Please rectify this in the input file, or"
                                                    + " report a bug if there is no overlap between"
                                                    + " these two codon positions.");
                                }
                            }
                        }
                        ...
                        for (String compare_to : hashmap_codonsets.keySet()) {
                            ...
                            if (compare_to.startsWith(":")) continue;

                            ArrayList<FromToPair> compare_list = hashmap_codonsets.get(compare_to);
                            for (FromToPair compare_ftp : compare_list) {
                                // System.err.println("Comparing " + name + ":" + ftp + " with " +
                                // compare_to + ":"
                                // + compare_ftp + " -> " + ftp.overlaps(compare_ftp));
                                if (compare_ftp.overlaps(ftp)) {
                                    throw new FormatException(
                                            "Sequence Matrix cannot handle overlapping datasets. In"
                                                    + " this file, character set "
                                                    + name
                                                    + " ("
                                                    + ftp
                                                    + ") overlaps with "
    ...
    /**
     * Check to see whether any cancelled sequences exist; any data in such sequences will be
     * completely LOST on export.
     */
    public boolean checkCancelledBeforeExport() {
        ...
        MessageBox mb =
                new MessageBox(
                        ...
                        "You have "
                                + cancelled
                                + " cancelled sequence(s) in your dataset. These sequences will not"
                                + " be exported! Are you sure you are okay with losing the data in"
                                + " these sequences?",
