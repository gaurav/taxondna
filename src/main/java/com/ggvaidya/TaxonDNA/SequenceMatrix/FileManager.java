...
public class FileManager implements FormatListener {
  ...
  //
  /**
   * Creates a FileManager. We need to know where the SequenceMatrix is, so we can talk to the user.
   */
  ...
  /**
   * Report an IOException to the user. Since IOExceptions are so common in our code, this is a real
   * time saver.
   */
  ...
  private void reportDelayAbortedException(DelayAbortedException e, String task) {
    MessageBox mb =
        new MessageBox(
            ...
            task + " cancelled",
            task
                + " was cancelled midway. The task might be incomplete, and any files being generated might have been partially generated.");

    mb.go();
  }

  /**
   * A dialog box to check whether the user would like to use sequence names or species names during
   * file import.
   *
  ...
  /**
   * Checks whether the user would like to convert all external gaps to question marks, and - if so
   * - does it.
   */
   ...
   *
   * <p>Important note: if the INITIAL_SEQNAME_PROPERTY for a sequence has already been set, we skip
   * processing that sequence altogether. The idea is that if the input file format already knows
   * what name the user likes, we can go ahead and use that.
   */
   ...
   * SequenceList (or null, if there was an error). This is really part of the addFile() system, so
   * it will provide its own errors to the user if things go wrong.
   *
    ...
    if (handler.getShortName().equals("NEXUS")) {
      ...
      if (!f_displayed_nexus_warning) {
        MessageBox.messageBox(
            ...
            "SequenceMatrix only parses the data and the character set " + "and codon set blocks.",
            MessageBox.MB_OK);
   ...
   *
   * <p>One caveat: we assume ALL from-tos where 'goesInThirds' is true goes in thirds, which means
   * we need to do some additional calculations to make sense. This is a little presumptive, but
   * we'll do it anyway.
   */
  private Vector<FromToPair> positionalInformationFor(
    ...
    for (FromToPair to_check : positionData) {
      ...
      if (to_check.to >= from && to_check.from <= to) {
        ...
        // System.err.println("Match while comparing [" + from + " to " + to + "] against to_check
        // [" + to_check.from + " to " + to_check.to + "]");

        // System.err.println("\tOffset is wrong; expected 1, obtained " + offset + " by subtracting
        // " + from + " from " + to_check.from + "\n");

        // System.err.println("\tWe obtain a " + (goesInThirds ? "goes in thirds" : "goes
        // sequentially" ) + " sequence from " + start_at + " to " + end_at + " from " + from + " to
   ...
   * to see it recognize overlapping datasets. Will I live to see my dreams realized? Only time will
   * tell.
   *
   ...
   *
   * <p>///////////////////////////////////////////////////////////////////////////////// // TODO:
   * This code is busted. Since we're not sure what ORDER the messages come // in (or that they get
   * entered into the vectors), we need to make sure we SORT // the vectors before we start anything
   * funky. Of course, we can't sort them as // is, which means another layer of hacks. // TODO.
   * /////////////////////////////////////////////////////////////////////////////////
   *
   * <p>// do stuff // // this is how it works: // 1. we will NEVER let anybody else see 'sequences'
   * (our main SequenceList) // 2. we split sequences into subsequencelists. These, we will let
   * people see. // 3. any characters remaining 'unmatched' are thrown into 'no sets' // 4. we split
   * everything :(. this is the best solution, but, err, that // means we have to figure out a
   * delete column. Sigh. //
   *
   * @return -1 if there was a fatal error (which has already been shown to the user), otherwise the
   *     number of character sets which have been successfully extracted.
   */
      ...
      while (i_sets.hasNext()) {
        ...
        while (i_seq.hasNext()) {
          ...
          while (i_coords.hasNext()) {
            ...
            try {
              // System.err.println("Cutting [" + name + "] " + seq.getFullName() + " from " + from
              ...
              seq_positions_N.addAll(
                  positionalInformationFor(positions_N, index_assembled_sequence, from, to, false));
              seq_positions_1.addAll(
                  positionalInformationFor(positions_1, index_assembled_sequence, from, to, true));
              seq_positions_2.addAll(
                  positionalInformationFor(positions_2, index_assembled_sequence, from, to, true));
              seq_positions_3.addAll(
                  positionalInformationFor(positions_3, index_assembled_sequence, from, to, true));
      ...
      if (no_of_sets > 0) {
        MessageBox mb =
            new MessageBox(
                ...
                "The file "
                    + file
                    + " contains character sets. Do you want me to split the file into character sets?",
                MessageBox.MB_YESNOTOALL | MessageBox.MB_TITLE_IS_UNIQUE);
  ...
  /**
   * Tries to load a particular file into the present SequenceList. If successful, it will sequences
   * the present file to be whatever was specified. If not successful, it will leave the external
   * situation (file, sequences) unchanged.
   */
  ...
  public void addFile() {
    FileDialog fd =
        new FileDialog(matrix.getFrame(), "Which file would you like to open?", FileDialog.LOAD);
    fd.setVisible(true);
  ...
  public void exportAsNexus() {
    ...
    Dialog frame =
        (Dialog) CloseableWindow.wrap(new Dialog(matrix.getFrame(), "Export as Nexus ...", true));
    ...
    tf_interleaveAt.setText(
        new Integer(matrix.getPrefs().getPreference("exportAsNexus_interleaveAt", 1000, 0, 1000000))
            .toString());
      ...
      try {
        ...
        if (new MessageBox(
                    ...
                    "I can't figure out which number you mean by '"
                        ...
                        + "'. I'm going to use '1000' as a default length. Is that alright with you?",
                    MessageBox.MB_YESNO)
                .showMessageBox()
            == MessageBox.MB_NO) {
          return;
        ...
        matrix
            ...
            .exportAsNexus(
                ...
                ProgressDialog.create(
                    ...
                    "All your sequences are being exported as a single Nexus file into '"
                        + file
                        + "'. Sorry for the wait!"));
      } catch (IOException e) {
        MessageBox mb =
  ...
  public void quickExportAsNexusNonInterleaved() {
      ...
      matrix
          ...
          .exportAsNexus(
              ...
              ProgressDialog.create(
                  matrix.getFrame(),
                  "Please wait, exporting sequences ...",
                  "All your sequences are being exported as a single, non-interleaved Nexus file into '"
                      + file
                      + "'. Sorry for the wait!"));
    } catch (IOException e) {
      MessageBox mb =
    ...
    new MessageBox(
            matrix.getFrame(),
            "All done!",
            "All your sequences were exported into '" + file + "' as a non-interleaved Nexus file.")
        .go();
  }
  ...
  public void quickExportAsNakedNexus() {
      ...
      matrix
          ...
          .exportAsNexus(
              ...
              ProgressDialog.create(
                  matrix.getFrame(),
                  "Please wait, exporting sequences ...",
                  "All your sequences are being exported as a naked Nexus file into '"
                      + file
                      + "'. Sorry for the wait!"));
    } catch (IOException e) {
      MessageBox mb =
    ...
    new MessageBox(
            matrix.getFrame(),
            "All done!",
            "All your sequences were exported into '" + file + "' as a naked Nexus file.")
        .go();
  }
  ...
  /**
   * Export all the sequences, randomly picking columns in groups of X, into a single directory. Fun
  ...
  public void exportSequencesByColumnsInGroups() {
    ...
    Dialog dg =
        (Dialog)
            CloseableWindow.wrap(
                new Dialog(matrix.getFrame(), "Please choose your output options ...", true));
    ...
    choice_formats.select(
        matrix
            ...
            .getPreference(
                ...
                2,
                0,
                choice_formats.getItemCount() - 1)); // 2 == NexusFile, at some point of time
    dinp.setFile(
        new File(matrix.getPrefs().getPreference("exportSequencesByColumnsInGroups_fileName", "")));
    if (matrix
            .getPrefs()
    ...
    tf_rands.setText(
        matrix.getPrefs().getPreference("exportSequencesByColumnsInGroups_noOfRands", "10"));
    ...
    int index_sel =
        list_seqNames.indexOf(
            matrix
                .getPrefs()
                .getPreference("exportSequencesByColumnsInGroups_choiceRefTaxon", "None"));
    ...
    if (btn.wasClicked()) {
      matrix
          .getPrefs()
          .setPreference(
              "exportSequencesByColumnsInGroups_choice", choice_formats.getSelectedIndex());
      matrix
          .getPrefs()
          .setPreference("exportSequencesByColumnsInGroups_fileName", dinp.getFile().getParent());
      matrix
          .getPrefs()
          .setPreference(
      ...
      matrix
          .getPrefs()
          .setPreference("exportSequencesByColumnsInGroups_noOfRands", String.valueOf(rands));
        ...
        matrix
            ...
            .exportColumnsInGroups(
                ...
                ProgressDialog.create(
                    matrix.getFrame(),
                    "Please wait, exporting sequences ...",
                    "All your sequences are being exported as individual files into '"
                        + dinp.getFile()
                        + "'. Please wait!"));
      } catch (IOException e) {
        MessageBox mb =
      ...
      new MessageBox(
              matrix.getFrame(),
              "All done!",
              "All your sequences were exported into individual files in '" + dinp.getFile() + "'.")
          .go();
  ...
  public void exportSequencesByColumn() {
    ...
    choice_formats.select(
        matrix
            .getPrefs()
            .getPreference(
                "exportSequencesByColumn_choice", 0, 0, choice_formats.getItemCount() - 1));
    ...
    if (btn.wasClicked()) {
      ...
      matrix
          .getPrefs()
          .setPreference("exportSequencesByColumn_choice", choice_formats.getSelectedIndex());
      matrix
          .getPrefs()
   ...
   * Once the file has finished loading, incorporateSets() will use this data to subdivide the input
   * file.
   */
          ...
          if (name.startsWith(":") && !name.equals(":0")) {
            ...
            for (String compare_to : hashmap_codonsets.keySet()) {
              ...
              for (FromToPair compare_ftp : compare_list) {
                // System.err.println("Comparing position " + name + ":" + ftp + " with position " +
            ...
            for (String compare_to : hashmap_codonsets.keySet()) {
              ...
              ArrayList<FromToPair> compare_list = hashmap_codonsets.get(compare_to);
              for (FromToPair compare_ftp : compare_list) {
                // System.err.println("Comparing " + name + ":" + ftp + " with " + compare_to + ":"
  ...
  public boolean checkCancelledBeforeExport() {
    ...
    MessageBox mb =
        new MessageBox(
            ...
            "You have "
                + cancelled
                + " cancelled sequence(s) in your dataset. These sequences will not be exported! Are you sure you are okay with losing the data in these sequences?",
