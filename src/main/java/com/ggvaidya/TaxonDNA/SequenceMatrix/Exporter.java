/**
 * The Exporter is responsible for exporting sequences (and statistics), generally after checking
 * with matrix.getPrefs() over which preferences have been selected, and how the user would like the
 * files.
 */
...
public class Exporter implements SequencesHandler {
    ...
    //
    /**
     * Exports the table as a tab delimited file. This is a pretty brainless,
     * dump-everything-on-the-table-to-file function.
     */
    public void exportTableAsTabDelimited(File file) throws IOException {
        ...
        MessageBox mb =
                new MessageBox(
                        matrix.getFrame(),
                        "Export successful!",
                        "This table has been successfully exported to '"
                                + file
    ...
    /**
     * Exports the entire table as individual gene files (one per column) into a particular
     * directory. This really is kinda convoluted ... no, VERY convoluted. Hatred.
     *
     * <p>Note: because of that bug where the "please wait" dialog opens and closes too quickly, I'm
     * just going to ignore the 'delay' argument. Please reinsert if needed.
     */
    public void exportSequencesByColumn(
            File dir, FormatHandler fh, boolean writeNASequences, DelayCallback delay)
        ...
        while (i.hasNext()) {
                ...
                if (writeTo.exists()) {
                    ...
                    if (attempts > 100)
                        throw new IOException(
                                "Can't create file '" + writeTo + "' - it already exists!");
            ...
            if (writeTo.exists() && !writeTo.canWrite()) {
                throw new IOException(
                        "Couldn't open '"
                                + writeTo
                                + "' for writing. Are you sure you have permissions to write into"
                                + " this directory?");
            }
    ...
    public void exportColumnsInGroups(
            int total_randomizations,
            int per_group,
            int taxa_to_randomly_delete,
            File f_directory_to_export_to,
            String taxonToNeverDelete,
            FormatHandler handler,
            boolean bool_includeNAs,
            DelayCallback delay)
            throws DelayAbortedException, IOException {
        // new MessageBox(matrix.getFrame(), "Dude", "Okay, check this out: " + total_randomizations
        // +
        // ", " + per_group).go();
        ...
        if (f_directory_to_export_to == null
                || !f_directory_to_export_to.exists()
                || !f_directory_to_export_to.canWrite()) {
            new MessageBox(
                            matrix.getFrame(),
                            "Folder doesn't exist, or isn't writable!",
                            "You are trying to write columns in groups into "
                                    + f_directory_to_export_to
                                    + ", but it either doesn't exist, or you don't have permission"
                                    + " to write to it.")
                    .go();
            return;
        }
        ...
        if (list_columns == null) {
            new MessageBox(
                            ...
                            "You have no columns to export. Please import some data and try again."
                                + " If that doesn't help, this is probably a programming problem,"
                                + " and should be reported to the developers.")
                    .go();
            return;
        }
        ...
        for (int randomization = 0; randomization < total_randomizations; randomization++) {
            ...
            try {
                ...
                new MessageBox(
                                ...
                                "I'm trying to create '"
                                        + f_dir
                                        + "' to store the results of randomization #"
                                        + (randomization + 1)
                                        + ", but I don't seem to have adequate permissions to do"
                                        + " this.")
                        .go();
                return;
            }
            ...
            int group_id =
                    -1; // so we'll be '0' on the first iteration, for symmetry with randomization
            while (list_to_pick.size() > 0) {
                ...
                // wait, do we?
                if (list_to_add.size()
                        == 0) // the for loop didn't run, because we perfectly picked the size ...
                    // or something.
                    // In any case, nothing to add, what.
                    break;
                ...
                while (i.hasNext()) {
                    String colName = (String) i.next();
                    sg.addColumnFromDataStore(
                            tm.getDataStore(),
                            colName,
                            bool_includeNAs); // add this column to the SequenceGrid.
                }
    ...
    /**
     * Returns a String with the taxonset named 'name'. This is a string describing the taxonset as
     * numbers from (zero + offset) to (N + offset), where 'offset' is the second argument.
     *
     * @param name The name of the Taxonset
     * @param offset The offset of the taxon indexes. If this is zero, the first taxon will be zero,
     *     and if this is one, the first taxon will be one.
     */
        ...
        if (name.startsWith(Taxonsets.prefix_Length)) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Can't figure out length for " + name + " in Exporter.getTaxonset()");
            }
            ...
            try {
                ...
                throw new RuntimeException(
                        "Can't figure out charset count for "
                                + name
                                + " in Exporter.getTaxonset()");
            }
    ...
    /**
     * Exports the current matrix as a Sequences file. I wish we could use the main system to pull
     * off this particular trick, but it really is a whole lot easier to just write it out. Atleast
     * I can say that's because the output format is so incredibly well done :P.
     */
    public void exportAsSequences(File f, DelayCallback delay)
            throws IOException, DelayAbortedException {
        TableManager tm = matrix.getTableManager();
    ...
    public void exportAsPhylip(File f, DelayCallback delay)
            throws IOException, DelayAbortedException {
        TableManager tm = matrix.getTableManager();
    ...
    /**
     * Export the current matrix as Nexus. Note that this function might change or move somewhere
     * else -- I haven't decided yet.
     *
     * <p>The way the data is structured (at the moment, haha) is: 1. Hashtable[colName] --&gt;
     * Hashtable[seqName] --&gt; Sequence 2. We can get seqName lists, sorted.
     *
     * <p>The way it works is fairly simple: 1. If PREF_NEXUS_BLOCKS: for every column: write the
     * column name in comments for every sequence: write the column name write the sequence write
     * the length ; write the column name in comments ; 2. If PREF_NEXUS_SINGLE_LINE: for every
     * sequence name: for every column: see if an entry occurs in the column if not write in a
     * 'blank' ; ;
     *
     * <p>3. If PREF_NEXUS_INTERLEAVED: create a new sequence list
     *
     * <p>for every sequence name: for every column: if column has sequence: add sequence else add
     * blank sequence ; ; ;
     *
     * <p>use NexusFile to spit out the combined file on the sequence list.
     *
     * @throws IOException if there was a problem writing this file
     */
    public void exportAsNexus(File f, int exportAs, int interleaveAt, DelayCallback delay)
            throws IOException, DelayAbortedException {
        ...
        if (tx.getTaxonsetList() != null) {
            ...
            while (i.hasNext()) {
                countThisLoop++;
                if (delay != null) delay.delay(countThisLoop, v.size());
                ...
                if (str != null) buff_sets.append("\tTAXSET " + taxonsetName + " = " + str + ";\n");
            }
        }
        ...
        while (i.hasNext()) {
            countThisLoop++;
            if (delay != null) delay.delay(countThisLoop, tm.getCharsets().size());
            ...
            if (seqNames.size() > 0) {
                ...
                for (int x = 1; x <= 3; x++) {
                    ...
                    while (i_v.hasNext()) {
                        ...
                        if (ftp.from == ftp.to) {
                            array_strbuff_positions[x].append((horzOffset + ftp.from) + " ");
                    ...
                    if (buff_this_gene.length() > 0) {
                        // Woah! Content!
                        buff_this_gene.insert(
                                0,
                                "\tCHARSET "
                                        + fixColumnName(colName)
                                        + "_pos"
                                        + position_names[x]
                                        + " = ");
                        buff_this_gene.append(";\n");
        ...
        if (flag_display_nexus_positions) buff_sets.insert(0, buff_nexus_positions);
        ...
        if (how == Preferences.PREF_NEXUS_BLOCKS || how == Preferences.PREF_NEXUS_SINGLE_LINE) {
            ...
            writer.println(
                    "\tDIMENSIONS NTAX="
                            + tm.getSequencesCount()
                            + " NCHAR="
                            + tm.getSequenceLength()
                            + ";");

            writer.print("\tFORMAT DATATYPE=DNA GAP=- MISSING=? ");
            if (how == Preferences.PREF_NEXUS_BLOCKS) writer.print("INTERLEAVE");
            writer.println(";");
        ...
        if (how == Preferences.PREF_NEXUS_BLOCKS) {
            ...
            while (i_cols.hasNext()) {
                ...
                if (!nakedNexusMode) writer.println("[beginning " + fixColumnName(colName) + "]");
                ...
                if (!nakedNexusMode) writer.println("[end of " + fixColumnName(colName) + "]");
                writer.println(""); // leave a blank line
            }

        } else if (how == Preferences.PREF_NEXUS_SINGLE_LINE
                || how == Preferences.PREF_NEXUS_INTERLEAVED) {
            // loop over sequence names

            Iterator i_rows = tm.getSequenceNames().iterator();
            countThisLoop = 0;
            while (i_rows.hasNext()) {
                if (delay != null) delay.delay(countThisLoop, tm.getSequenceNames().size());
                ...
                if (how == Preferences.PREF_NEXUS_SINGLE_LINE) {
                    if (nakedNexusMode) writer.print(seqName.replaceAll("[^0-9A-Za-z]", "_") + " ");
                    else writer.print("'" + getNexusName(seqName) + "' ");
                ...
                while (i_cols.hasNext()) {
                    ...
                    if (how == Preferences.PREF_NEXUS_SINGLE_LINE) writer.print(seq.getSequence());
                    else if (how == Preferences.PREF_NEXUS_INTERLEAVED)
                        seq_interleaved = seq_interleaved.concatSequence(seq);
                    else
                        throw new RuntimeException(
                                "'how' makes no sense in SequenceGrid.exportAsNexus()! [how = "
                                        + how
                                        + "]");
                }

                if (how == Preferences.PREF_NEXUS_INTERLEAVED) seq_interleaved.changeName(seqName);

                if (how == Preferences.PREF_NEXUS_SINGLE_LINE) {
                    if (nakedNexusMode) writer.println("");
                    else writer.println(" [" + length + " bp]");
                } else if (how == Preferences.PREF_NEXUS_INTERLEAVED) list.add(seq_interleaved);
            }
        }
        ...
        if (how == Preferences.PREF_NEXUS_BLOCKS || how == Preferences.PREF_NEXUS_SINGLE_LINE) {
            ...
            if (!nakedNexusMode) writer.println(buff_sets);
        ...
        if (how == Preferences.PREF_NEXUS_INTERLEAVED) {
            if (nakedNexusMode) buff_sets = new StringBuffer(); // Blank out the sets.
            ...
            nf.writeNexusFile(
                    ...
                    ProgressDialog.create(
                            ...
                            "Writing out the compiled sequences. Sorry for not warning you about"
                                    + " this before. Almost done!"));
        }
    }
    ...
    /**
     * Export the current matrix as TNT. Note that this function might change or move somewhere else
     * -- I haven't decided yet.
     *
     * <p>TODO: interleaved: we really ought to output this as [ACTG], etc.
     *
     * @throws IOException if there was a problem writing this file
     */
    public void exportAsTNT(File f, DelayCallback delay) throws IOException, DelayAbortedException {
        ...
        if (tx.getTaxonsetList() != null) {
            if (tx.getTaxonsetList().size() >= 32) {
                new MessageBox(
                                matrix.getFrame(),
                                "Too many taxonsets!",
                                "According to the manual, TNT can only handle 32 taxonsets. You"
                                        + " have "
                                        + tx.getTaxonsetList().size()
                                        + " taxonsets. I will write the remaining taxonsets into"
                                        + " the file title, from where you can copy it into the"
                                        + " correct position in the file as needed.")
                        .go();
            }
            ...
            while (i.hasNext()) {
                ...
                if (str != null) {
                    ...
                    else buff_title.append("=(" + taxonsetName + ") " + str + "\n");
                    x++;
                }
            }
                    ...
                    if (v != null) {
                        ...
                        while (i_v.hasNext()) {
                            ...
                            if (ftp.from == ftp.to) {
                                positionalData
                                        .get(charsetName)
                                        .append((horzOffset + ftp.from - 1) + " ");
                            } else {

                                // Iterate, iterate.
                                for (int y = (horzOffset + ftp.from);
                                        y <= (horzOffset + ftp.to);
                                        y += increment_by) {
        ...
        for (Object a : charsetNames) {
            ...
            if (positionalData.get(x) == null) continue;
        ...
        if (number_of_columns >= 32) {
            new MessageBox(
                            ...
                            "According to the manual, TNT can only handle 32 character sets. You"
                                    + " have "
                                    + number_of_columns
                                    + " character sets. I will write out the remaining character"
                                    + " sets into the file title, from where you can copy it into"
                                    + " the correct position in the file as needed.")
                    .go();
        }
        ...
        if (colid > (codonsets_go_into_title_at - 1)) buff_title.append("\n;");
        ...
        if (buff_title.length() > 0) {
            writer.println(
                    "Additional taxonsets and character sets will be placed below this line.");
     ...
     *
     * @param movesIn Should be '0' for sequences which move in 3s, and '3' for sequences which move
     *     in 3s.
     */
    private List<FromToPair> combineFTPs(List<FromToPair> list, int movesIn) {
        ...
        if (current_sequence != null) results.add(current_sequence);
