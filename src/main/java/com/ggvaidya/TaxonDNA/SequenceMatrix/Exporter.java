...
public class Exporter implements SequencesHandler {
    ...
    while (i.hasNext()) {
      ...
      if (writeTo.exists() && !writeTo.canWrite()) {
        throw new IOException(
            "Couldn't open '"
                + writeTo
                + "' for writing. Are you sure you have permissions to write into this directory?");
      }
  ...
  public void exportColumnsInGroups(
    ...
    if (f_directory_to_export_to == null
      ...
      new MessageBox(
              ...
              "You are trying to write columns in groups into "
                  ...
                  + ", but it either doesn't exist, or you don't have permission to write to it.")
          .go();
      return;
    }
    ...
    if (list_columns == null) {
      new MessageBox(
              ...
              "You have no columns to export. Please import some data and try again. If that doesn't help, this is probably a programming problem, and should be reported to the developers.")
          .go();
      return;
    }
    ...
    for (int randomization = 0; randomization < total_randomizations; randomization++) {
      ...
      try {
        ...
        new MessageBox(
                ...
                "I'm trying to create '"
                    ...
                    + (randomization + 1)
                    + ", but I don't seem to have adequate permissions to do this.")
            .go();
        return;
      }
      ...
      while (list_to_pick.size() > 0) {
        ...
        // wait, do we?
        if (list_to_add.size()
            == 0) // the for loop didn't run, because we perfectly picked the size ... or something.
        ...
        while (i.hasNext()) {
          ...
          sg.addColumnFromDataStore(
              tm.getDataStore(), colName, bool_includeNAs); // add this column to the SequenceGrid.
        }
  ...
  public String getTaxonset(String name, int offset) {
    ...
    if (name.startsWith(Taxonsets.prefix_Length)) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Can't figure out charset count for " + name + " in Exporter.getTaxonset()");
      }
  ...
  /**
   * Export the current matrix as Nexus. Note that this function might change or move somewhere else
   ...
   * column name in comments for every sequence: write the column name write the sequence write the
   * length ; write the column name in comments ; 2. If PREF_NEXUS_SINGLE_LINE: for every sequence
   * name: for every column: see if an entry occurs in the column if not write in a 'blank' ; ;
   *
  ...
  public void exportAsNexus(File f, int exportAs, int interleaveAt, DelayCallback delay)
      ...
      if (seqNames.size() > 0) {
        ...
        for (int x = 1; x <= 3; x++) {
          ...
          while (i_v.hasNext()) {
            ...
            // buff_nexus_positions.append("[" + horzOffset + "] (" + ftp.from + ") - (" + ftp.to +
            // ")" + str_end);
          ...
          if (buff_this_gene.length() > 0) {
            ...
            buff_this_gene.insert(
                0, "\tCHARSET " + fixColumnName(colName) + "_pos" + position_names[x] + " = ");
            buff_this_gene.append(";\n");
    ...
    if (how == Preferences.PREF_NEXUS_BLOCKS || how == Preferences.PREF_NEXUS_SINGLE_LINE) {
      ...
      writer.println(
          "\tDIMENSIONS NTAX=" + tm.getSequencesCount() + " NCHAR=" + tm.getSequenceLength() + ";");
    ...
    if (how == Preferences.PREF_NEXUS_BLOCKS) {
      ...
      while (i_rows.hasNext()) {
        ...
        } else if (how == Preferences.PREF_NEXUS_INTERLEAVED) seq_interleaved = new Sequence();
        ...
        while (i_cols.hasNext()) {
          ...
          if (how == Preferences.PREF_NEXUS_SINGLE_LINE) writer.print(seq.getSequence());
            ...
            throw new RuntimeException(
                "'how' makes no sense in SequenceGrid.exportAsNexus()! [how = " + how + "]");
        }
  ...
  public void exportAsTNT(File f, DelayCallback delay) throws IOException, DelayAbortedException {
    ...
    if (tx.getTaxonsetList() != null) {
      if (tx.getTaxonsetList().size() >= 32) {
        new MessageBox(
                matrix.getFrame(),
                "Too many taxonsets!",
                "According to the manual, TNT can only handle 32 taxonsets. You have "
                    + tx.getTaxonsetList().size()
                    + " taxonsets. I will write the remaining taxonsets into the file title, from where you can copy it into the correct position in the file as needed.")
            .go();
      }
        ...
        for (int x = 1; x <= 3; x++) {
          ...
          if (v != null) {
            ...
            while (i_v.hasNext()) {
              ...
              if (ftp.from == ftp.to) {
                positionalData.get(charsetName).append((horzOffset + ftp.from - 1) + " ");
              } else {
    ...
    if (number_of_columns >= 32) {
      new MessageBox(
              ...
              "According to the manual, TNT can only handle 32 character sets. You have "
                  + number_of_columns
                  + " character sets. I will write out the remaining character sets into the file title, from where you can copy it into the correct position in the file as needed.")
          .go();
    }
