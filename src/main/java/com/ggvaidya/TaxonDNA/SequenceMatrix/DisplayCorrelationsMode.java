/**
 * This is a DisplayMode which displays the sequences (as returned by dataStore) by their
 * correlations. This is VERY complicated to display, so here's to hoping we Get It Right.
 *
 * <p>CDM is a whole lot like PDM, except that they're nothing at all alike. At all. I can hardly
 * stress this enough. So, CDM was initially forked straight off the PDM code, but we've since gone
 * in and cut a lot of the PDM bits out - except that we'd still like to do PDM-like calculations
 * and all, so it's pretty odd. Oh well, pish. It's just another great Friday evening now.
 */
...
public class DisplayCorrelationsMode extends DisplayMode implements MouseListener, Runnable {
    ...
    private int ignore_col =
            -1; // the column component of the sequence to ignore while calculating correlation
    private int ignore_row =
    ...
    private class Score implements Comparable {
        ...
        /**
         * Full on constructor; sets the constant as well as the name of this 'score' and its
         * pairwise distance.
         */
        ...
        /**
         * This is the constructor everybody uses because they don't _want_ to play around with the
         * constant.
         */
        ...
        public int compareTo(Object o) {
            ...
            return constant
                    * ((int)
                            (com.ggvaidya.TaxonDNA.Common.DNA.Settings.makeLongFromDouble(
                                            getPairwise())
                                    - com.ggvaidya.TaxonDNA.Common.DNA.Settings.makeLongFromDouble(
                                            s.getPairwise())));
        }
    ...
    public List getSortedSequences(Set sequences) {
        ...
        if (seqName_top == null) {
            if (sortedSequences.size() > 0) seqName_top = (String) sortedSequences.get(0);
        } else {
        ...
        if (columnList == null || sequencesList == null || seqName_top == null) {
            ...
            // throw new RuntimeException("Cannot updateDisplay() - essential variables have not
            // been
            // set!");
            //
        ...
        if (override_reference_sequence != null) seqName_top = override_reference_sequence;

        // Okay: NOW, we need to calculate all the distances! Isn't this fun?!
        distances =
                new double[columnList.size()]
                        [sequencesList.size()]; // TODO: catch OutOfMemory here?
        ...
        for (int x = 0; x < columnList.size(); x++) {
            ...
            for (int y = 0; y < sequencesList.size(); y++) {
                ...
                if (seq_compare == null) {
                    ...
                    // System.err.println("No compare seq for (" + colName + ", " + seqName + ");
                    // seqName_top
                    // = " + seqName_top);
    ...
    private Hashtable ht_matches =
            new Hashtable(); // Hash String(geneName) -> Vector[String](descriptions)
    ...
    /**
     * This (if you'll believe it!) is the callback used by the lists to let us know what was
     * selected.
     */
    ...
    public double getCorrelation(int x, int y) {
        ...
        if (ignore_col != -1) {
            ...
            if (x != ignore_col && y != ignore_col) {
                ...
                if (correlations[x][y] != -1) return correlations[x][y];
                if (correlations[y][x] != -1) return correlations[y][x];
            }
        }
        ...
        for (int c = 1; c < sortedSequences.size(); c++) {
            if (c == ignore_row) {
                if (x == ignore_col || y == ignore_col) continue;
            }
        ...
        double r =
                (((double) n * sum_xy) - (sum_x * sum_y))
                        / (Math.sqrt(variable_x) * Math.sqrt(variable_y));
    ...
    public double getAverageR() {
        ...
        if (N == 1) return -1.0; // don't try this unless you've got atleast two columns
        ...
        for (int z = 0; z < initialSequencesList.size(); z++) {
            ...
            for (int y = 0; y < sortedColumns.size(); y++) {
                ...
                for (int x = 1; x < sortedSequences.size(); x++) {
                    ...
                    //					String name = seqName_z + ":" + seqName + ":" + colName + "\t" +
                    // (float)r2_initial
                    // + "\t" + (float)r2_final + "\t(" + (float) diff + ")";
                    //					TODO: we will need a way to 'fall back', in case the system's font
                    // doesn't
                    ...
                    if (identical(diff, 0.0)) str_direction = "\u2194"; // '<->' symbol
                    String name =
                            seqName_z
                                    + ":"
                                    + seqName
...
class CorrelationsColorRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
        ...
        if (isSelected == true) basicColor = Color.RED;
