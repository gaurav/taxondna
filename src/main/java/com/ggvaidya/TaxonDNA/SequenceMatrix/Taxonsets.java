/**
 * A frame which allows the user to select options for taxonsets. We also actually *determine* those
 * taxonsets. There are also other evil hacky things we will do with all of this. Cheers.
 */
...
public class Taxonsets implements WindowListener, ItemListener, ActionListener {
  ...
  public static final String prefix_CharSets =
      "CHARSETS_ATLEAST_"; // I hope this is understandable =/
  ...
  private Checkbox check_generateGeneSets =
      new Checkbox("Generate taxonsets for all taxa having information for a particular gene");

  private Button btn_Ok = new Button("OK");

  /**
   * Constructor. Sets up the UI (on the dialog object, which isn't madeVisible just yet) and *TBD*
   */
  public Taxonsets(SequenceMatrix matrix) {
    ...
    rl.add(
        new Label("Create a taxonset consisting of only those taxons which have at least "),
        RightLayout.FILL_3);
    ...
    rl.add(
        list_totalLength,
        RightLayout.NEXTLINE | RightLayout.STRETCH_X | RightLayout.STRETCH_Y | RightLayout.FILL_3);
  ...
  public void setupDefaults() {
    ...
    while (i.hasNext()) {
      ...
      list_totalLength.add(
          prefix_Length + x + ": Containing at least " + x + " bp combined length");
    ...
    while (i.hasNext()) {
      ...
      list_totalCharSets.add(
          prefix_CharSets + x + ": Containing at least " + x + " character sets");
  ...
  /** Our proxy for dialog's setVisible(). Please use this to 'activate' the dialog. */
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (src.equals(btn_Ok)) setVisible(false);
      ...
      try {
        ...
        MessageBox mb =
            new MessageBox(
                matrix.getFrame(),
                "I can't understand!",
                "You specify that the new taxonset should have '"
                    + newValue
                    + "' "
                    + name
                    + ", but '"
                    + newValue
                    + "' isn't a valid number! Please lets use a number I can understand, thanks.",
                MessageBox.MB_OK);
    ...
    if (src.equals(btn_totalLength_Delete) || src.equals(btn_totalCharSets_Delete)) {
      ...
      if (list.getItemCount() == 0) ((Button) src).setEnabled(false);
    }
  }

  /**
   * Handles Item events (such as whether the *_Delete buttons should be enabled or not should be
   * enabled or not).
   */
  public void itemStateChanged(ItemEvent e) {
    ...
    if (e.getStateChange() == ItemEvent.DESELECTED) {
      if (src.equals(list_totalLength)) btn_totalLength_Delete.setEnabled(false);

      if (src.equals(list_totalCharSets)) btn_totalCharSets_Delete.setEnabled(false);
    } else if (e.getStateChange() == ItemEvent.SELECTED) {
      if (src.equals(list_totalLength)) btn_totalLength_Delete.setEnabled(true);

      if (src.equals(list_totalCharSets)) btn_totalCharSets_Delete.setEnabled(true);
    }
  }
  ...
  /**
   * Returns a Vector of all the taxonSets we have (as String). You can then query this list against
   * Exporter.getTaxonset(String) to get the Taxonset in question (as a single line, etc.)
   */
