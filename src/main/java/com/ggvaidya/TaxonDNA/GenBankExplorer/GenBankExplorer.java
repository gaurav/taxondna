...
public class GenBankExplorer implements ActionListener, ItemListener, DropTargetListener {
  ...
  /**
   * Creates a new GenBankExplorer object. We create a new toplevel. Be warned that we do NOT spawn
   * a thread: if TaxonDNA wants to create a GenBankExplorer for whatever reason, it will have to do
      ...
      try {
        ...
        MessageBox mb =
            new MessageBox(
                ...
                "I couldn't write to '"
                    + f.getAbsolutePath()
                    + "'. Are you sure you have the necessary permissions, and that the disk isn't full?",
                MessageBox.MB_ERROR);
        mb.go();
