/**
 * ClustalMapping.java 'Maps' sequence names into Clustal-acceptable names, then back from Clustal
 * (once processing is completed).
 *
 * <p>How this works: 1. The exported AlignmentHelper input file is actually a complete FASTA file,
 * with fullNames changed. fullName(new) = "seq" + uniqueId; Here, the uniqueId is the GI, unless no
 * GI exists, in which case we use the Sequences' UUID (note that the UUID contains a '_').
 * Furthermore, the GI will NOT include the sequence information (i.e., if the reported GI is
 * '10241:&lt;1034-1234', SpeciesIdentifier will use only consider the '10241' as the GI number)
 *
 * <p>Example names include: &gt;seq12930110 (GI number) &gt;seqU11129439_103 (UUID)
 *
 * <p>Yes, this does mean complete duplication of the dataset, but the only alternative (feed in the
 * original file again) will involve a further complication of an already cluttered user interface,
 * we won't go there.
 *
 * <p>Note that where UUIDs are used, the original file is REPLACED (and saved back in its new
 * position), with '[uniqueid:11129439_103]' added to the end of the sequence. IF THIS HAPPENS, WE
 * WILL RESAVE THIS FILE.
 *
 * <p>2. To avoid confusion, we ALWAYS close the present file before we do anything else. This is
 * also confusing, but hopefully less so. Oh well.
 */
/*
    TaxonDNA
...
public class ClustalMapping implements ActionListener {
    ...
    public void createUI() {
        ...
        finp_clustalMapped =
                new FileInputPanel(
                        "Clustal-mapped Fasta file: ", FileInputPanel.MODE_FILE_WRITE, dialog);
        btn_exportClustalMapped = new JButton("Export to Clustal-mapped Fasta file");
        finp_finalOutput =
                new FileInputPanel("Final Fasta file: ", FileInputPanel.MODE_FILE_WRITE, dialog);
        ...
        rl.add(
                new Label("1. Please select a new file to export these sequences to:"),
                RightLayout.NONE);
        ...
        rl.add(
                new Label("2. Please select a file to export the clustal-mapped sequences to:"),
                RightLayout.NEXTLINE);
        rl.add(finp_finalOutput, RightLayout.NEXTLINE | RightLayout.STRETCH_X);
        rl.add(
                new Label(
                        "Any sequences not present in the Clustal-mapped Fasta file will be written"
                                + " to 'missing.txt' in the same directory as the final export file"
                                + " specified above."),
                RightLayout.NEXTLINE | RightLayout.STRETCH_X);
        ...
        if (e.getSource().equals(btn_exportClustalMapped)) {
            ...
            try {
                ...
                new FastaFile()
                        .writeFile(
                                ...
                                ProgressDialog.create(
                                        ...
                                        "The final Fasta file is being exported. Sorry for the"
                                                + " wait!"));
                        ...
                        MessageBox mb =
                                new MessageBox(
                                        ...
                                        "The file '"
                                                ...
                                                + " it?\n\n"
                                                + "If you say 'Yes' here, the file '"
                                                + f_missing
                                                + "' will be deleted.\n"
                                                + "If you say 'No' here, the file '"
                                                + f_missing
                                                + "' will not be altered. No missing file will be"
                                                + " saved.",
                                        MessageBox.MB_YESNO);
                    ...
                    if (f_missing != null) {
                        ...
                        new FastaFile()
                                .writeFile(
                                        ...
                                        ProgressDialog.create(
                                                explorer.getFrame(),
                                                "Please wait, exporting missing sequences to a"
                                                        + " separate Fasta file ...",
                                                "The Fasta file containing missing seuqences is"
                                                        + " being written out now. Please wait a"
                                                        + " moment!"));
                    }
                }
                ...
                new MessageBox(
                                explorer.getFrame(),
                                "All done!",
                                "Successfully exported " + desc_final + desc_missing + ".")
                        .go();
                ...
                new MessageBox(
                        ...
                        "There was an error writing to '"
                                ...
                                + "'. Try again, and ensure you have adequate permissions to the"
                                + " files you are trying to write to. The error which occured is: "
                                + exp.getMessage());
    ...
    private void loadSequenceList() {
        try {
            list_initial =
                    explorer.getSelectedSequenceList(
                            ProgressDialog.create(
                                    explorer.getFrame(),
                                    "Please wait, assembling sequences ...",
                                    "I am assembling all selected sequences preparatory to writing"
                                            + " them into a file. Sorry for the delay!"));
    ...
    /**
     * Imports all the data in inputFile (combined with the present file, which acts as a mapfile)
     * into a NEW sequenceSet, which is spawned off into it's own SpeciesIdentifier. (it's the only
     * way i can think of of doing this without confusing the user, or obliterating his dataset
     * without any complaints). Missing sequences will have the warning flag set, so they "fall" to
     * the bottom of the list.
     *
     * @return null; the final sequences are stored in list_final, and the missing sequences in
     *     list_missing.
     */
    public void importSequenceSet(SequenceList set_initial, File inputFile) {
        ...
        try {
            ...
            MessageBox mb =
                    new MessageBox(
                            getFrame(),
                            "There is an error in '" + error_occured_in + "'!",
                            "The following error occured while trying to read '"
                                    + error_occured_in
                                    + "'. Please make sure that it has been formatted correctly.");
            mb.go();
            return;
        } catch (DelayAbortedException e) {
            // go on
        ...
        } else list_missing = null; // nuffink missing

        list_final = set_final;
    }

    /**
     * This method checks to see if every sequence in the specified SequenceList has a (hopefully
     * unique, and we DO test this) identifier. We assume that GIs are completely unique. We also
     * create an arbitrary value called the '[uniqueid:([\d\-]+)]', which uniquely identifies the
     * sequence. Since we're the only one who uses the 'uniqueid', we'll work it entirely out of
     * here. Nobody else needs unique id's at this point, but in case they do ... err ... code's
     * going to move.
     *
     * @return true, iff unique IDs were generated. If we return false, we do NOT guarantee every
     *     Sequence will return a valid GI or uniqueid. We have already informed the user of this.
     */
    public boolean createUniqueIds(SequenceList list) {
        ...
        while (i.hasNext()) {
            ...
            if (id == null || id.equals("")) {
                // there is no GI
                //				if(!warned) {
                //					mb = new MessageBox(getFrame(), "Sequences without GI numbers detected!",
                // "Some of
                // the sequences in this dataset do not have GI numbers. To create Clustal input
                // files, I'm
                // going to have to allocate unique identifiers to these sequences. Doing so will
                // not cause
                // problems with this dataset, but it WILL rewrite the original file. THE CURRENT
                // FILE WILL
                // BE OVERWRITTEN, AND ANY CHANGES YOU'VE MADE WILL BE SAVED PERMANENTLY TO DISK.
                // Are you
                // sure you want to do this?", MessageBox.MB_YESNO);
                //					if(mb.showMessageBox() == MessageBox.MB_YES) {
                //						// yes! go ahead
                //						warned = true;
                //					} else {
                //						// no ... so, don't process at all.
                //						mb = new MessageBox(getFrame(), "Clustal export failed.", "I can't export
                // files for
                // Clustal without assigning unique identifiers to each sequence. Please re-run if
                // you would
                // like to export files for Clustal.");
                //						mb.go();
                //
                //						return false;
                //					}
        ...
        if (vec_nonIdentical.size() > 0) {
            new MessageBox(
                            ...
                            "The following sequences could not be exported, since they share GI"
                                + " numbers with other sequences in your dataset. They will be"
                                + " exported to a Fasta file entitled 'duplicates.txt' in the same"
                                + " directory as the Clustal-mapped Fasta file.\n"
    ...
    /**
     * Exports all the data in 'set' into an output file. This means we check to see if the present
     * dataset is "mappable", and change it if it isn't. The output file will contain a list of all
     * the sequences, in appropriate FASTA format (good for passing into AlignmentHelper). See docs
     * for this class on how this works.
     */
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if (id == null || id.equals("")) {
                    ...
                    Pattern p = Pattern.compile("\\[uniqueid:(.*)\\]");
                    Matcher m = p.matcher(seq.getFullName());
                    if (m.find()) id = "U" + m.group(1);
                    else {
                        //						System.err.println("Sequence: " + seq.getFullName());
                        throw new RuntimeException(
                                "Something wrong with this program in Clustal:exportSequenceSet()");
                    }
                }
                ...
                if (uniques.get(id) != null) {
                    ...
                    //					MessageBox mb = new MessageBox(getFrame(), "You have duplicate
                    // sequences!", "Two
                    // sequences in this dataset have identical GI numbers. I can't use this for a
                    // mapfile.
                    // Please delete one of the duplicate pair of sequences. The duplicate pair
                    // which caused
                    // this error was:\n\t" + seq.getFullName() +"\nand\nGI:\t" + id);
                    //					mb.go();
            ...
            if (sl_duplicateSequences.count() > 0) {
                try {
                    ...
                    new FastaFile()
                            .writeFile(
                                    f,
                                    sl_duplicateSequences,
                                    ProgressDialog.create(
                                            explorer.getFrame(),
                                            "Please wait, exporting duplicate sequences ...",
                                            "All duplicate sequences are now being exported to '"
                                                    + f
                                                    + "'. Please be patient!"));
                } catch (IOException e) {
                    // TBD TODO XXX
            ...
            MessageBox mb =
                    new MessageBox(
                            ...
                            "Success!",
                            no
                                    + " sequences were exported successfully. You may now run"
                                    + " Clustal on the sequences you specified. Once that is done,"
                                    + " please follow step 2 to retrieve the original sequences.");
            mb.go();
            ...
            MessageBox mb =
                    new MessageBox(
                            ...
                            "There was an error writing to '"
                                    + outputFile
                                    + "'. Are you sure you have write permissions to both the"
                                    + " Clustal input and the map file? The technical description"
                                    + " of this error is: "
                                    + e);
            mb.go();
            return false;
