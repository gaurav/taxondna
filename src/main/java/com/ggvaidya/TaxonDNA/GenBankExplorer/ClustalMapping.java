...
public class ClustalMapping implements ActionListener {
  ...
  public void createUI() {
    ...
    finp_clustalMapped =
        new FileInputPanel("Clustal-mapped Fasta file: ", FileInputPanel.MODE_FILE_WRITE, dialog);
    ...
    rl.add(
        new Label("1. Please select a new file to export these sequences to:"), RightLayout.NONE);
    ...
    rl.add(
        new Label(
            "Any sequences not present in the Clustal-mapped Fasta file will be written to 'missing.txt' in the same directory as the final export file specified above."),
    ...
    if (e.getSource().equals(btn_exportClustalMapped)) {
      ...
      try {
        ...
        if (list_missing != null && list_missing.size() > 0) {
            ...
            MessageBox mb =
                new MessageBox(
                    ...
                    "The file '"
                        ...
                        + f_missing
                        + "' will not be altered. No missing file will be saved.",
                    MessageBox.MB_YESNO);
          ...
          if (f_missing != null) {
            desc_missing =
                " and " + list_missing.size() + " missing sequences to '" + f_missing + "'";
        ...
        new MessageBox(
            ...
            "There was an error writing to '"
                ...
                + "'. Try again, and ensure you have adequate permissions to the files you are trying to write to. The error which occured is: "
                + exp.getMessage());
   ...
   * sequence. Since we're the only one who uses the 'uniqueid', we'll work it entirely out of here.
   * Nobody else needs unique id's at this point, but in case they do ... err ... code's going to
   * move.
   *
  ...
  public boolean createUniqueIds(SequenceList list) {
    ...
    while (i.hasNext()) {
      ...
      if (id == null || id.equals("")) {
        // there is no GI
        //				if(!warned) {
        //					mb = new MessageBox(getFrame(), "Sequences without GI numbers detected!", "Some of
        // the sequences in this dataset do not have GI numbers. To create Clustal input files, I'm
        // going to have to allocate unique identifiers to these sequences. Doing so will not cause
        // problems with this dataset, but it WILL rewrite the original file. THE CURRENT FILE WILL
        // BE OVERWRITTEN, AND ANY CHANGES YOU'VE MADE WILL BE SAVED PERMANENTLY TO DISK. Are you
        ...
        //						mb = new MessageBox(getFrame(), "Clustal export failed.", "I can't export files for
        // Clustal without assigning unique identifiers to each sequence. Please re-run if you would
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        if (uniques.get(id) != null) {
          ...
          //					MessageBox mb = new MessageBox(getFrame(), "You have duplicate sequences!", "Two
          // sequences in this dataset have identical GI numbers. I can't use this for a mapfile.
          // Please delete one of the duplicate pair of sequences. The duplicate pair which caused
          // this error was:\n\t" + seq.getFullName() +"\nand\nGI:\t" + id);
          //					mb.go();
      ...
      MessageBox mb =
          new MessageBox(
              ...
              "Success!",
              no
                  + " sequences were exported successfully. You may now run Clustal on the sequences you specified. Once that is done, please follow step 2 to retrieve the original sequences.");
      mb.go();
      ...
      MessageBox mb =
          new MessageBox(
              ...
              "There was an error writing to '"
                  + outputFile
                  + "'. Are you sure you have write permissions to both the Clustal input and the map file? The technical description of this error is: "
                  + e);
      mb.go();
      return false;
    } finally {
