...
public class GenBankFile {
    ...
    public class Section {
         ...
         */
        /**
         * The value() of an entry is the EXACT STRING which needs to be placed into the GenBank
         * file after the appropriate space for the name(). For instance, for REFERENCE: value() =
         * "2 (bases 1 to 5028) AUTHORS Roemer,T., Madden,K., Chang,J. and Snyder,M. TITLE Selection
         * of axial growth sites in yeast requires Axl2p, a novel plasma membrane glycoprotein
         * JOURNAL Genes Dev. 10 (7), 777-793 (1996) PUBMED 8846915" The section will be written
         * back to file as new String(name() + appropriate_spaces + value()), so the format had
         * better be <em>exactly</em> right.
         */
        public String value() {
            return buff.toString().trim();
        }
         ...
         * and in all user-facing surfaces (unless it's some sort of what-will-this-output
         * question). The individual Section classes are responsible for ensuring that the entry()
         * and value() methods are suitably synchronized. Section itself will return a spaceless
    ...
    /**
     * A class for a Location. These are locations as defined in section 3.4.12.2 of release 157.0
     * of NCBI-GenBank. To simplify, there are seven types of valid locations in a GenBank file.
     */
    ...
    public class Location {
        public class SingleLocation extends Location {
            ...
            public SingleLocation(String str) throws FormatException {
                if (str.indexOf("join") != -1) {
                    ...
                    throw new FormatException(
                            "Currently, 'join's inside other operators (such as 'complement') are"
                                    + " not supported at present. Sorry!");
                }
                ...
                if (index_to_extender != -1) {
                    to_extended = true;
                    str =
                            str.substring(0, index_to_extender)
                                    + str.substring(index_to_extender + 1);
                }
            ...
            public boolean doesIntersect(SingleLocation s) {
                if (getFrom() <= s.getFrom()
                        && getTo() >= s.getFrom()) { // intersects atleast at s.from
                    return true;
                } else if (getTo() >= s.getTo()
                        && getFrom() <= s.getTo()) { // insersects atleast at s.to
                    return true;
                }
            ...
            /**
             * Does this SingleLocation intersect with a Location? Now, a location has a full
             * powered way of doing this, and we should have the
             * SingleLocation.doesIntersect(SingleLocation) case above, so hopefully this won't just
             * sit around looping infinitely.
             */
        ...
        public Sequence getSubsequence(Sequence seq) throws SequenceException {
            ...
            for (int x = 0; x < locations.size(); x++) {
                sequence =
                        sequence.concatSequence(((Location) locations.get(x)).getSubsequence(seq));
            }
                ...
                switch (state) {
                    ...
                    case 3: // waiting for key-value pair OR id
                        if (tok.charAt(0) == '/') {
                            ...
                            if (index_eq == -1) {
                                // 								throw new FormatException("In locus " +
                                // getLocus().getName() + ": feature
                                // '" + tok + "' is invalid or incomplete (it ought to be
        ...
        try {
            ...
            while (true) {
                ...
                if (d != null)
                    d.delay(
                            count_lines,
                            Integer.MAX_VALUE); // reasonable amount of possible file lines
                count_lines++;
            }
        } catch (Exception e) {
