/**
 * The Exporter is ultimately in charge of all exports. To make things just a *little* bit easier,
 * the Exporter will be independent of the rest of the system. You pass the Exporter java.util.List
 * of SequenceContainers, and it'll export them IN THAT ORDER into any format you like. As in
 * SequenceMatrix, we'll have multiple format functions to handle this.
 */
...
public class Exporter {
    ...
    /**
     * Checks that a list of SequenceContainers is 'okay'. Eventually, this means we'll check to
     * make sure you're not exporting identical sequences, overlapping sequences, etc. For now, not
     * much.
     */
    ...
    public void reportIOException(IOException e, File f) {
        reportException(
                "Error: Could not access/write to file.",
                "I could not access/write to the file '"
                        + f.getAbsolutePath()
                        + "'! Are you sure you have permissions to read from or write to this"
                        + " file?\n\n"
                        + "The technical description of the error I got is: "
                        + e.getMessage());
    }
    ...
    /**
     * Export to multiple Fasta files. Note that this means we don't HAVE a container set. So,
     * instead: 1. We need to ask the user where he wants the file to go. 2. We need to get our
     * hands on the current FeatureBin. 3. Iterate through all the categories, then the
     * subcategories. 4. Come up with valid names ($name_$type.txt) in the directory specified. 5.
     * And, err ... that's it.
     *
     * @param min_species the minimum number of species to export. Files with less than min_species
     *     species will not be exported at all.
     */
    public void exportMultipleFasta(int min_species) {
        ...
        if (explorer.getViewManager().getGenBankFile() == null) {
            new MessageBox(
                            explorer.getFrame(),
                            "No file loaded!",
                            "There's no file to export (or a bug in the program). Either way, you"
                                    + " need to open a file.")
                    .go();
        }
        ...
        if (f_base == null) return; // cancelled
        ...
        MessageBox mb =
                new MessageBox(
                        ...
                        "Are you sure you want to export all files to "
                                + f_base
                                + "? I will refuse to overwrite any files in this directory, but if"
                                + " the export aborts, you might be left with an incomplete set of"
                                + " files.",
        ...
        if (mb.showMessageBox() == MessageBox.MB_YES) {
            ...
            ProgressDialog pd =
                    ProgressDialog.create(
                            ...
                            "I'm processing features; I'll start writing them in a bit. Sorry for"
                                    + " the wait!");
            ...
            while (i.hasNext()) {
                ...
                while (f.exists())
                    f =
                            new File(
                                    f_base,
                                    file_system_sanitize(list.getName() + "_" + number + ".txt"));
                ...
                try {
                    ...
                    reportException(
                            "Error exporting sequences!",
                            "The following error occured while combining the sequences: "
                                    + e.getMessage()
                                    + ". This is probably an error in the program itself.");
                } catch (IOException e) {
    ...
    // given a string, return the same string with all possibly controversial characters replaced
    // with
    // '_'
    ...
    /**
     * Export the java.util.List of SequenceContainers as a FASTA file. As you can see, we throw no
     * exceptions: any errors are reported to the user before we return.
     */
    ...
    public void export(java.util.List containers, FormatHandler fh) {
            ...
            count =
                    _export(
                            ...
                            ProgressDialog.create(
                                    ...
                                    "I am assembling all selected sequences in preparation for"
                                            + " export. Please give me a second!"),
                            false, // no_overwrite; we don't need this, since they've already
                            // acknowledged file
                            // existance, etc.
        ...
        MessageBox mb =
                new MessageBox(
                        ...
                        "Export successful!",
                        count
                                + " features were successfully exported to '"
                                + f.getAbsolutePath()
                                + "' in the "
                                + fh.getShortName()
                                + " format.");
    ...
    public int _export(
            ...
            int min_species)
            throws IOException, DelayAbortedException, SequenceException {
        ...
        ProgressDialog pd =
                ProgressDialog.create(
                        ...
                        "I am exporting "
                                ...
                                + "' in the "
                                + fh.getShortName()
                                + " format. Sorry for the wait!");

        fh.writeFile(f, sl, pd);
        return count;
    }

    public SequenceList combineContainers(java.util.List list, DelayCallback delay)
