...
public class Exporter {
  ...
  /**
   * Checks that a list of SequenceContainers is 'okay'. Eventually, this means we'll check to make
   * sure you're not exporting identical sequences, overlapping sequences, etc. For now, not much.
   */
  ...
  public void reportIOException(IOException e, File f) {
    reportException(
        ...
        "I could not access/write to the file '"
            + f.getAbsolutePath()
            + "'! Are you sure you have permissions to read from or write to this file?\n\nThe technical description of the error I got is: "
            + e.getMessage());
  }
   ...
   * instead: 1. We need to ask the user where he wants the file to go. 2. We need to get our hands
   * on the current FeatureBin. 3. Iterate through all the categories, then the subcategories. 4.
   * Come up with valid names ($name_$type.txt) in the directory specified. 5. And, err ... that's
   * it.
   *
  ...
  public void exportMultipleFasta(int min_species) {
    ...
    if (explorer.getViewManager().getGenBankFile() == null) {
      new MessageBox(
              explorer.getFrame(),
              "No file loaded!",
              "There's no file to export (or a bug in the program). Either way, you need to open a file.")
          .go();
    }
    ...
    MessageBox mb =
        new MessageBox(
            ...
            "Are you sure you want to export all files to "
                + f_base
                + "? I will refuse to overwrite any files in this directory, but if the export aborts, you might be left with an incomplete set of files.",
  ...
  // given a string, return the same string with all possibly controversial characters replaced with
  // '_'
    ...
    try {
      count =
          _export(
              ...
              false, // no_overwrite; we don't need this, since they've already acknowledged file
              // existance, etc.
