...
| `citing_works.csv` | **Phase 2 deliverable.** Flat one-row-per-paper table (2,509 + 1 header). Multi-valued fields are `;`-joined. OpenRefine-friendly. Regenerated by `../../scripts/analyze_phase2.py`. |
...
`citing_works.jsonl` is regenerated from `raw/` by `../../scripts/build_citing_works.py`; `citing_works.csv` is regenerated from `citing_works.jsonl` by `../../scripts/analyze_phase2.py`. Both are checked in for convenience but are fully derivable from the raw pages.
