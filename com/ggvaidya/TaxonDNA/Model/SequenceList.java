/**
 * SequenceList is a class to organise sequences into a "list".
 * It *is* a list (an ordered set of elements).
 *
 * SequenceLists are either created independently and filled (using the null
 * constructor) or loaded in from a file. Loading from a file can throw
 * IOExceptions (in case of IO-issues) as well as SequenceListExceptions.
 * SequenceListException wraps most other exceptions. If you want the
 * full detailed error message, you are free to use the static readFile()
 * function.
 *
 * I'm obviously in several minds over this, and I'm not sure how this
 * is going to end up. Assume all mixed up until further notice.
 *  
...
// 		
	...
	private static void initFormatHandlers() {
		if(formatHandlers.size() == 0) {
			formatHandlers.add(new com.ggvaidya.TaxonDNA.Model.formats.FastaFile());
			//formatHandlers.add(new com.ggvaidya.TaxonDNA.DNA.formats.MegaFile());
			formatHandlers.add(new com.ggvaidya.TaxonDNA.Model.formats.NexusFile());
			formatHandlers.add(new com.ggvaidya.TaxonDNA.Model.formats.TNTFile());
...
//			
	...
	/**
	 * Creates a SequenceList from the specified file using the specified
	 * formatHandler. Was there ever a more elegant way to create a
	 * SequenceList? No, there cannot be.
	 *
	 * @throws SequenceListException if any exception was thrown creating this SequenceList. 
	 * 		It wraps the exception and provides a useful message at the same time.
		...
		throws SequenceListException, DelayAbortedException 
	{
		...
		try {
			if(handler.mightBe(file))
				handler.appendFromFile(this, file, delay);
			else
				throw new SequenceListException(
						"The specified file '" + fullPath +
						"' does not seem to be in the " + handler.getShortName() + " format. " +
						"Try to use another import method to load this file, use 'Open' to let " +
						"this program try to guess the file format (it can understand " +
						formatsHandled + " formats), or convert it into one of these formats " +
						"before loading them up again.");
		} catch(IOException e) {
			throw new SequenceListException(
					"A system error occured while trying to read '" + 
					fullPath + "'. Are you sure that the file exists, " +
					"and you have the correct permissions to read it?\n" + 
					"Technical explanation: " + e, e);
			...
			throw new SequenceListException(
					"There is an error in the format for file '" + fullPath + 
	...
	/**
	 * Creates a SequenceList from a Collection of Sequences. Any collection will do (we just feed it into the addAll
	 * anyway). Note that this will leave the file as 'null', so you'll have to use changeFile(), etc. to make this
	 * SequenceList feel like it *belongs* somewhere.
	 *
	 */
...
//		
	...
	private class SortByName implements Comparator {
		public int 	compare(Object o1, Object o2) {
			DNASequence seq1 = (DNASequence) o1;
			DNASequence seq2 = (DNASequence) o2;
	...
	private class SortRandom implements Comparator {
		...
		/**
		 * Assign a random number to each and every
...
//	
	/**
	 * Resorts this class using the specified sorting scheme. The sorting schemes
	 * are integer constants of this class. There is no way to determine how
	 * this class is sorted, because I can't think of why anyone would want to
	 * do this. You just call 'resort' to whatever you want; we're smart enough
	 * not to resort if we're already sorted that way. Of course, this means
	 * that:
	 * 1.	Once sorted, we must maintain the sort in all additions/deletions.
	...
	public int resort(int sortMethod) {
		...
		if(sortMethod == SORT_RANDOM || sortMethod == SORT_RANDOM_WITHIN_SPECIES || sortedBy != sortMethod) {
...
//
	 ...
	 * multithreaded environment. We're going to try and implement this ourself in all our
	 * internal functions (e.g. resort), but you should too.
	 */
	public void lock() {
		if(DEBUG_LOCK)
...
//
//	6.	The following section reads SequenceLists out of files. We figure out which handler to use,
...
//
	 ...
	 */
	public static SequenceList readFile(File f, FormatHandler h, DelayCallback delay) throws SequenceListException, DelayAbortedException {
		initFormatHandlers();
	...
	public static SequenceList readFile(File file, DelayCallback delay, FormatListener listener) throws SequenceListException, DelayAbortedException {
		...
		while(i.hasNext()) {
			...
			if(i.hasNext())
				...
				validFormats.append("or " + handler.getShortName());
		}
...
//	
	/**
	 * Has this SequenceList been modified?
	 * TODO: This mechanism needs writing up, if it's any use,
	 ...
	 */
	public com.ggvaidya.TaxonDNA.Model.formats.FormatHandler getFormatHandler() {
	...
	/**
	 * Does this contain this Sequence?.
	 * TODO: Can be optimized. 
	 */
	public boolean contains(Object o) {
		DNASequence seq = (DNASequence) o;
		return sequences.contains(seq);
	}

	/**
	 * Returns an iterator over all the sequences currently in our
	 * sequencesbase. This is a normal, fail-fast iterator. I don't
	 * think it's guaranteed to be in order, but I think it is 
	 * guaranteed not to repeat elements. I think. Don't look at me,
	 * I'm just using Vector.iterator() for now.
	 */
	...
	public Iterator conspecificIterator(String speciesName) {
		...
		if(ht_species.get(speciesName) != null) {		
			...
			seq = (DNASequence) this.get(index);
		} else {
			String last_spName = null;

			while(index < sequences.size()) {
				seq = (DNASequence) sequences.get(index);
				if(seq != null) {
	...
	*/
	...
	public int getMaxLength() {
		...
		while(i.hasNext()) {
			...
			if(seq.getLength() > maxLength)
				maxLength = seq.getLength();		
		}
	...
	public String toString() {
		return "SequenceList(contains " + sequences.size() + " sequences, sorted by method " + sortedBy + ")";
	}
...
//
	 ...
	 */
	public void setFormatHandler(com.ggvaidya.TaxonDNA.Model.formats.FormatHandler handler) {
	 ...
	 */
	public boolean add(DNASequence seq) {
	...
	/**
	 * Retains only the elements in this collection; i.e. intersection of this and Collection c. Right now, this is
	 * unimplemented, until someone can figure out what it's good for.
	 */
	public boolean retainAll(Collection c) {
		throw new UnsupportedOperationException("If you need SequenceSet to do a retainAll(), you'll have to write it yourself!");
	}
	...
	public Object set(int x, Object o) {
		DNASequence seq = (DNASequence) o;
		DNASequence last = (DNASequence) sequences.set(x, seq);
		sortedBy = SORT_UNSORTED;
		return last;
	}	
...
//
	...
	public void test(TestController testMaster, DelayCallback delay) throws DelayAbortedException {
		...
		DNASequence seq = DNASequence.makeEmptySequence("Test pretest", 32);
		sl.add(seq);
		sl.add(seq);
		DNASequence seq1 = DNASequence.makeEmptySequence("Test test", 32);
		sl.add(seq1);
		sl.add(seq1);
		sl.add(seq1);
		sl.add(seq1);
		sl.add(seq1);
		seq = DNASequence.makeEmptySequence("Test posttest", 32);
		sl.add(seq);
		sl.add(seq);
...
class ConspecificIterator implements Iterator {
	...
	private static DNASequence target;
	private static int x;
	private static boolean justDeleted = false;
			
	public ConspecificIterator(SequenceList list, DNASequence target, int index) {
	...
	public boolean hasNext() {
		if(x < 0 || x >= list.count())
			return false;
		DNASequence seq = (DNASequence) list.get(x);
