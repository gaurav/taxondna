...
/*
    TaxonDNA
    Copyright (C) 2007, 2010 Gaurav Vaidya
...
//	
	 ...
	 */
	public DataSequence(String name, String seq) throws SequenceException {
		changeName(name);
		changeSequence(seq);
	}
	 ...
	 */
	public static DNASequence createSequence(String name, String seq) throws SequenceException {
		try {
			return new DNASequence(name, seq);
		} catch(SequenceException e) {
			return new DataSequence(name, seq);
		}
	}

	public static DNASequence promoteSequence(DNASequence x) {
		if(DataSequence.class.isAssignableFrom(x.getClass())) {
			try {
				return new DNASequence(x.getFullName(), x.getSequence());
			} catch(SequenceException e) {
				return x;
			}	
		} else
	...
	public DNASequence getSubsequence(int from, int to) throws SequenceException {
		...
		DNASequence seq = DataSequence.createSequence(getFullName() + " (segment:" + from + "-" + to + ":inclusive)", seq_str);
