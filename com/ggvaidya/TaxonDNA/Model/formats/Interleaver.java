...
public class Interleaver {
	 ...
	 */
	public DNASequence getSequence(String name)  {
		return (DNASequence) seqs.get(name);
	}

	/**
	 * Set OR REPLACE the sequence with the specified name.
	 */
	public void setSequence(String name, DNASequence seq) {
	...
	public void appendSequence(String name, String sequence) throws SequenceException {
		appendSequence(name, DataSequence.createSequence(name, sequence));
	}

	public void appendSequence(String name, DNASequence seq) {
		if(seqs.get(name) == null) {
			sequenceNames.add(name);
			seqs.put(name, seq);
		} else {
			// append it onto the currently existing 'Sequence'
			DNASequence our_seq = (DNASequence) seqs.get(name);
