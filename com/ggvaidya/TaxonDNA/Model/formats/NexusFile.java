					...
					if(name == null) {
						...
						try {
							...
							if(!isDatasetInterleaved || hash_names.get(name) == null) {
								...
								seq = new DataSequence(name, strseq);
								appendTo.add(seq);
								hash_names.put(name, seq);
							} else {
								// it DOES exist, append it
								seq = (DNASequence) hash_names.get(name);
...
//						}
		...
		while(i_names.hasNext()) {
			...
			DNASequence sold = (DNASequence) hash_names.get(str_name);
			DNASequence snew = DataSequence.promoteSequence(sold);
			if(!snew.equals(sold)) {
...
//					int to = -1;
	...
	public void writeNexusFile(File file, SequenceList set, int interleaveAt, String otherBlocks, DelayCallback delay) throws IOException, DelayAbortedException {
		...
		while(i_seqs.hasNext()) {
			...
			if(seq.getClass().equals(DataSequence.class)) {
				dataType = "STANDARD";
		...
		if(!interleaved) {
			...
			while(i_names.hasNext()) {
				...
				DNASequence seq = (DNASequence) names.get(name);
		...
		writer.println("END;\n");
                ...
                // might be helpful).
                DNASequence seq = (DNASequence) set.get(0);
                String str_end = "";
		...
		if(how == EXPORT_AS_BLOCKS) {
			...
			while(i_cols.hasNext()) {
				...
				while(i_seqs.hasNext()) {
					...
					DNASequence seq = grid.getSequence(colName, seqName);
			...
			while(i_rows.hasNext()) {
				...
				while(i_cols.hasNext()) {
					...
					if(seq == null)
						seq = DNASequence.makeEmptySequence(colName, grid.getColumnLength(colName));
