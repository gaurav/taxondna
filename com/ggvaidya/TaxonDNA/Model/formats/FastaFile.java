...
public class FastaFile extends BaseFormatHandler implements Testable {
	...
	public void appendFromFile(SequenceList list, File file, DelayCallback delay) throws IOException, SequenceException, FormatException, DelayAbortedException
	{
		...
		while(reader.ready()) {
			...
			if(pBlank.matcher(line).matches()) {
				...
				if(!name.equals("")) {
					try {
						DNASequence x = makeSequence(delay, name, seq);
						list.add(x);
	 ...
	 */
	private DNASequence makeSequence(DelayCallback delay, String name, String seq) throws SequenceException {
		...
		return DataSequence.createSequence(name, seq.replaceAll("\\s+", ""));	// delete whitespace characters from the sequence before adding.
	}
