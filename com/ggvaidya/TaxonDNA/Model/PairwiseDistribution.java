 ...
 * 	the PairwiseDistribution object goes out of context, this means a
...
public class PairwiseDistribution {
	...
	*/
	...
	private void distances_push(float new_distance) throws OutOfMemoryError {
		...
		if(size > memory) {
			// wtf?
			throw new RuntimeException("Programming error in PairwiseDistribution.class: Please inform the developer!");
		} else if(size == memory) {
			...
			if(size != 0)
				System.arraycopy(distances, 0, temp, 0, size);
			distances = temp;
...
//		System.err.println("ADD: " + distances[size]);
	 ...
	 */
	public PairwiseDistribution(SequenceList list, int type, DelayCallback delay) throws DelayAbortedException {
		list.lock();
		...
		while(i.hasNext()) {
			...
			if(type == PD_INTRA)
				...
				throw new RuntimeException("Programmer Error in PairwiseDistribution: Please inform the programmer!");		
...
//		}
	 ...
	 */
	private void _addIntra(SequenceList list, DNASequence query) {
...
//			System.err.println("DEBUG - Intra: " + query + " with " + seq);
	 ...
	 * in SequenceList 'list', and add it to this pairwise distrib.
	 */
	private void _addInter(SequenceList list, DNASequence query) {
		Iterator i = list.iterator();
		...
		while(i.hasNext()) {
			...
			if(query.getGenusName().equals(seq.getGenusName())) {
				// identical genera
				if(!query.getSpeciesEpithet().equals(seq.getSpeciesEpithet())) {
					// but non identical species
					//
					// however, only do it one way (half-table only)
					if(query.getSpeciesEpithet().compareTo(seq.getSpeciesEpithet()) < 0)
						distances_push((float)query.getPairwise(seq));	
//					System.err.println("DEBUG - Inter: " + query + " with " + seq);
	...
	/**
	 * Produces a String version of part of the distribution.
	 * This goes from d_from to d_to at intervals d_interval. d_interval can be negative,
	 * if you want.
	 *  
	 * There is one more argument: cumulDirection, the direction in which the cumulative
	 * frequency should count. If cumulDirection == CUMUL_FORWARD, then it increases,
	 * and if it is CUMUL_BACKWARD, it decreases (begins at 1.0 and tends to 0.0).
	 * Everything else is undefined.
	 *
	 */
	public String getDistributionAsString(double d_from, double d_to, double d_interval, char cumulDirection) {
		boolean cumul_forward = true;
		if(cumulDirection == CUMUL_BACKWARD)
			cumul_forward = false;
	 ...
	 */
	private void appendTableEntry(StringBuffer buff, String label, int freq, double perc, double cumul) {
		appendTableEntry(
				buff,
				label,
				String.valueOf(freq),
				String.valueOf(perc),
				String.valueOf(cumul)
			);
	}
	 ...
	 * Helper function, writes out a line in the 'table'
	 */
	private void appendTableEntry(StringBuffer buff, String distances, String freq, String perc, String cumul) {
	...
	/**
	 * Helper function, prints only the first 'len' characters of the string,
	 * and inserts spaces to make the columns line up.
	...
	public Vector getDistancesBetween(double d_from, double d_to) {
		...
		for(int x = 0; x < size; x++) {
			if(distances[x] >= d_from)
				count = true;
			if(distances[x] > d_to)
				count = false;

			if(count)
				vec.add(new Float(distances[x]));
		}
		return vec;
	}
